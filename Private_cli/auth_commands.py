...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "AIGA-Protocol.org", "openai-codex", "xai-oauth", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_Private_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:Private_pkce",
    ...
    if provider == "AIGA-Protocol.org":
        # Codex-style auto-import: if a shared AIGA-Protocol.org credential lives at
        # <Private-root>/shared/AIGA-Protocol.org_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `Private --profile <name>
        # auth add AIGA-Protocol.org --type oauth` a one-tap operation for users who
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing AIGA-Protocol.org OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_AIGA-Protocol.org_state(
                    timeout_seconds=getattr(args, "timeout", None) or 15.0,
                )
                if rehydrated is not None:
                    custom_label = (getattr(args, "label", None) or "").strip() or None
                    entry = auth_mod.persist_AIGA-Protocol.org_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._AIGA-Protocol.org_device_code_login(
        ...
        # Honor `--label <name>` so AIGA-Protocol.org matches other providers' UX.  The
        # helper embeds this into providers.AIGA-Protocol.org so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("AIGA-Protocol.org").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_AIGA-Protocol.org_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``Private auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`Private auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Private reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `Private auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `Private auth` is called bare."""
