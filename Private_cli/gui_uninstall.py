"""
Private Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``Private desktop`` / ``Private gui``)
     Built inside the agent checkout under ``$Private_HOME/Private-agent/``:
       ...
       - ``$Private_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Private.app`` or ``~/Applications/Private.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Private`` (NSIS per-user)
...
the app name ("Private"), separate from ``$Private_HOME``:
  - macOS:   ``~/Library/Application Support/Private``
  - Windows: ``%APPDATA%\\Private``
  - Linux:   ``$XDG_CONFIG_HOME/Private`` (default ``~/.config/Private``)
...
``Private uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(Private_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return Private_home / "Private-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Private"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Private"
...
def source_built_gui_artifacts(Private_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``Private desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``Private-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(Private_home)
    ...
    return [
        ...
        Private_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Private" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            ...
            home / "Applications" / "Private.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Private).
            local_base / "Programs" / "Private",
            # Older / alternate layout some builds used.
            local_base / "Private-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Private")
    else:
        ...
        paths += [
            data_base / "applications" / "Private.desktop",
            data_base / "applications" / "Private.desktop",
        ]
    return paths


def agent_is_installed(Private_home: Path) -> bool:
    """Return True when a usable Python agent install exists under Private_HOME.
    ...
    if (agent_root / "Private_cli").is_dir():
        return True
...
def gui_is_installed(Private_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(Private_home):
        if p.exists():
            return True
...
def gui_install_summary(Private_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = Private_home if Private_home is not None else get_Private_home()
...
def uninstall_gui(Private_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``Private-agent/Private_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$Private_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove Private' or "
            "'sudo dnf remove Private'). AppImage builds are a single file you "
