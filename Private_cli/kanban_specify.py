...
Used by ``Private kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``Private_cli/goals.py`` — same aux
...
Private_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("Private_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Private Agent board.
...
def _profile_author() -> str:
    """Mirror of ``Private_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("Private_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=Private_KANBAN_SPECIFY_MAX_TOKENS,
