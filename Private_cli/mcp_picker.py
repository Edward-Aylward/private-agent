"""MCP picker — interactive `Private mcp picker` (also the default `Private mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``Private mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `Private plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from Private_cli.cli_output import prompt_yes_no
from Private_cli.curses_ui import curses_single_select
from Private_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Private session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from Private_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `Private mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: Private mcp install <name>    Picker: Private mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Private is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Private — run `Private update` "
...
def show_catalog() -> None:
    """`Private mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`Private mcp picker` (and default `Private mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`Private mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `Private mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
