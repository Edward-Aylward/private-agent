"""
Profile management for multiple isolated Private instances.

Each profile is a fully independent Private_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.Private/profiles/<name>/`` by default.

The "default" profile is ``~/.Private`` itself — backward compatible,
zero migration needed.

Usage::

    Private profile create coder          # fresh profile + bundled skills
    Private profile create coder --clone  # also copy config, .env, SOUL.md, skills
    Private profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    Private -p coder chat                 # or via flag
    Private profile use coder             # set as sticky default
    Private profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See Private_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.Private``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   Private-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `Private profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `Private
...
# `Private skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Private infrastructure directories that only the default profile
         (``~/.Private``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_Private_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.Private) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "Private-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "Private", "default", "test", "tmp", "root", "sudo",
})

# Private subcommands that cannot be used as profile names/aliases
_Private_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the Private root, NOT to the current Private_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where Private_HOME points outside
    ``~/.Private``, profiles live under ``Private_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_Private_home() / "profiles"


def _get_default_Private_home() -> Path:
    """Return the default (pre-profile) Private_HOME path.

    In standard deployments this is ``~/.Private``.
    In Docker/custom deployments where Private_HOME is outside ``~/.Private``
    (e.g. ``/opt/data``), returns Private_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_Private_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Private installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its Private_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_Private_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, Private subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _Private_SUBCOMMANDS:
        return f"'{canon}' conflicts with a Private subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``Private -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``Private profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``Private profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Private config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_Private_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``Private update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.Private)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.Private/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `Private update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`Private profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `Private profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `Private -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches Private_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``Private profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "Private_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.Private).\n"
            "To remove everything, use: Private uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from Private_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set Private_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("Private_HOME")
    try:
        os.environ["Private_HOME"] = str(profile_dir)
        from Private_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["Private_HOME"] = old_home
        elif "Private_HOME" in os.environ:
            del os.environ["Private_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.Private/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: Private profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from Private_HOME.

    Returns ``"default"`` if Private_HOME is not set or points to ``~/.Private``.
    Returns the profile name if Private_HOME points into ``~/.Private/profiles/<name>``.
    Returns ``"custom"`` if Private_HOME is set to an unrecognized path.
    """
    from Private_constants import get_Private_home
    Private_home = get_Private_home()
    resolved = Private_home.resolve()

    default_resolved = _get_default_Private_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.Private itself — its parent is ~/ and its
        # directory name is ".Private", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: Private profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.Private itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.Private). "
            "Specify a different name: Private profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="Private_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"Private_{old_name}"
    legacy_old_host = f"Private.{old_name}"
    new_host = f"Private_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_Private_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("Private_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a Private_HOME path string.

    Called early in the CLI entry point, before any Private modules
    are imported, to set the Private_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
