#!/usr/bin/env python3
"""
Private CLI - Main entry point.

Usage:
    Private                     # Interactive chat (default)
    Private chat                # Interactive chat
    Private gateway             # Run gateway in foreground
    Private gateway start       # Start gateway as service
    Private gateway stop        # Stop gateway service
    Private gateway status      # Show gateway status
    Private gateway install     # Install gateway service
    Private gateway uninstall   # Uninstall gateway service
    Private setup               # Interactive setup wizard
    Private logout              # Clear stored authentication
    Private status              # Show status of all components
    Private cron                # Manage cron jobs
    Private cron list           # List cron jobs
    Private cron status         # Check if cron scheduler is running
    Private doctor              # Check configuration and dependencies
    Private honcho setup                    # Configure Honcho AI memory integration
    Private honcho status                   # Show Honcho config and connection status
    Private honcho sessions                 # List directory → session name mappings
    Private honcho map <name>               # Map current directory to a session name
    Private honcho peer                     # Show peer names and dialectic settings
    Private honcho peer --user NAME         # Set user peer name
    Private honcho peer --ai NAME           # Set AI peer name
    Private honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    Private honcho mode                     # Show current memory mode
    Private honcho mode [hybrid|honcho|local]  # Set memory mode
    Private honcho tokens                   # Show token budget settings
    Private honcho tokens --context N       # Set session.context() token cap
    Private honcho tokens --dialectic N     # Set dialectic result char cap
    Private honcho identity                 # Show AI peer identity representation
    Private honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    Private honcho migrate                  # Step-by-step migration guide: OpenClaw native → Private + Honcho
    Private version             Show version
    Private update              Update to latest version
    Private uninstall           Uninstall Private Agent
    Private acp                 Run as an ACP server for editor integration
    Private sessions browse     Interactive session picker with search

    Private claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: Private_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``Private_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``Private update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``Private_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, Private crashes on import and the user can't run
# ``Private update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import Private_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'Private' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``Private tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``Private.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("Private")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"Private", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"Private")
        # Windows: the .exe name is already ``Private.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* Private_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("Private_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".Private", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``Private_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("Private_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``Private_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("Private_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`Private --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from Private_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Private Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``Private --version`` before config/logging imports on Termux."""
    if os.environ.get("Private_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from Private_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from Private_cli.subcommands.cron import build_cron_parser
from Private_cli.subcommands.gateway import build_gateway_parser
from Private_cli.subcommands.profile import build_profile_parser
from Private_cli.subcommands.model import build_model_parser
from Private_cli.subcommands.setup import build_setup_parser
from Private_cli.subcommands.postinstall import build_postinstall_parser
from Private_cli.subcommands.whatsapp import build_whatsapp_parser
from Private_cli.subcommands.slack import build_slack_parser
from Private_cli.subcommands.login import build_login_parser
from Private_cli.subcommands.logout import build_logout_parser
from Private_cli.subcommands.auth import build_auth_parser
from Private_cli.subcommands.status import build_status_parser
from Private_cli.subcommands.webhook import build_webhook_parser
from Private_cli.subcommands.hooks import build_hooks_parser
from Private_cli.subcommands.doctor import build_doctor_parser
from Private_cli.subcommands.security import build_security_parser
from Private_cli.subcommands.dump import build_dump_parser
from Private_cli.subcommands.debug import build_debug_parser
from Private_cli.subcommands.backup import build_backup_parser
from Private_cli.subcommands.import_cmd import build_import_cmd_parser
from Private_cli.subcommands.config import build_config_parser
from Private_cli.subcommands.version import build_version_parser
from Private_cli.subcommands.update import build_update_parser
from Private_cli.subcommands.uninstall import build_uninstall_parser
from Private_cli.subcommands.dashboard import build_dashboard_parser
from Private_cli.subcommands.gui import build_gui_parser
from Private_cli.subcommands.logs import build_logs_parser
from Private_cli.subcommands.prompt_size import build_prompt_size_parser
from Private_cli.subcommands.memory import build_memory_parser
from Private_cli.subcommands.acp import build_acp_parser
from Private_cli.subcommands.tools import build_tools_parser
from Private_cli.subcommands.insights import build_insights_parser
from Private_cli.subcommands.skills import build_skills_parser
from Private_cli.subcommands.pairing import build_pairing_parser
from Private_cli.subcommands.plugins import build_plugins_parser
from Private_cli.subcommands.mcp import build_mcp_parser
from Private_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (Private tools, Private setup, Private model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'Private {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any Private module import.
#
# Many modules cache Private_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("Private_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.Private/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set Private_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `Private mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Private' own profile selector.
        """
        try:
    ...
    # after the subcommand (`Private chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors Private_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If Private_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.Private/profiles/coder or
    # /opt/data/profiles/coder).  If Private_HOME points to the Private root
    # instead (e.g. systemd hardcodes Private_HOME=/root/.Private), we must
    # still read active_profile — the user may have switched profiles via
    # `Private profile use` and the gateway should honour that choice.
    # See issue #22502.
    Private_home_env = os.environ.get("Private_HOME", "")
    if profile_name is None and Private_home_env:
        if Path(Private_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the Private root
    if profile_name is None:
        try:
            from Private_constants import get_default_Private_root

            active_path = get_default_Private_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set Private_HOME
    if profile_name is not None:
        try:
            from Private_cli.profiles import resolve_profile_env

            Private_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent Private from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["Private_HOME"] = Private_home
...
# Load .env from ~/.Private/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from Private_cli.config import get_Private_home
from Private_cli.env_loader import load_Private_dotenv

load_Private_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → Private_REDACT_SECRETS env
# var BEFORE Private_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_Private_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "Private_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["Private_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `Private` subcommands
...
try:
    from Private_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from Private_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if Private_constants not importable yet
...
from Private_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to Private_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (Private_cli.main._model_flow_*) keep resolving unchanged.
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_Private_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("Private_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("Private_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from Private_cli.config import get_env_path, get_Private_home, load_config
    from Private_cli.auth import get_auth_status

    # Determine whether Private itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from Private_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_Private_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from Private_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if Private has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Private to use their tokens.
    if _has_Private_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from Private_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, Private_bin
        cli_args: the original CLI arguments (everything after 'Private')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    Private_bin = container_info["Private_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo Private {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo Private {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  Private --tui --resume {target}")
    if title:
        print(f'  Private --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @Private/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@Private" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/Private-ink/package.json",
    "packages/Private-ink/index.js",
    "packages/Private-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``Private_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("Private_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``Private_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("Private_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    Private_home = os.environ.get("Private_HOME") or str(Path.home() / ".Private")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "Private_HOME": Private_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(Private_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(Private_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if Private_cli_dir is None:
        Private_cli_dir = Path(__file__).parent
    bundled = Private_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (Private_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("Private_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from Private_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("Private_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with Private_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset Private_TUI_DIR (e.g. `unset Private_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("Private_QUIET"):
    ...
    if tui_dev:
        # Keep the local @Private/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @Private/ink resolves through
        # packages/Private-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "Private-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from Private_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from Private_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="Private-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["Private_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["Private_PYTHON_SRC_ROOT"] = os.environ.get(
        "Private_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("Private_PYTHON", sys.executable)
    env.setdefault("Private_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["Private_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["Private_MODEL"] = model
        env["Private_INFERENCE_MODEL"] = model
    if provider:
        env["Private_TUI_PROVIDER"] = provider
        env["Private_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["Private_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["Private_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["Private_TUI_SKILLS"] = value
    if query:
        env["Private_TUI_QUERY"] = query
    if image:
        env["Private_TUI_IMAGE"] = image
    if checkpoints:
        env["Private_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["Private_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["Private_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["Private_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["Private_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["Private_ACCEPT_HOOKS"] = "1"
    ...
    # Private_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `Private --tui` try to
    ...
    # still set Private_TUI_RESUME themselves.
    env.pop("Private_TUI_RESUME", None)
    if resume_session_id:
        env["Private_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `Private update` so
    ...
    if code == 42:
        from Private_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``Private_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``Private kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``Private kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("Private_KANBAN_BOARD"):
        return
    try:
        from Private_cli.kanban_db import get_current_board

        os.environ["Private_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.Private/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Private — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Private still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``Private_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("Private_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'Private sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from Private_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from Private_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'Private doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Private isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  Private setup")
        print()

        from Private_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'Private setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from Private_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["Private_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.Private/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["Private_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["Private_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["Private_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from Private_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from Private_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from Private_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Private?")
        print()
    ...
    # has no creds.json.  Every subsequent `Private gateway` then paid a 30s
    ...
    session_dir = get_Private_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: Private gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `Private gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  Private gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Private Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  Private gateway")
        ...
        print()
        print("  Or install as a service: Private gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'Private whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from Private_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from Private_cli.config import stamp_install_method
    from Private_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ Private post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from Private_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``Private model``) and the setup wizard
    ...
    from Private_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("Private_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from Private_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'Private model' for "
                "available providers, or run 'Private doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in Private_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.Private/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.Private/.env if the active provider is not 'custom'.
    ...
    """
    from Private_cli.config import get_env_value, save_env_value, load_config
...
#
# Private uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `Private model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `Private model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`Private_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from Private_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``Private model`` flow, then route aux tasks to them here.
    """
    from Private_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from Private_cli.auth import _prompt_model_selection
    from Private_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from Private_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from Private_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from Private_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Private URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from Private_cli.curses_ui import curses_radiolist
...
# downstream call sites read `Private_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `Private` invocation — including
# fast paths like `Private --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``Private setup`` / ``Private model``.
    ...
    recover from a malformed paste without editing ``~/.Private/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from Private_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from Private_cli.config import save_env_value
    from Private_cli.secret_prompt import masked_secret_prompt
    ...
    from Private_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `Private setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from Private_constants import display_Private_home as _dhh_fn

            print(
                f"    Private will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Private CLI with a provider."""
    from Private_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from Private_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from Private_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from Private_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from Private_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from Private_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``Private slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: Private slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `Private slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from Private_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from Private_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from Private_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from Private_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `Private security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from Private_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from Private_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from Private_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from Private_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Private home directory to a zip file."""
    if getattr(args, "quick", False):
        from Private_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from Private_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Private backup from a zip file."""
    from Private_cli.backup import run_import

    run_import(args)


def _print_version_info(*, check_updates: bool = True) -> None:
    from Private_cli.banner import format_banner_version_label
...
def cmd_uninstall(args):
    """Uninstall Private Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from Private_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from Private_cli.uninstall import run_gui_uninstall
    ...
    from Private_cli.uninstall import run_uninstall
...
# Critical files that every ``Private`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``Private update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "Private_cli/main.py",
    "Private_cli/config.py",
    "Private_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``Private`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="Private-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``Private update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``Private_cli/web_dist/`` (per vite.config.ts
    outDir: "../Private_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "Private_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``Private`` launcher present but ``Private_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the Private venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — Private venv python3, no nix-shell overhead
    ...
    # a Private venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``Private update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``Private web``).
    ...
    # build path usable on a stock `py -m Private_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (Private web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``Private update`` can unconditionally call ``Private desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``Private desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $Private_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $Private_HOME."""
    from Private_constants import get_Private_home
    return get_Private_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``Private update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Private.app/Contents/MacOS/Private"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Private.exe",
            release_dir / "win-ia32-unpacked" / "Private.exe",
            release_dir / "win-arm64-unpacked" / "Private.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "Private",
            release_dir / "linux-unpacked" / "Private",
            release_dir / "linux-arm64-unpacked" / "Private",
            release_dir / "linux-arm64-unpacked" / "Private",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Private'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Private.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Private.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Private.exe: Access is
    ...
    an unrelated "Private" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Private is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Private.app/Contents/MacOS/Private  ->  app bundle = .../Private.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Private Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Private Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from Private_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["Private_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["Private_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "Private_root", None):
        env["Private_DESKTOP_Private_ROOT"] = str(Path(args.Private_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["Private_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  Private gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Private.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Private` rename: unpack-electron
                ...
                if purged:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Private.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Private.exe, close any")
                    print("  running Private desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> Private desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Private is
    ...
    if source_mode:
        print("→ Launching Private Desktop from source build...")
    ...
    print(f"→ Launching packaged Private Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``Private dashboard`` processes other than ourselves.

    ``Private dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``Private update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Private Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``Private dashboard`` as
    ...
    environment variable ``Private_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "Private dashboard",
        "Private_cli.main dashboard",
        "Private_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "Private.*dashboard"`) keeps us consistent
            # with `Private_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `Private update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``Private update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``Private update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: Private curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``Private dashboard`` processes.

    Called at the end of ``Private update`` (default ``reason``) and also
    from ``Private dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Private Desktop Electron app spawns this dashboard as a
    # backend child, it sets Private_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("Private_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    Private dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Private Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `Private update "
            f"--branch {branch}`, or update against main with `Private update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="Private-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"Private-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for Private-agent itself
        ...
        # GitHub ZIPs extract to Private-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"Private-agent-{branch}")
    ...
    from Private_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from Private_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "Private-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Private behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes Private completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Private couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Private couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Private behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Private couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Private couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `Private update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_Private_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_Private_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``Private update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from Private_constants import get_default_Private_root

    default_home = get_default_Private_root()
...
# Install-scoped breadcrumb dropped right before ``Private update`` mutates the
...
# survives and the next ``Private`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $Private_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``Private update`` left half-done.
    ...
    protocol stream (``Private acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `Private update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from Private_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``Private update`` is still progressing even if pip/uv itself is silent.
    """
...
def _Private_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "Private.exe",
        scripts_dir / "Private-gateway.exe",
    ]


def _detect_concurrent_Private_instances(
    ...
    The Private Desktop Electron app spawns ``Private.EXE`` as a backend child,
    so during ``Private update`` the user-invoked process and the desktop's
    ...
    shims (``Private.exe`` / ``Private-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``Private update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``Private.exe``) is a separate process from the Python
    ...
    for shim in _Private_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated Private.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``Private update``
    ...
    #      Private.exe sitting *under* a non-Private parent (e.g. a Private
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "Private.exe"
    lines = ["✗ Another Private.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Private Desktop, exit any open `Private` REPLs, and")
    lines.append("  stop the gateway (`Private gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `Private update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_Private_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``Private.exe``.
    ...
    when ``Private update`` runs, ``Private.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``Private.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next Private invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Private Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Private Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _Private_exe_shims(scripts_dir):
        ...
        print(
            "    Close Private Desktop, exit other `Private` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `Private update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_Private_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``Private.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``Private.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``Private`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_Private_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``Private.exe.old.*`` left by prior updates.

    Called early on every Private invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``Private update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `Private update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``Private.exe`` / ``Private-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_Private_exe`` for the rationale.
    """
    ...
    # hours later inside ``Private update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``Private update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``Private.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``Private`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `Private update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `Private update --force` after closing other Private processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from Private_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$Private_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from Private_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `Private update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``Private update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.Private/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``Private update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``Private update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.Private/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``Private update --gateway``) the update is already
    ...
    try:
        ...
        # Private_cli.config.get_Private_home to simulate setup failure.
        from Private_cli.config import get_Private_home as _get_Private_home

        logs_dir = _get_Private_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``Private update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from Private_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``Private update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from Private_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from Private_cli.config import recommended_update_command
        from Private_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``Private update`` without requiring a reinstall.  The
    ...
    ``Private`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/Private.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v Private'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/Private, code at /usr/local/lib/Private-agent).
    fhs_link = Path("/usr/local/bin/Private")
    ...
    path_comment = (
        "# Private Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of Private_HOME before running the update.
    ...
    Private_HOME directories.  The ``--backup`` flag on ``Private update``
    ...
    try:
        from Private_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_Private_home so the user sees ~/.Private/...
    try:
        from Private_constants import get_Private_home, display_Private_home

        home = get_Private_home()
        try:
            display_path = f"{display_Private_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  Private import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``Private update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Private Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Private Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `Private update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Private via pip (for PyPI installs)."""
    from Private_cli import __version__
    from Private_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "Private-agent"]
        ...
        cmd = [pipx, "upgrade", "Private-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "Private-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "Private-agent"]
    ...
    print("✓ Update complete! Restart Private to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Private Agent...")
    print()

    # On Windows, abort early if another Private.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_Private_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $Private_HOME, this is cheap
        ...
        try:
            from Private_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in Private_cli/config.py bricked
        # every user who ran ``Private update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``Private
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``Private update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_Private_home added to Private_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``Private`` launch finishes the
        ...
        # build.  ``Private desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``Private desktop`` shouldn't be forced into a full
        # Electron build by ``Private update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "Private_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `Private desktop` to retry)")
        ...
        # current — so copy it straight over ~/.Private/cache/model_catalog.json
        ...
        # modules in this process.  Reload Private_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_Private_home() added since the last release.
        try:
            import importlib
            import Private_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit Private_HOME so
        # it is not affected by sync_skills()'s module-level Private_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's Private_HOME env var points at the default or a named profile.
        try:
            from Private_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'Private config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: Private config migrate")
            else:
                print()
                print("Skipped. Run 'Private config migrate' later to configure.")
        else:
        ...
        try:
            from Private_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `Private curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``Private update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from Private_cli.tools_config import install_cua_driver
        ...
        # When running as ``Private update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_Private_home() / ".update_exit_code"
            try:
        ...
        try:
            from Private_cli.gateway import (
                is_macos,
            ...
            # Discover all Private-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. Private-gateway.service or Private-gateway-coder.service
                            ...
                            # path in `Private gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: Private gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: Private -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Private gateway unit files are still installed.
        # When both Private.service (from a pre-rename install) and the
        # current Private-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `Private update` surfaces the issue until the user migrates.
        try:
            from Private_cli.gateway import (
                has_legacy_Private_units,
                _find_legacy_Private_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_Private_units():
                print()
                print("⚠ Legacy Private gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_Private_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (Private.service) fight the current")
                print("  Private-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    Private gateway migrate-legacy")
                print()
        ...
        print("  Private model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``Private -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from Private_constants import display_Private_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `Private profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_Private_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → Private -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.Private)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  Private profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     Private -p {name} chat")
                else:
        ...
        from Private_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from Private_constants import get_Private_home as _hh
        ...
        from Private_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `Private profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → Private -p {name}  ({wrapper})")
        print()
        ...
        from Private_cli.profiles import profile_exists
        ...
        from Private_cli.profiles import rename_profile
        ...
        from Private_cli.profiles import export_profile
        ...
        from Private_cli.profiles import import_profile
        ...
        import tempfile
        from Private_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="Private_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  Private -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      Private -p {plan.manifest.name} chat")
        ...
        from Private_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `Private profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"Private -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from Private_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("Private_requires"):
            print(f"Requires:     Private {data['Private_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from Private_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.Private_requires:
        print(f"  Requires: Private {mf.Private_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``Private dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``Private dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No Private dashboard processes running.")
        return 0

    print(f"{len(pids)} Private dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No Private dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `Private update`.
    ...
    # launches the dashboard (`worker dashboard` → Private_HOME points into
    ...
    try:
        from Private_cli.profiles import get_active_profile_name
    ...
    if (
        ...
        reexec_argv = [
            sys.executable, "-m", "Private_cli.main",
            "-p", "default",
            "dashboard",
            "--port", str(args.port),
            "--host", args.host,
        ...
        env = os.environ.copy()
        # Drop the profile Private_HOME so the child binds the machine root.
        env.pop("Private_HOME", None)
        os.execvpe(sys.executable, reexec_argv, env)

    # Attach gui.log early so dashboard startup/build failures are captured in
    # the same logs directory as every other Private surface.
    try:
        from Private_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
    ...
    if "Private_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["Private_WEB_DIST"])
            if "Private_WEB_DIST" in os.environ
            else PROJECT_ROOT / "Private_cli" / "web_dist"
        )
    ...
    try:
        from Private_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from Private_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from Private_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from Private_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Private log files."""
    from Private_cli.logs import tail_log, list_logs
...
# so that in ``Private -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``Private_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``Private -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``Private`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("Private_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    if _is_tui_chat_launch(args):
        ...
        try:
            from Private_cli.mcp_startup import start_background_mcp_discovery
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from Private_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from Private_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["Private_DEFER_AGENT_STARTUP"] = "1"
            os.environ["Private_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["Private_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `Private --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_Private_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_Private_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_Private_home()}/memories/\n")
    else:
        from Private_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Private Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from Private_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from Private_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from Private_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from Private_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from Private_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from Private_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from Private_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from Private_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from Private_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for Private CLI."""
    # Cosmetic: make the process show up as 'Private' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from Private_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``Private.exe.old.*`` quarantine files left by previous
    # ``Private update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_Private_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``Private update``
    ...
    # ``Private skills install update``) merely defers recovery one launch;
    # under-matching (missing ``Private -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in Private_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from Private_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `Private model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from Private_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from Private_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in Private_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in Private_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in Private_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in Private_cli/subcommands/whatsapp.py)
    # =========================================================================
    build_whatsapp_parser(subparsers, cmd_whatsapp=cmd_whatsapp)

    # =========================================================================
    # slack command  (parser built in Private_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from Private_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in Private_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in Private_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in Private_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in Private_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in Private_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in Private_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from Private_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in Private_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in Private_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in Private_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in Private_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in Private_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in Private_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.Private/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo Private uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from Private_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in Private_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in Private_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in Private_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in Private_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from Private_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in Private_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``Private --help``, ``Private version``, ``Private logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from Private_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from Private_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in Private_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in Private_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `Private computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `Private tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: Private computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: Private computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in Private_cli/subcommands/mcp.py)
    # =========================================================================
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_Private_home() / "sessions"
            ...
            # Launch Private --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in Private_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in Private_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in Private_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in Private_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in Private_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in Private_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in Private_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in Private_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Private-Setup.exe success screen tells users to
    # run `Private desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in Private_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in Private_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in Private_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``Private -c Pokemon Agent Dev`` → ``Private -c 'Pokemon Agent Dev'``
    ...
    from Private_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'Private -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (Private hooks list, cron
