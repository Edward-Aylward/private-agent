"""
Private fallback — manage the fallback provider chain.
...
Subcommands:
  Private fallback [list]   Show the current fallback chain (default when no subcommand)
  Private fallback add      Pick provider + model via the same picker as `Private model`,
                           then append the selection to the chain
  Private fallback remove   Pick an entry to delete from the chain
  Private fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.Private/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from Private_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `Private model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  Private fallback add")
        print()
        return
    ...
    print("  Docs: https://Private-agent.AIGA-Protocol.orgresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `Private model`, then append the selection to the chain."""
    from Private_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `Private model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `Private fallback list` to view, or `Private fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``Private fallback [subcommand]``."""
