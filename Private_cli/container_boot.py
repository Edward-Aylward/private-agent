...
``$Private_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-Private-setup
(the stage2 hook) has chowned the volume and seeded $Private_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$Private_HOME``,
    not under ``profiles/``). The dispatcher in ``Private_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``Private gateway start`` (no ``-p``) targets. Without it,
    bare ``Private gateway start`` inside the container would land on
    ...
    ``$Private_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        Private_home: The container's Private_HOME (typically /opt/data).
            Profiles live under ``<Private_home>/profiles/<name>/``;
            the default profile lives at ``<Private_home>`` itself.
    ...
    # ``Private gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(Private_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(Private_home)
    ...
    profiles_root = Private_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `Private profile create` (config.yaml
            # is not — that comes later via `Private setup`). Use it as the
            ...
            # via ``Private -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(Private_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = Private_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("Private_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "Private":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `Private -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit Private
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the Private user) won't hit EACCES. See
...
def _write_reconcile_log(
    Private_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $Private_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    Private_home = Path(os.environ.get("Private_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        Private_home=Private_home, scandir=scandir,
    )
    for a in actions:
        print(
