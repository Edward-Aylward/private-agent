...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.Private/cache/model_catalog.json``.
...
def _cache_path() -> Path:
    ...
    return get_Private_home() / "cache" / "model_catalog.json"
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``Private update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.Private/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``Private model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
