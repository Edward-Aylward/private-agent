...
Add, remove, or reorder entries here — both `Private setup` and
`Private` provider-selection will pick up the change automatically.
"""
...
_Private_USER_AGENT = f"Private-cli/{_Private_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `Private model`
    flow without maintaining a separate static list.
    """
    from Private_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $Private_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $Private_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Private refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Private that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Private that doesn't ship
...
# this list:  Private model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `Private model` interactive picker
# ---------------------------------------------------------------------------
...
#
# Some vendors expose several Private provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `Private model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``Private model``, the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``Private auth add openai-codex`` without
    ``Private model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``Private model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    Private-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from Private_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — Private-agent
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``Private model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from Private_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from Private_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Private' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``Private_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Private should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``Private auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.Private/.env``.
    ...
    """
    try:
        from Private_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from Private_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Private release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Private release). Falls back to the hardcoded catalog if no token
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``Private model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $Private_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from Private_constants import get_Private_home
    return get_Private_home() / "provider_models_cache.json"
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _Private_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Private' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _Private_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from Private_constants import get_Private_home
    return get_Private_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Private will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Private cannot verify the model name."
