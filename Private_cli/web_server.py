"""
Private Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m Private_cli.main web          # Start on http://127.0.0.1:9119
    python -m Private_cli.main web --port 8080
"""
...
try:
    ...
    # running `Private dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["Private_WEB_DIST"]) if "Private_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``Private gateway run`` — but the
    desktop app spawns a ``Private dashboard`` backend, not a gateway, so a cron
    ...
    Private_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (Private_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `Private
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("Private_DESKTOP") == "1":
...
app = FastAPI(title="Private Agent", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# Private_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("Private_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Private-Session-Token"
...
#
# This list is defined in ``Private_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from Private_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Private-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from Private_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Private (no terminal prompt), "
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Private provider slug — fine.
    ...
    1. Vendor-name → Private-provider mapping: when the provider string is
       not a known Private provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from Private_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from Private_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from Private_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "Private_DASHBOARD_FILES_ROOT"
...
def _default_Private_root_is_opt_data() -> bool:
    raw = os.environ.get("Private_HOME", "").strip()
    if not raw:
        return False
    try:
        from Private_constants import get_default_Private_root

        root = get_default_Private_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    if not _local_dashboard_request(request) or _default_Private_root_is_opt_data():
...
async def get_status():
    ...
    try:
        from Private_cli.dashboard_auth import list_providers as _list_providers
    ...
    return {
        ...
        "Private_home": str(get_Private_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the Private home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_Private_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_Private_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_Private_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_Private_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_Private_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchroAIGA-Protocol.org*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from Private_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.Private/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_Private_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "Private-update": "Private-update.log",
...
def _spawn_Private_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``Private <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``Private_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "Private_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "Private_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``Private gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``Private gateway restart`` children race each other on the
    ...
    return _spawn_Private_action(["gateway", "restart"], "gateway-restart"), False
...
async def restart_gateway():
    """Kick off a ``Private gateway restart`` in the background."""
    try:
...
@app.post("/api/Private/update")
async def update_Private():
    """Kick off ``Private update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("Private-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "Private-update",
    ...
    try:
        proc = _spawn_Private_action(["update"], "Private-update")
    except Exception as exc:
        _log.exception("Failed to spawn Private update")
...
@app.get("/api/Private/update/check")
async def check_Private_update(force: bool = False):
    """Report whether a Private update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/Private/update`` actually runs ``Private update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Private version string
    ...
    try:
        ...
        if force:
            try:
                (get_Private_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.Private/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from Private_state import SessionDB
    from Private_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Private surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Private supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in Private_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None):
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `Private model`
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.Private/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from Private_cli.model_cost_guard import expensive_model_warning
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Private from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Private to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Private from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Private to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Private in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Private through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Private via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Private to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Private inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Private through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Private to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Private to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Private Agent").strip() or "Private Agent"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Private appears
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.Private/.anthropic_oauth.json`` — Private-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_Private_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_Private_oauth_credentials,
            _Private_OAUTH_FILE,
        )
    except ImportError:
        read_Private_oauth_credentials = None  # type: ignore
        _Private_OAUTH_FILE = None  # type: ignore

    Private_creds = None
    if read_Private_oauth_credentials:
        try:
            Private_creds = read_Private_oauth_credentials()
        except Exception:
            Private_creds = None
    if Private_creds and Private_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "Private_pkce",
            "source_label": f"Private PKCE ({_Private_OAUTH_FILE})",
            "token_preview": _truncate_token(Private_creds.get("accessToken")),
            "expires_at": Private_creds.get("expiresAt"),
            "has_refresh_token": bool(Private_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from Private_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Private even
    when they also have a separate Private-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "Private auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "Private auth add qwen-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "Private auth add anthropic",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("Private_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Private-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _Private_OAUTH_FILE
            if _Private_OAUTH_FILE.exists():
                _Private_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from Private_cli.auth import clear_provider_auth
...
# Guarded so Private web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Private file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``Private auth add anthropic``.
    """
    from agent.anthropic_adapter import _Private_OAUTH_FILE
    payload = {
    ...
    _Private_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _Private_OAUTH_FILE.with_name(
        f"{_Private_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _Private_OAUTH_FILE)
        try:
            _Private_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Private-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
# the tokens exactly like `Private auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("Private_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `Private auth add xai-oauth`'s credential-pool insert.
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``Private auth add minimax-oauth``.
    """
    from Private_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from Private_cli.auth import _save_codex_tokens
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `Private sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `Private sessions prune`)."""
...
async def get_logs(
    ...
    from Private_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_Private_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from Private_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, Private_HOME)."""
    from Private_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["Private_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process Private_HOME at import time. The dashboard is a single
...
#
# Wraps the same config data layer the CLI uses (Private_cli.mcp_config), so
# servers managed here show up under `Private mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from Private_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from Private_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from Private_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from Private_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # Private_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds Private_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_Private_action(
...
#
# Wraps the same JSON store the CLI uses (Private_cli.webhook); the webhook
...
async def list_webhooks():
    import Private_cli.webhook as wh
...
# spawn the real `Private gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_Private_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_Private_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from Private_cli.auth import read_credential_pool
...
# `Private memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `Private memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_Private_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_Private_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_Private_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `Private import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_Private_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from Private_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from Private_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <Private_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_Private_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_Private_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``Private`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from Private_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`Private -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the Private_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_Private_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "Private setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local Private_HOME override so the write lands in the target
    ...
    """
    from Private_constants import set_Private_home_override, reset_Private_home_override

    token = set_Private_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
        ...
        reset_Private_home_override(token)
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local Private_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the Private_HOME override. Returns the
    number of skills newly disabled.
    """
    from Private_constants import set_Private_home_override, reset_Private_home_override
    from Private_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's Private_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``Private profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from Private_HOME).
    """
    from Private_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``Private profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the Private_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``Private profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from Private_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_Private_home()`` at call
       time — the context-local override from ``set_Private_home_override``
    ...
    retargeted to the *current* ``get_Private_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a Private_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
    ...
    from Private_constants import (
        get_Private_home,
        set_Private_home_override,
        reset_Private_home_override,
    )
...
async def get_skills(profile: Optional[str] = None):
    ...
    from Private_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``Private tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``Private tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``Private tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.Private/.env`` via ``save_env_value`` —
    the same store ``Private tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``Private tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``Private -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same Private_HOME the rest of the
...
#
# The endpoint spawns the same ``Private --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from Private_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from Private_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from Private_cli.dashboard_auth.audit import AuditEvent, audit_log
        from Private_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``Private --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``Private_TUI_RESUME`` env var —
    matching what ``Private_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``Private_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``Private_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``Private_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_Private_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``Private -p <name>`` performs. The in-process
    ``Private_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from Private_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("Private_TUI_DISABLE_MOUSE", "1")
    env.setdefault("Private_TUI_INLINE", "1")

    if profile_dir is not None:
        env["Private_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["Private_TUI_RESUME"] = resume

    if sidecar_url:
        env["Private_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile Private_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["Private_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from Private_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Private inside WSL2 to use the dashboard's /chat "
...
# Private_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`Private_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from Private_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/Private/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /Private`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__Private_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/Private``)
        ...
        ``__Private_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__Private_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__Private_BASE_PATH__="{prefix}";'
                f"window.__Private_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__Private_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__Private_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__Private_BASE_PATH__="{prefix}";'
                f"window.__Private_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/Private`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Private backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.Private/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.Private/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_Private_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.Private/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as Private_cli.plugins):
    1. User plugins:    ~/.Private/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.Private/plugins/  (only if Private_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from Private_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_Private_home() / "plugins", "user"),
    ...
    # ``Private_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("Private_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".Private" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from Private_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_Private_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"Private auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from Private_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from Private_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from Private_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from Private_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.Private/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.Private/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"Private_dashboard_plugin_{plugin['name']}"
...
from Private_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from Private_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing Private_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Private Agent exposes a
    ...
    print(f"  Private Web UI → http://{host}:{port}")
