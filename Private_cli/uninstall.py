"""
Private Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.Private/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Private PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing Private-agent or Private PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Private Agent" comment and following line
                if '# Private Agent' in line or '# Private-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('Private' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing Private
                if 'Private' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the Private wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "Private",
        Path("/usr/local/bin/Private"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains Private_cli reference)
                content = wrapper.read_text()
                if 'Private_cli' in content or 'Private-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(Private_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``Private`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Private home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (Private_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``Private gateway run`` processes
    ...
    try:
        from Private_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from Private_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``Private_HOME`` and ``Private_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\Private\git\cmd``, ``%LOCALAPPDATA%\Private\git\bin``,
#      ``%LOCALAPPDATA%\Private\git\usr\bin``, ``%LOCALAPPDATA%\Private\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\Private\git\`` and Node to
#      ``%LOCALAPPDATA%\Private\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``Private dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\Private\gateway-service\`` and sometimes
...
def _Private_path_markers(Private_home: Path) -> list[str]:
    """Path-entry substrings that identify Private-owned User-PATH entries."""
    root = str(Private_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare Private-agent install dir.
    markers = [root + "\\Private-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if Private_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "Private".  We don't want to catch
    # unrelated entries like "cPrivate-foo" or "ephermeral", so we look for
    # backslash-Private as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(Private_home: Path) -> list[str]:
    """Strip Private-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _Private_path_markers(Private_home)
            kept: list[str] = []
...
def remove_Private_env_vars_windows() -> list[str]:
    """Delete Private_HOME and Private_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("Private_HOME", "Private_GIT_BASH_PATH"):
                try:
...
def _is_default_Private_home(Private_home: Path) -> bool:
    """Return True when ``Private_home`` points at the default (non-profile) root."""
    try:
        from Private_constants import get_default_Private_root
        return Private_home.resolve() == get_default_Private_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its Private_HOME directory.

    We shell out to ``Private -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current Private_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m Private_cli.main` so we don't depend on a `Private`
    #    wrapper that may be half-removed mid-uninstall.
    Private_invocation = [_sys.executable, "-m", "Private_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                Private_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's Private_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``Private uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$Private_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from Private_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(Private_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Private Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Private Chat GUI installation was found.")
        print(f"  Checked: {Private_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Private agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(Private_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Private agent at {Private_home / 'Private-agent'}")
        print(f"  • Your config, sessions, and secrets under {Private_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Private agent is still installed. Run 'Private' to use the CLI,")
    print("or 'Private uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.Private/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.Private/ for future reinstall
    """
    project_root = get_project_root()
    Private_home = get_Private_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie Private_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_Private_home(Private_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.Private data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Private Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {Private_home / 'config.yaml'}")
    print(f"  Secrets: {Private_home / '.env'}")
    print(f"  Data:    {Private_home / 'cron/'}, {Private_home / 'sessions/'}, {Private_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their Private_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Private data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Private code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``Private`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$Private_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in Private_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\Private\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(Private_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Private-owned PATH entries in User environment")

        log_info("Removing Private_HOME / Private_GIT_BASH_PATH User env vars...")
        removed_env = remove_Private_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Private-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing Private command...")
    ...
    #     (only when they still point into this Private home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Private-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(Private_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Private-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(Private_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE Private_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from Private_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(Private_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.Private/, just remove the Private-agent subdir
            if Private_home in project_root.parents or project_root.parent == Private_home:
    ...
    #     under Private_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(Private_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(Private_home)
    ...
    # 5. Optionally remove ~/.Private/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile Private_HOME dirs live under
        ...
        try:
            if Private_home.exists():
                ...
                log_success(f"Removed {Private_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {Private_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {Private_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {Private_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://Private-agent.AIGA-Protocol.orgresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://Private-agent.AIGA-Protocol.orgresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Private Agent! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m Private_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import Private_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``Private_constants`` + ``Private_cli.colors``
    (and lazily ``Private_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m Private_cli.uninstall")
    parser.add_argument(
        "--mode",
