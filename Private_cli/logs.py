"""``Private logs`` — view and filter Private log files.
...
Usage examples::
    ...
    Private logs                    # last 50 lines of agent.log
    Private logs -f                 # follow agent.log in real time
    Private logs errors             # last 50 lines of errors.log
    Private logs gateway -n 100    # last 100 lines of gateway.log
    Private logs gui -f            # follow gui.log (dashboard/pty/ws)
    Private logs desktop -f        # follow desktop.log (Electron app boot/backend)
    Private logs --level WARNING    # only WARNING+ lines
    Private logs --session abc123   # filter by session ID substring
    Private logs --component tools  # only tool-related lines
    Private logs --since 1h         # lines from the last hour
    Private logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_Private_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Private runs — try 'Private chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_Private_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_Private_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_Private_home()}/logs/")
        return

    print(f"Log files in {display_Private_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'Private chat' to generate logs)")
