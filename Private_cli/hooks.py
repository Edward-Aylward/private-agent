"""Private hooks — inspect and manage shell-script hooks.

Usage::

    Private hooks list
    Private hooks test <event> [--for-tool X] [--payload-file F]
    Private hooks revoke <command>
    Private hooks doctor

Consent records live under ``~/.Private/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.Private/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``Private hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: Private hooks {list|test|revoke|doctor}")
        print("Run 'Private hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.Private/config.yaml.")
        print("See `Private hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `Private hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `Private hooks test` and `Private hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Private wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `Private hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `Private hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `Private hooks doctor`.")
