"""
Interactive setup wizard for Private Agent.
...
# display_Private_home imported lazily at call sites (stale-module safety during Private update)
...
from Private_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from Private_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Private Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Private using environment variables or config commands:")
    print_info("  Private config set model.provider custom")
    print_info("  Private config set model.base_url http://localhost:8080/v1")
    print_info("  Private config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'Private setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from Private_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'Private setup')")


def _print_setup_summary(config: dict, Private_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'Private setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_AIGA-Protocol.org:
        ...
        try:
            ...
            from Private_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_AIGA-Protocol.org:
        ...
        try:
            ...
            from Private_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_AIGA-Protocol.org:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'Private setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'Private setup tts'"))
    else:
    ...
    # Spotify (OAuth via Private auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'Private setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {Private_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('Private setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('Private setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('Private setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('Private setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('Private setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('Private config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('Private config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('Private config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('Private', Colors.GREEN)}              Start chatting")
    print(f"   {color('Private gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('Private doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `Private tools` and `Private setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``Private model``)
    ...
    provider added to ``Private model`` is automatically available here.
    ...
    # Delegate to the shared Private model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from Private_cli.main import select_provider_and_model
    try:
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``Private model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from Private_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Private auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via Private setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'Private setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Private runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `Private setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `Private setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into Private_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("Private_MAX_ITERATIONS")
    ...
    print_info("  Run `Private setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # Private_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from Private_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
    ...
    try:
        ...
        if "/profiles/" in Private_home:
            profile_name = Private_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Private delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `Private update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under Private_HOME, and print
    ...
    """
    try:
        from Private_cli.slack_cli import _build_full_manifest
        from Private_constants import get_Private_home

        manifest = _build_full_manifest(
            bot_name="Private",
            bot_description="Your Private agent on Slack",
        )
        target = Path(get_Private_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `Private slack manifest --write` anytime to refresh after "
            "Private adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Private delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Private to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from Private_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from Private_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Private from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'Private setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     Private config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_Private_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo Private gateway install --system")
                print_info("  Or run in foreground:  Private gateway")
        else:
            from Private_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   Private gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   Private gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `Private setup tools` and `Private tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from Private_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from Private_cli.config
    ...
    if section_key == "model":
        ...
        from Private_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_Private.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_Private migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_Private", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Private.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Private to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Private at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Private at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Private at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Private at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Private equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Private config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Private.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Private's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(Private_home: Path) -> bool:
    ...
    print_info("Private can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: Private claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=Private_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: Private claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Private configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Private config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Private (use Private claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print_info("  Run `Private portal info` to inspect routing.")
    print_info("  Run `Private` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      Private setup           — full or quick (auto-detected)
      Private setup model     — just model/provider
      Private setup tts       — just text-to-speech
      Private setup terminal  — just terminal backend
      Private setup gateway   — just messaging platforms
      Private setup tools     — just tool configuration
      Private setup agent     — just agent settings
    """
    from Private_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Private Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Private Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Private Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, Private_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Private configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'Private setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(Private_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, Private_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {Private_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'Private config edit'")
    ...
    # Tune later with `Private setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, Private_home)
...
def _run_first_time_quick_setup(config: dict, Private_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'Private setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    Private setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  Private setup gateway")
    print()

    _print_setup_summary(config, Private_home)


def _run_quick_setup(config: dict, Private_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'Private setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Private to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'Private setup gateway'.")
