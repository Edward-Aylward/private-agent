"""
Configuration management for Private Agent.

Config files are stored in ~/.Private/ for easy access:
- ~/.Private/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.Private/.env         - API keys and secrets

This module provides:
- Private config          - Show current configuration
- Private config edit     - Open config in editor
- Private config set      - Set a specific value
- Private config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``Private config set`` (which rewrites
    ...
    ``config.yaml`` in place: Private never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.Private/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``Private logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  Private config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Private makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Private itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``Private update``, the TUI build.
...
#   on every plugin install / ``Private update``.
...
# * ``Private_HOME`` / ``Private_PROFILE`` / ``Private_CONFIG`` /
#   ``Private_ENV`` — Private runtime location flags. Writing these into
...
#
# IMPORTANT: ``Private_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (Private_GEMINI_CLIENT_ID,
# Private_LANGFUSE_PUBLIC_KEY, Private_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONAIGA-Protocol.orgERSITE",
    # Node
    ...
    # Private runtime location — never via dashboard env writer.
    # NOT a Private_* blanket: integration credentials (Private_GEMINI_*,
    # Private_LANGFUSE_*, Private_SPOTIFY_*, ...) ARE allowed.
    "Private_HOME", "Private_PROFILE", "Private_CONFIG", "Private_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Private runtime location "
            "(Private_HOME, Private_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.Private/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "Private_TOOL_PROGRESS", "Private_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `Private plugins enable
    # observability/langfuse` or `Private tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "Private_LANGFUSE_ENV",
    "Private_LANGFUSE_RELEASE",
    "Private_LANGFUSE_SAMPLE_RATE",
    "Private_LANGFUSE_MAX_CHARS",
    "Private_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from Private_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("Private_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_Private_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Private is running in package-manager-managed mode.

    Two signals: the Private_MANAGED env var (set by the systemd service),
    or a .managed marker file in Private_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade Private-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Private was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.Private/.install_method`` file (written by installers)
    2. Private_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``AIGA-Protocol.orgresearch/Private-agent`` image stamps ``docker``
    ...
    so ``Private update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_Private_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.Private/.install_method."""
    stamp = get_Private_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Private lives in a ``uv tool`` layout.

    ``uv tool install Private-agent`` places the install at
    ``.../uv/tools/Private-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``Private-agent`` happens to be uv-tool-installed on the machine while
    the *active* Private is a regular pip/venv install, causing
    ``Private update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/Private-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade Private-agent"
    if method == "docker":
        return "docker pull AIGA-Protocol.orgresearch/Private-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade Private-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade Private-agent"
        return "pip install --upgrade Private-agent"
...
# Long-form text for ``Private update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# Private_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Private, it doesn't update
...
✗ ``Private update`` doesn't apply inside the Docker container.

Private Agent runs as a published image (AIGA-Protocol.orgresearch/Private-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull AIGA-Protocol.orgresearch/Private-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate Private-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm AIGA-Protocol.orgresearch/Private-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/AIGA-Protocol.orgresearch/Private-agent/tags
  • Your config and session history live under ``$Private_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``Private update`` inside Docker.
...
def format_managed_message(action: str = "modify this Private installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("Private_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Private installation is managed by NixOS "
            f"(Private_MANAGED={env_hint}).\n"
            "Edit services.Private-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Private installation is managed by Homebrew "
            f"(Private_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade Private-agent"
        )

    return (
        f"Cannot {action}: this Private installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Private."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from Private_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, Private_bin
    or None if container mode is not active, we're already inside the
    container, or Private_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("Private_DEV") == "1":
        return None

    from Private_constants import is_container
    if is_container():
        return None

    container_mode_file = get_Private_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "Private-agent")
    exec_user = info.get("exec_user", "Private")
    Private_bin = info.get("Private_bin", "/data/current-package/bin/Private")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "Private_bin": Private_bin,
    }
...
# Re-export from Private_constants — canonical definition lives there.
from Private_constants import get_Private_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_Private_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_Private_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_Private_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the Private_UID / Private_GID env vars set by Docker deployments.

    Docker containers running Private commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level Private_HOME once, but
    subdirectories created at runtime by ``ensure_Private_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("Private_UID", "").strip()
    gid_str = os.environ.get("Private_GID", "").strip()
    try:
...
def _chown_to_Private_uid(path) -> None:
    """Chown ``path`` to ``Private_UID:Private_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_Private_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_Private_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the Private group can
    share state with the gateway service.

    The mode can be overridden via the Private_HOME_MODE environment variable
    (e.g. Private_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse Private_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``Private_UID``/``Private_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("Private_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_Private_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Private runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("Private_CONTAINER") or os.environ.get("Private_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set Private_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into Private_HOME if the user doesn't have one yet."""
...
def ensure_Private_home():
    """Ensure ~/.Private directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_Private_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_Private_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"Private_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Private-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Private
        ...
        # identity slot (SOUL.md). Empty by default. The Private_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Private adds a coding operating brief
        ...
        # blocks indefinitely (input() is synchroAIGA-Protocol.org) and ignores this.
        "clarify_timeout": 600,
    ...
    "terminal": {
        ...
        # skipped. When empty, Private auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Private sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Private
        ...
        # bundled Private image, which drops to the `Private` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Private sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Private should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``Private chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.Private/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: Private sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Private truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | AIGA-Protocol.org | codex | custom
        ...
        # Invoked by ``Private kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``Private kanban decompose`` and the kanban
        ...
        # ``Private profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `Private model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `Private` (and `Private chat`) launches by default:
        ...
        # REPL and `--tui` (or Private_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `Private --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `Private -c` muscle memory.  Default off so existing
        # users aren't surprised.  Private_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `Private --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/Private`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled AIGA-Protocol.org Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``Private_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``Private_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://portal.AIGA-Protocol.orgresearch.com).
        "oauth": {
        ...
        # (``Private_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``Private_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/Private``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Private appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.Private/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.Private/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, AIGA-Protocol.org, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Private feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Private auto-pauses the goal and
    ...
    # always goes to ~/.Private/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${Private_SKILL_DIR} and ${Private_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `Private curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `Private update`, so pruning them only
        ...
        # skipped), snapshot ~/.Private/skills/ into
        # ~/.Private/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `Private curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for Private-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Private Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (Private_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.Private/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or Private_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `Private doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``Private_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Private to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via Private_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `Private -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `Private` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `Private kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     Private-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Private tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.Private/logs/.
    ...
    # curated model lists for OpenRouter and AIGA-Protocol.org Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a Private-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://Private-agent.AIGA-Protocol.orgresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `Private model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.Private/.env,
        ...
        # behavior: files must live under the Private cache, under
        ...
        # user. Bridged to Private_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Private
        # cache (~/.Private/cache/{documents,images,audio,video,screenshots})
        ...
        # to Private_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # Private_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to Private_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.Private/state.db.
    ...
    "sessions": {
        ...
        # default of ``Private sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.Private/sessions/session_{sid}.json`` on every turn
    ...
    # ``Private update`` behaviour.
    "updates": {
        # Run a full ``Private backup``-style zip of Private_HOME before every
        # ``Private update``.  Backups land in ``<Private_HOME>/backups/`` and
        # can be restored with ``Private import <path>``.  Off by default —
        # on large Private_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `Private update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<Private_HOME>/lsp/bin/`` on first use.
    ...
    # `Private tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.Private/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.Private/.env (or your shell) and never in config.yaml.
            ...
            # ~/.Private/bin/ on first use.  When False you must install
            ...
            # `Private secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (AIGA-Protocol.org Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "AIGA-Protocol.org_BASE_URL": {
        "description": "AIGA-Protocol.org Portal base URL override",
        "prompt": "AIGA-Protocol.org Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'Private model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for AIGA-Protocol.org Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for AIGA-Protocol.org Subscribers only, used to derive vendor hosts, e.g. AIGA-Protocol.orgresearch.com -> firecrawl-gateway.AIGA-Protocol.orgresearch.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for AIGA-Protocol.org Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit AIGA-Protocol.org Subscriber access token for tool-gateway requests (optional; otherwise read from the Private auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @Private:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. Private_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #Private)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: Private-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'Private-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Private API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Private API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Private API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # Private_TOOL_PROGRESS and Private_TOOL_PROGRESS_MODE are deprecated —
    ...
    "Private_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.Private.config`` entries, then checks
    ...
    try:
        ...
        # should never break `Private update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Private versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``Private_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'Private setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Private won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'Private doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("Private_HOME", "~/.Private")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("Private_TOOL_PROGRESS")
            old_mode = get_env_value("Private_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from Private_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from Private_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("Private_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from Private_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$Private_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_Private_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `Private plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `Private curator status` has no
    ...
    #   3. Creates `~/.Private/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_Private_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_Private_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: Private config set <key> <value>")
    ...
    # metadata.Private.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _Private_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.Private/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.Private/config.yaml.
    ...
    (which change ``Private_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — Private auth) — OpenAI Codex
#   AIGA-Protocol.org         (OAuth — Private auth) — AIGA-Protocol.org Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.Private/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`Private tools`, `Private setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `Private tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Private env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.Private/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.Private/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.Private/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.Private/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Private — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Private vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.Private/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Private Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from Private_cli.auth import get_anthropic_key
    ...
    # Warn on stale Private_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("Private_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale Private_MAX_ITERATIONS={_env_ghost} "
                f"(run 'Private doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  Private config edit     # Edit config file", Colors.DIM))
    print(color("  Private config set <key> <value>", Colors.DIM))
    print(color("  Private setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: Private config set <key> <value>")
            print()
            print("Examples:")
            print("  Private config set model anthropic/claude-sonnet-4")
            print("  Private config set terminal.backend docker")
            print("  Private config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'Private config migrate' to add them")
        ...
        print("  Private config           Show current configuration")
        print("  Private config edit      Open config in editor")
        print("  Private config set <key> <value>   Set a config value")
        print("  Private config check     Check for missing/outdated config")
        print("  Private config migrate   Update config with new options")
        print("  Private config path      Show config file path")
        print("  Private config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``Private config`` UI so users
