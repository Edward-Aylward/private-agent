"""
Dump command for Private CLI.

Outputs a compact, plain-text summary of the user's Private setup
...
from Private_cli.config import get_Private_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.Private_build_sha`` by the Dockerfile's
    ``Private_GIT_SHA`` build-arg (see ``Private_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``Private dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from Private_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(Private_home: Path) -> int:
...
def _cron_summary(Private_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = Private_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"Private_home:      {display_Private_home()}")
    ...
    api_keys = [
        ...
        ("AIGA-Protocol.org_API_KEY", "AIGA-Protocol.org"),
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `Private auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `Private auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["Private-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(Private_home)}")
    lines.append(f"  skills:             {_count_skills(Private_home)}")
