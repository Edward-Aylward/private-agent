"""
Cron subcommand for Private CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(Private\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*Private)"
    r"|(systemctl\s+(restart|stop|start)\s+.*Private)"
    r"|(p?kill\s+.*Private.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'Private cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: Private gateway install", Colors.DIM))
        print(color("                    sudo Private gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    Private gateway install    # Install as a user service")
        print("    sudo Private gateway install --system  # Linux servers: boot-time system service")
        print("    Private gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `Private gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: Private cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
