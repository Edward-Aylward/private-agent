"""`Private checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.Private/checkpoints/``.  Actions:

    Private checkpoints               # same as `status`
    Private checkpoints status        # total size, project count, breakdown
    Private checkpoints list          # per-project checkpoint counts + workdir
    Private checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    Private checkpoints clear [-f]    # nuke the entire base (asks first)
    Private checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    Private checkpoints
    Private checkpoints prune --retention-days 3 --max-size-mb 200
    Private checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: Private checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``Private checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `Private checkpoints` → status
