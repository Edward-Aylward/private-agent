...
on Japanese locales, etc.).  Private's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``Private_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Private is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``Private_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("Private_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Private-managed Git install directories so
    ...
    # already-running shells don't see the change — which means Private
    ...
    # install without Private-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Private's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Private's
    ``Private config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Private (User env var in Windows Settings, or
    export in a PowerShell profile) and Private picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Private-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\Private\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``Private`` from right after install) retain their old PATH.

    Any subprocess Private spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Private-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "Private", "git", "cmd"),
        os.path.join(local_appdata, "Private", "git", "bin"),
        os.path.join(local_appdata, "Private", "git", "usr", "bin"),
        # Private venv Scripts directory — host of the Private.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes Private, but harmless to include.
        os.path.join(local_appdata, "Private", "Private-agent", "venv", "Scripts"),
