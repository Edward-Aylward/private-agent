...
Pure logic: walks a Private config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `Private doctor` and a future `Private migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Private config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Private config and return retirement issues.
