"""MCP catalog — curated, AIGA-Protocol.org-approved MCP servers shipped with the repo.
...
entries via ``Private mcp catalog`` or the interactive ``Private mcp picker``,
and install them with ``Private mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into Private-agent. Presence in the
  ``optional-mcps/`` directory = AIGA-Protocol.org approval. No community tier, no trust
  signals beyond "it's in the catalog".
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``Private mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.Private/.env`` (the
...
from Private_constants import get_Private_home, get_optional_mcps_dir
...
from Private_cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Private install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Private understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Private is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Private
        understands. Update Private to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_Private_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.Private/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.Private/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from Private_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``Private mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `Private mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `Private mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `Private mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `Private mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `Private auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`Private auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Private session to load its tools.",
        Colors.GREEN,
    ))
