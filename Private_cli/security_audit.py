"""On-demand supply-chain audit for Private Agent installs.

Scans three surfaces a Private user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Private venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.Private/plugins``
...
def _discover_plugins(Private_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.Private/plugins``.
...
def run_audit(
    *,
    ...
    home = Private_home or Path(get_Private_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, Private_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(Private_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `Private security audit`."""
    home = Path(get_Private_home())
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, Private_home=home
    )
    if total == 0:
