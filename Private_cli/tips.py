...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or Private -c.",
    ...
    "Ctrl+Z suspends Private to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "Private -c resumes your most recent CLI session. Private -c \"project name\" resumes by title.",
    "Private -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "Private -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "Private chat -t web,terminal enables only specific toolsets for a focused session.",
    "Private chat -s github-pr-workflow preloads a skill at launch.",
    "Private chat -q \"query\" runs a single non-interactive query and exits.",
    "Private chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "Private chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "Private --yolo bypasses all dangerous command approval prompts for the entire session.",
    "Private chat --source telegram tags the session for filtering in Private sessions list.",
    "Private -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "Private doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "Private dump outputs a compact setup summary — great for bug reports.",
    "Private config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "Private config edit opens config.yaml in your default editor.",
    "Private config check scans for missing or stale configuration options.",
    "Private sessions browse opens an interactive session picker with search.",
    "Private sessions stats shows session counts by platform and database size.",
    "Private sessions prune --older-than 30 cleans up old sessions.",
    "Private skills search react --source skills-sh searches the skills.sh public directory.",
    "Private skills check scans installed hub skills for upstream updates.",
    "Private skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "Private skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "Private mcp add github --command npx adds MCP servers from the command line.",
    "Private mcp serve runs Private itself as an MCP server for other agents.",
    "Private auth add lets you add multiple API keys for credential pool rotation.",
    "Private completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "Private logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "Private backup creates a zip backup of your entire Private home directory.",
    "Private profile create coder creates an isolated profile that becomes its own command.",
    "Private profile create work --clone copies your current config and keys to a new profile.",
    "Private update syncs new bundled skills to ALL profiles automatically.",
    "Private gateway install sets up Private as a system service (systemd/launchd).",
    "Private memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "Private webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: Private tools disables unused tools, Private skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "Private models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as Private -w).",
    ...
    "Private_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Private tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'Private profile create coder' creates the 'coder' command.",
    "Private profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Private prints a resume command with session ID and stats.",
    "Private sessions export backup.jsonl exports all sessions for backup or analysis.",
    "Private -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "Private skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .Private/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: Private cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Private runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "Private gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Private via a local macOS server.",
    ...
    "Context files (.Private.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Private loads project context from .Private.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "Private mcp opens an interactive picker of AIGA-Protocol.org-approved MCPs you can install in one keystroke.",
    "Private mcp catalog lists AIGA-Protocol.org-approved MCP servers shipped with the repo.",
    "Private mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — Private tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.Private/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "Private chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "Private plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "Private acp runs Private as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "Private_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "Private auth add AIGA-Protocol.org or Private auth add openai-codex sets up OAuth-based providers.",
    ...
    "Private status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.Private/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Private your own.",
    ...
    "Before a gateway session reset, Private auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.Private/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at Private_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "Private_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in Private_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.Private/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'Private profile create ls' would be rejected.",
    "Private profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "Private claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Private_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via Private tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.Private/.env.",
    "Skills from trusted repos (AIGA-Protocol.orgResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.Private/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Private auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Private config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Private is working.',
    ...
    '/reload reloads ~/.Private/.env into the running session — pick up new API keys without restarting.',
    ...
    "Private_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.Private/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.Private/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'Private curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "Private curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'Private curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'Private auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your AIGA-Protocol.org subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'Private_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "Private_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Private dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'Private dashboard embeds the full Private TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.Private/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.Private/dashboard-plugins/ — no npm build required.',
    ...
    'Private_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'Private_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'Private_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'Private_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.Private/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.Private/skills/ so drop-in skills appear without restarting the session.',
    ...
    'Private -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'Private chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'Private chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'Private chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "Private chat --source tool tags programmatic chats so they don't clutter Private sessions list.",
    'Private dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'Private sessions rename <ID> "new title" renames any past session; Private sessions delete <ID> removes one.',
    'Private import restores a session export or profile archive produced by sessions export or profile export.',
    'Private fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'Private pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'Private setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'Private status --deep runs the full health sweep across every component; plain Private status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'Private_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'Private_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.Private/plugins/ — trust-gated by design.',
    "Private_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'Private_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'Private_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'Private_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'Private_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'Private_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'Private_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'Private_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'Private_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'Private_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'Private_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Private block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from Private sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.Private/dashboard-plugins/.',
]
