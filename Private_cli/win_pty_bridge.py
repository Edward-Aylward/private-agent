"""Windows ConPTY bridge for the `Private dashboard` chat tab.

Drop-in counterpart to ``Private_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``Private_cli.web_server`` consumes: ``spawn``, ``read``,
