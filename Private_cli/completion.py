"""Shell completion script generation for Private CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_Private_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Private Agent bash completion
# Add to ~/.bashrc:
#   eval "$(Private completion bash)"

_Private_profiles() {{
    local profiles_dir="$HOME/.Private/profiles"
...
_Private_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_Private_profiles)" -- "$cur"))
        return
    fi
...
complete -F _Private_completion Private
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef Private
# Private Agent zsh completion
# Add to ~/.zshrc:
...
_Private_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.Private/profiles" ]]; then
        profiles+=($HOME/.Private/profiles/*(N/:t))
    fi
...
_Private() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_Private_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'Private command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Private Agent fish completion",
        "# Add to your config:",
        "#   Private completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __Private_profiles",
        "    echo default",
        "    if test -d $HOME/.Private/profiles",
        "        for d in $HOME/.Private/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c Private -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c Private -f -s p -l profile"
        " -d 'Profile name' -xa '(__Private_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__Private_profiles)' -d 'Profile name'"
                )
