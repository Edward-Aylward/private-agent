"""Slash command definitions and autocomplete for the Private CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Private config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Private works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Private is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.Private/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Private Agent to the latest version", "Info"),
    CommandDef("version", "Show Private Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`Private_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/Private`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Private
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``Private skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``Private skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``Private skills list`` and
...
# unrelated command landed. These claim their slots right after /Private,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /Private <command>).
...
# native slot, the alias spelling stays reachable via /Private reset).
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/Private <verb>`` subcommand.
    ...
    can still reach them via ``/Private <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/Private`` is always reserved as the first entry so the
    legacy ``/Private <subcommand>`` form keeps working for anything that
    ...
    # Reserve /Private as the catch-all top-level command.
    entries.append(("Private", "Talk to Private or run a subcommand", "[subcommand] [args]"))
    seen.add("Private")
    ...
    # everything except /Private, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://Private-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /Private handler.

    Maps both canonical names and aliases so /Private bg do stuff works
    the same as /Private background do stuff.

    Plugin-registered slash commands are included so ``/Private <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from Private_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
