...
Used by ``Private profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``Private_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Private Agent kanban board.
...
Rules:
  ...
  - Never write "Private Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_Private_home())
        else:
