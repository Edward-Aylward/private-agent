"""
Backup and import commands for Private CLI.

`Private backup` creates a zip archive of the entire ~/.Private/ directory
(excluding the Private-agent repo and transient files).

`Private import` restores from a backup zip, overlaying onto the current
Private_HOME root.
"""
...
from Private_constants import get_default_Private_root, get_Private_home, display_Private_home
...
# ``Private-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/Private-agent/``
# are not accidentally excluded.
_EXCLUDED_DIRS = {
    "Private-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to Private root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``Private-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/Private-agent/`` — must be preserved.
        if part == "Private-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside Private_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Private home directory."""
    Private_root = get_default_Private_root()

    if not Private_root.is_dir():
        print(f"Error: Private home directory not found at {Private_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"Private-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"Private-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_Private_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(Private_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(Private_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``Private-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "Private-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(Private_root)
    ...
    print(f"\nRestore with: Private import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Private backup.
    ...
    # Look for telltale files that a Private home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Private backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.Private/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a Private dir name
        if prefix in {".Private", "Private"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_Private_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (Private_root / "config.yaml").exists()
        has_env = (Private_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Private configuration.")
        ...
        for member in members:
            ...
            target = Private_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(Private_root.resolve())
            except ValueError:
        ...
        profiles_dir = Private_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # Private_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: Private profile list  (after installing Private)")

        # Guidance
        print()
        if not (Private_root / "Private-agent").is_dir():
            print("Note: The Private-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: Private update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  Private -p {pname} gateway install")

        print("Done. Your Private configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and Private backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to Private_HOME).
...
# — `Private update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(Private_home: Optional[Path] = None) -> Path:
    home = Private_home or get_Private_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    Private_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(Private_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside Private_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``Private update``.
    ...
    Args:
        ...
        Private_home: Override for the Private home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(Private_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for Private backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_Private_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, Private_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``Private_root`` to ``out_path``.
...
def _pre_update_backup_dir(Private_home: Optional[Path] = None) -> Path:
    home = Private_home or get_Private_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of Private_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<Private_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``Private update``) should continue even if the backup fails.
    """
    Private_root = Private_home or get_default_Private_root()
    if not Private_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(Private_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, Private_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `Private claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of Private_HOME under ``backups/`` before a
    ``Private claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``Private import <archive>``.  Writes to
    ``<Private_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `Private import` and the
