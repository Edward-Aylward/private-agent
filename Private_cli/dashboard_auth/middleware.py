...
from Private_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
from Private_cli.dashboard_auth.cookies import read_session_cookies
from Private_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /Private``, the
    ``login_url`` is prefixed (``/Private/login?next=...``) so the
    ...
    """
    from Private_cli.dashboard_auth.prefix import prefix_from_request
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["Private.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``Private_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `AIGA-Protocol.org` provider first, which
        # tries to reach AIGA-Protocol.org Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        if refreshed is not None:
            ...
            from Private_cli.dashboard_auth.cookies import (
                detect_https,
                set_session_cookies,
            )
            from Private_cli.dashboard_auth.prefix import prefix_from_request
        ...
        from Private_cli.dashboard_auth.cookies import clear_session_cookies
        from Private_cli.dashboard_auth.prefix import prefix_from_request
