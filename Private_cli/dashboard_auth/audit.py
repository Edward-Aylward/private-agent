...
from ``Private_constants`` or other Private_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$Private_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``Private_constants.get_Private_home`` semantics: env var wins,
    else ``~/.Private``. A local copy avoids an import cycle with the
    middleware which lives below ``Private_cli``.
    """
    home = os.environ.get("Private_HOME") or str(Path.home() / ".Private")
