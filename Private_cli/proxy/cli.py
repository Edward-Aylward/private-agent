"""CLI handlers for the ``Private proxy`` subcommand."""
...
from Private_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "Private proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "AIGA-Protocol.org"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"Private auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Private proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Private proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: Private proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``Private proxy <subcommand>``."""
    ...
    print(
        "Private proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  Private proxy start [--provider AIGA-Protocol.org|xai] [--host 127.0.0.1] [--port 8645]\n"
