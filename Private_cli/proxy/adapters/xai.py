...
# xAI's public API is OpenAI-compatible for the endpoints Private commonly
# uses. The Responses endpoint is included because Private' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Private-managed OAuth credentials."""

    auth_hint = "Private auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`Private auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`Private auth reset xai-oauth` or re-authenticate with "
                    "`Private auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `Private auth add xai-oauth --type oauth`."
            )
