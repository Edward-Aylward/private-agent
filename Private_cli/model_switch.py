...
- ``Private_cli.providers``        -- canonical provider identity + overlays
- ``Private_cli.model_normalize``  -- per-provider name formatting
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``Private config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from Private_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'Private model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'Private doctor' — config issues detected:"
        ...
        from Private_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from Private_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from Private_cli.providers import resolve_user_provider as _ruser
    ...
    # Mirrors the same logic in Private_cli.runtime_provider.resolve_runtime_provider;
    ...
    Private_warn = _check_Private_model_warning(new_model)
    if Private_warn:
        warnings.append(Private_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from Private_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from Private_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from Private_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    if "lmstudio" not in curated and (
        ...
        from Private_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Private-mapped providers ---
    from Private_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from Private_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for Private_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(Private_id)
        if (
            _alias_target
            and _alias_target != Private_id
        ...
        pconfig = PROVIDER_REGISTRY.get(Private_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (Private_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from Private_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(Private_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `Private model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(Private_id)
        if not model_ids:
            model_ids = curated.get(Private_id, [])
            if Private_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(Private_id, model_ids)
        total = len(model_ids)
    ...
    # Build reverse mapping: models.dev ID → Private provider ID.
    # Private_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Private IDs ("copilot").
    _mdev_to_Private = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in Private_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Private slug — e.g. "github-copilot" → "copilot"
        Private_slug = _mdev_to_Private.get(pid, pid)
        if Private_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(Private_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, Private_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or Private_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(Private_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", Private_slug, exc)
        ...
        # configured.
        if not has_creds and Private_slug == "anthropic":
            try:
                ...
                if (Private_creds and Private_creds.get("accessToken")) or \
        ...
        if Private_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(Private_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(Private_slug)
            ...
            if not model_ids:
                model_ids = curated.get(Private_slug, []) or curated.get(pid, [])
                if Private_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(Private_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": Private_slug,
            "name": get_label(Private_slug),
            "is_current": Private_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "Private",
        ...
        seen_slugs.add(Private_slug.lower())
        _record_builtin_endpoint(Private_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or Private_OVERLAYS (keeps /model in sync
    # with `Private model`).
    try:
        from Private_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Private's canonical write key (matches
            ...
            # Private writes ``models:`` as a dict keyed by model id
            # (see Private_cli/main.py::_save_custom_provider); older
            ...
            if api_url and api_key and discover:
                try:
                    from Private_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Private's own writer uses; a hyphen variant is accepted
            ...
            # active model. Private's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, Private_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`Private_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from Private_cli.models import fetch_openrouter_models
