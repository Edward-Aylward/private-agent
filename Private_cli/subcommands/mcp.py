"""``Private mcp`` subcommand parser.

Extracted from ``Private_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
from Private_cli.subcommands._shared import add_accept_hooks_flag
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Private as an MCP server",
        description=(
            "Manage MCP server connections and run Private as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'Private mcp add' to connect to a new server, or\n"
            "'Private mcp serve' to expose Private conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Private as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `Private mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `Private mcp install n8n`)",
    )
