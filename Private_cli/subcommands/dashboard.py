"""``Private dashboard`` subcommand parser.
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Private Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `Private dashboard` cmdlines and SIGTERM them directly — the same
    # path `Private update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Private dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running Private dashboard processes and exit",
    )
    # Backward-compat shim: older Private desktop app shells (<= 0.15.x) spawn the
    # backend as `Private dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Private couldn't start" with no actionable cause. Accept and silently
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://Private.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
