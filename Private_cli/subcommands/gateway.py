"""``Private gateway`` and ``Private proxy`` subcommand parsers.
...
def build_gateway_parser(subparsers, *, cmd_gateway: Callable, cmd_proxy: Callable) -> None:
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if Private_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy Private.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Private gateway unit files "
            "(e.g. Private.service) left over from older installs. Profile "
            "units (Private-gateway-<profile>.service) and unrelated "
