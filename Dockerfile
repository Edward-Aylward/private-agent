...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/Private/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when Private
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# Private process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main Private process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Private WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via Private_UID at runtime
RUN useradd -u 10000 -m -d /opt/data Private
...
#
# ui-tui/packages/Private-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/Private-ink/ ui-tui/packages/Private-ink/
...
# lock on the @Private/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/Private/.venv at first
...
COPY --chown=Private:Private . .

# ---------- Permissions ----------
# Make install dir world-readable so any Private_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the Private user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# Private_cli/main.py succeeds (see #18800). /opt/Private/web is build-time
# only (Private_WEB_DIST points at Private_cli/web_dist) and is intentionally
# not chowned here.
# /opt/Private/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the Private UID is remapped at boot (#27221).
# The .venv MUST remain Private-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/Private && \
    chown -R Private:Private /opt/Private/.venv /opt/Private/ui-tui /opt/Private/gateway /opt/Private/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the Private user via
# `s6-setuidgid Private` in its run script. If Private_UID is unset, services
# run as the default Private user (UID 10000).

# ---------- Link Private-agent itself (editable) ----------
...
# container always returns nothing — meaning `Private dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the Private_GIT_SHA build-arg to
# /opt/Private/.Private_build_sha at build time, and have
# Private_cli/build_info.py read it at runtime.  Both `Private dump` and
...
ARG Private_GIT_SHA=
RUN if [ -n "${Private_GIT_SHA}" ]; then \
        printf '%s\n' "${Private_GIT_SHA}" > /opt/Private/.Private_build_sha && \
        chown Private:Private /opt/Private/.Private_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-Private + dashboard.
...
# `exec Private`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $Private_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/Private/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-Private-setup && \
    chmod +x /etc/cont-init.d/01-Private-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV Private_WEB_DIST=/opt/Private/Private_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/Private/ui-tui/dist/entry.js`) and skip the
...
ENV Private_TUI_DIR=/opt/Private/ui-tui
ENV Private_HOME=/opt/data

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> Private ...` they default to root, and any file the
# command writes under $Private_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/Private/bin/Private, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid Private`
# when invoked as root. Non-root callers (supervised processes,
# `--user Private`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/Private/.venv/bin/Private). See the shim source for
# the opt-out env var (Private_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/Private-exec-shim.sh /opt/Private/bin/Private

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> Private ...` and any
# subprocess that doesn't activate the venv first still find Private.
#
# /opt/Private/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/Private/bin:/opt/Private/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. Private
# subcommand vs. no-args), drops to the Private user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/Private/docker/main-wrapper.sh" ]
CMD [ ]
