...
- 用户希望 **Private 本身驱动浏览器** → 使用 Private 内置的浏览器工具（Browserbase / Camofox）。page-agent 是*相反*的方向。
