...
sidebar_position: 7
title: "Docker"
description: "在 Docker 中运行 Private Agent 以及将 Docker 用作终端后端"
---

# Private Agent — Docker

Docker 与 Private Agent 的交集有两种截然不同的方式：

1. **在 Docker 中运行 Private** — agent 本身在容器内运行（本页的主要内容）
2. **Docker 作为终端后端** — agent 在宿主机上运行，但将每条命令在单个持久化 Docker 沙箱容器中执行，该容器在工具调用、`/new` 和子 agent 之间保持存活，直至 Private 进程结束（参见 [配置 → Docker 后端](./configuration.md#docker-backend)）

本页介绍选项 1。容器将所有用户数据（配置、API 密钥、会话、技能、记忆）存储在从宿主机挂载于 `/opt/data` 的单个目录中。镜像本身是无状态的，可通过拉取新版本进行升级而不会丢失任何配置。

## 快速开始

如果这是你第一次运行 Private Agent，请在宿主机上创建一个数据目录，并以交互方式启动容器以运行设置向导：
...
内置 Web dashboard 作为可选的子进程在与 gateway 相同的容器内运行。设置 `Private_DASHBOARD=1` 可在容器回环地址（`127.0.0.1`）上默认运行 dashboard：
...
| 环境变量 | 描述 | 默认值 |
|---------------------|-------------|---------|
| `Private_DASHBOARD` | 设为 `1`（或 `true` / `yes`）以在主命令旁启动 dashboard | *（未设置——不启动 dashboard）* |
| `Private_DASHBOARD_HOST` | dashboard HTTP 服务器的绑定地址 | `127.0.0.1` |
| `Private_DASHBOARD_PORT` | dashboard HTTP 服务器的端口 | `9119` |
| `Private_DASHBOARD_INSECURE` | 设为 `1`（或 `true` / `yes`）以在不启用 OAuth 鉴权门控的情况下绑定。仅在可信网络（且通过没有 OAuth 契约的反向代理时）使用——dashboard 会暴露 API 密钥与会话数据 | *（未设置——当注册了 `DashboardAuthProvider` 时启用门控）* |

默认情况下，dashboard 保持在回环地址（`127.0.0.1`），以避免将
Web 界面暴露到网络。若要有意发布，请设置
`Private_DASHBOARD_HOST=0.0.0.0`。当以下两项同时满足时，
...
在可信局域网中的场景——请设置 `Private_DASHBOARD_INSECURE=1`。
...
`/opt/data` 卷是所有 Private 状态的唯一数据来源。它映射到宿主机的 `~/.Private/` 目录，包含：

| 路径 | 内容 |
|------|----------|
| `.env` | API 密钥和机密 |
| `config.yaml` | 所有 Private 配置 |
| `SOUL.md` | Agent 个性/身份 |
| `sessions/` | 对话历史 |
| `memories/` | 持久化记忆存储 |
| `skills/` | 已安装的技能 |
...
:::warning
切勿同时对同一数据目录运行两个 Private **gateway** 容器——会话文件和记忆存储不支持并发写入。
:::

## 多 profile 支持

Private 支持[多个 profile](../reference/profile-commands.md)——独立的 `~/.Private/` 目录，让你可以从单个安装运行独立的 agent（不同的 SOUL、技能、记忆、会话、凭据）。**在 Docker 下运行时，不建议使用 Private 内置的多 profile 功能。**
...
- **独立生命周期** — 可独立升级、重启、暂停或回滚每个 agent（`docker restart Private-work` 不会影响 `Private-personal`）。
...
services:
  Private:
    ...
    ports:
      ...
      - "9119:9119"   # dashboard（仅在 Private_DASHBOARD=1 时生效）
    volumes:
      - ~/.Private:/opt/data
    environment:
      - Private_DASHBOARD=1
...
- Python 3 及所有 Private 依赖（`uv pip install -e ".[all]"`）
...
1. 以 root 身份运行 `/etc/cont-init.d/01-Private-setup`（即 `docker/stage2-hook.sh`）：可选的 UID/GID 重映射、修复卷所有权、首次启动时初始化 `.env` / `config.yaml` / `SOUL.md`、同步内置技能。
2. 运行 `/etc/cont-init.d/02-reconcile-profiles`（即 `Private_cli.container_boot`）：遍历 `$Private_HOME/profiles/<name>/`，在 `/run/service/gateway-<profile>/` 下重建各 profile 的 gateway s6 服务槽，并仅自动启动上次记录状态为 `running` 的 profile（参见 [Per-profile gateway 监管](#per-profile-gateway-supervision)）。
3. 启动静态的 `main-Private` 和 `dashboard` s6-rc 服务。
4. 将容器的 CMD 作为主程序 exec（`/opt/Private/docker/main-wrapper.sh`），根据用户传给 `docker run` 的参数进行路由：
   - 无参数 → `Private`（默认）
   - 第一个参数是 PATH 上的可执行文件（如 `sleep`、`bash`）→ 直接 exec
   - 其他情况 → `Private <args>`（子命令透传）
   主程序退出时容器退出，并使用其退出码。
...
:::warning 权限模型
除非你在命令链中保留 `/init`（或等效的旧版 `docker/entrypoint.sh` shim，它会转发到 stage2 hook），否则不要覆盖镜像入口点。s6-overlay 的 `/init` 以 root 运行，以便在首次启动时对卷执行 chown，然后通过 `s6-setuidgid` 为每个受监管的服务**以及**主程序降权至 `Private` 用户。在官方镜像内以 root 启动 `Private gateway run` 默认会被拒绝，因为这可能在 `/opt/data` 中留下 root 所有的文件，导致后续 dashboard 或 gateway 启动失败。仅在你有意接受该风险时才设置 `Private_ALLOW_ROOT_GATEWAY=1`。
:::

### Per-profile gateway 监管

在容器内，每个通过 `Private profile create <name>` 创建的 profile 都会自动在 `/run/service/gateway-<name>/` 注册一个受 s6 监管的 gateway 服务。你在宿主机上运行的生命周期命令在此同样适用：

```sh
Private profile create coder            # 注册 gateway-coder s6 槽
Private -p coder gateway start          # s6-svc -u  → 受监管的 gateway
Private -p coder gateway stop           # s6-svc -d  → 服务停止
Private -p coder gateway restart        # s6-svc -t  → 向 supervisor 发送 SIGTERM
Private profile delete coder            # 拆除 s6 槽
```

**相比 pre-s6 镜像的监管优势：**

- Gateway 崩溃后由 `s6-supervise` 在约 1 秒退避后自动重启。
- Dashboard 崩溃后自动重启（设置 `Private_DASHBOARD=1` 以启动）。
- `docker restart` 保留运行中的 gateway：cont-init 协调器读取 `$Private_HOME/profiles/<name>/gateway_state.json`，若上次记录状态为 `running` 则恢复该槽。已停止的 gateway 保持停止状态。
- 各 profile 的 gateway 日志持久化于 `$Private_HOME/logs/gateways/<profile>/current`（由 `s6-log` 轮转），协调器的操作记录在每次启动时追加到 `$Private_HOME/logs/container-boot.log`。

在容器内执行 `Private status` 会显示 `Manager: s6 (container supervisor)`。使用 `/command/s6-svstat /run/service/gateway-<name>` 查看原始 supervisor 状态（注意 `/command/` 仅在监管树进程的 PATH 中；从 `docker exec` 调用时请传入绝对路径）。
...
当使用 Docker 作为执行环境时（不是上述方法，而是 agent 在 Docker 沙箱内运行命令——参见 [配置 → Docker 后端](./configuration.md#docker-backend)），Private 为所有工具调用复用单个长期运行的容器，并自动将技能目录（`~/.Private/skills/`）和技能声明的所有凭据文件以只读卷的形式绑定挂载到该容器中。技能脚本、模板和引用在沙箱内无需手动配置即可使用，由于容器在 Private 进程的整个生命周期内持续存在，你安装的任何依赖或写入的文件都会在下次工具调用时保留。
...
对于发布到 npm 或 PyPI 的任何工具，指示 Private 通过 `npx`（npm）或 `uvx`（Python）运行，并将该命令记入其持久记忆。如果工具需要配置文件或凭据，指示其将这些文件放在 `/opt/data` 下（如 `/opt/data/<tool>/config.yaml`）。
...
对于 npm 或 PyPI 之外的工具——`apt` 包、预构建二进制文件、镜像中未包含的语言运行时——指示 Private 如何安装（如 `apt-get update && apt-get install -y <package>`），并告知它记住该安装命令。工具在容器剩余生命周期内持续可用，Private 在容器重启后下次需要该工具时会重新运行安装命令。
...
对于自带服务（数据库、Web 服务器、队列、无头浏览器集群）或过于庞大而不适合放在 Private 容器内的工具，将其作为独立容器运行在共享 Docker 网络上。Private 通过容器名称访问 sidecar，与访问本地推理服务器的方式相同（参见 [连接本地推理服务器](#connecting-to-local-inference-servers-vllm-ollama-etc)）。
...
在 Private 容器内，sidecar 可通过 `http://my-tool:<port>` 访问（或其提供的任何协议）。这种模式使每个服务的生命周期、资源限制和升级节奏保持独立，避免因单个工具的依赖而使 Private 镜像臃肿。
...
在 Docker 中运行 Private 且推理服务器（vLLM、Ollama、text-generation-inference 等）也在宿主机或另一个容器中运行时，网络配置需要额外注意。
...
然后在 `~/.Private/config.yaml` 中，使用**容器名称**作为主机名：
...
:::tip 关键点
- 使用**容器名称**（`vllm`）作为主机名——而非 `localhost` 或 `127.0.0.1`，它们指向 Private 容器本身。
...
1. 两个容器是否在同一 Docker 网络上（`docker network inspect Private-net`）
...
检查日志：`docker logs Private`。常见原因：
- `.env` 文件缺失或无效——先以交互方式运行以完成设置
- 开放端口时存在端口冲突

### "Permission denied" 错误

容器的 stage2 hook 通过 `s6-setuidgid` 在每个受监管的服务内将权限降至非 root 用户 `Private`（UID 10000）。如果宿主机的 `~/.Private/` 由不同 UID 拥有，请设置 `Private_UID`/`Private_GID` 以匹配宿主机用户，或确保数据目录可写：
