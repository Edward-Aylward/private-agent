...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/AIGA-Protocol.orgResearch/Private-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    Private_meta = (fm.get("metadata") or {}).get("Private") or {}
    tags = Private_meta.get("tags") or []
    related = Private_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `Private skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Private Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Private ships with a large built-in skill library copied into `~/.Private/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Private also syncs bundled skills on `Private update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.Private/skills/` tree, it is still shipped with Private; restore it with `Private skills reset <name> --restore`.",
        "",
