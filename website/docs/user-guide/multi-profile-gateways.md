...
If you only run one Private agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Private agents that should all
...
(`ai.Private.gateway-<name>.plist`) or systemd user service
(`Private-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
Private profile create coder
Private profile create personal-bot
Private profile create research
...
`~/.local/bin/Private-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: Private-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    Private gateway "$action"
  else
    Private -p "$profile" gateway "$action"
  fi
}
...
```bash
Private-gateways start      # start every configured profile
Private-gateways stop       # stop every configured profile
Private-gateways restart    # restart all
Private-gateways status     # status across all
Private-gateways list       # delegates to `Private gateway list`
```

:::tip
The `default` profile is targeted with `Private gateway <action>` (no `-p`),
not `Private -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `Private -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.Private.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/Private-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.Private.gateway.plist` /
...
```bash
# Default profile
tail -f ~/.Private/logs/gateway.log
tail -f ~/.Private/logs/gateway.error.log

# Named profile
tail -f ~/.Private/profiles/<name>/logs/gateway.log
tail -f ~/.Private/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.Private/logs/gateway.log ~/.Private/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
Private logs -f                  # follow default profile
Private -p coder logs -f         # follow one profile
Private logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
Private profile list             # profiles + model + gateway state
Private-gateways status          # full status across every profile
launchctl list | grep Private    # macOS — PIDs and labels
systemctl --user list-units 'Private-gateway-*'   # Linux — units
```
...
The default profile uses `~/.Private/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
Private config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.Private/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=Private --why="gateways running" \
  sleep infinity &
...
`Private-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.Private/.env ~/.Private/profiles/*/.env
```

## Updating the code

`Private update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `Private gateway start` after a previous `Private gateway stop`. The
...
```bash
ps -ef | grep "Private_cli.*-p <profile>"
cat ~/.Private/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.Private.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.Private.gateway-<profile>.plist

# Linux
systemctl --user restart Private-gateway-<profile>.service
```

### Health check

```bash
Private doctor                  # default profile
Private -p <profile> doctor     # one profile
```
