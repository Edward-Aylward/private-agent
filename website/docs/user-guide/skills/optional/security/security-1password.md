...
| | |
|---|---|
| Source | Optional — install with `Private skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Private Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Private terminal calls (desktop app flow only)
...
### Service Account (recommended for Private)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.Private/.env` (the skill will prompt for this on first load).
...
## Private Execution Pattern (desktop app flow)

Private terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/Private-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/Private-op.sock"
