...
| | |
|---|---|
| Source | Optional — install with `Private skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Private or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Private. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.Private/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.Private/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Private integration, either:

- configure the server in `~/.Private/config.yaml` using the `native-mcp` skill, or
...
### Private cannot see the deployed server

The server-building part may be correct while the Private config is not. Load the `native-mcp` skill and configure the server in `~/.Private/config.yaml`, then restart Private.
