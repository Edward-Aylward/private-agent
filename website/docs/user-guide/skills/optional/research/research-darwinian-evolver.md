...
| | |
|---|---|
| Source | Optional — install with `Private skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Private Agent |
| License | MIT |
...
import upstream classes into Private itself.
...
```bash
mkdir -p ~/.Private/cache/darwinian-evolver && cd ~/.Private/cache/darwinian-evolver
...
```bash
cd ~/.Private/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.Private/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.Private/skills/research/darwinian-evolver
DE_DIR=~/.Private/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Private core.
   Custom driver scripts under `~/.Private/skills/...` are user-side and fine.
