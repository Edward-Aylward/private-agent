...
| | |
|---|---|
| Source | Optional — install with `Private skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Private Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.Private/.env`:
...
```bash
CANVAS="python $Private_HOME/skills/productivity/canvas/scripts/canvas_api.py"
