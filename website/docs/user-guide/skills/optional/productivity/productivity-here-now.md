...
| | |
|---|---|
| Source | Optional — install with `Private skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${Private_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${Private_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${Private_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client Private
```
...
```bash
PUBLISH="${Private_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client Private
```
...
```bash
DRIVE="${Private_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `Private`)    |
