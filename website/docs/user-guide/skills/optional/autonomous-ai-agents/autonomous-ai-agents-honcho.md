...
description: "Configure and use Honcho memory with Private -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Private -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `Private skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Private

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Private profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/Private#running-honcho-locally-with-Private

### Verify

```bash
Private honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Private profile's AI peer
...
Honcho models conversations as interactions between **peers**. Private creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Private instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Private run |
| `global` | Single session across all directories |

Manual override: `Private honcho map my-project-name`
...
Each Private profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block Private.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `Private.coder` host block in `honcho.json`
...
```bash
Private honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Private when Honcho memory is active.
...
Config file: `$Private_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `Private honcho setup`. Ensure `memory.provider: honcho` is in `~/.Private/config.yaml`.

### Memory not persisting across sessions
Check `Private honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `Private profile create <name> --clone`. For existing profiles: `Private honcho sync`.
...
| Command | Description |
|---------|-------------|
| `Private honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `Private honcho status` | Show resolved config, connection test, peer info for active profile |
| `Private honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `Private honcho disable` | Disable Honcho for the active profile |
| `Private honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `Private honcho peers` | Show peer identities across all profiles |
| `Private honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `Private honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `Private honcho sessions` | List known directory-to-session-name mappings |
| `Private honcho map <name>` | Map current working directory to a Honcho session name |
| `Private honcho identity` | Seed AI peer identity or show both peer representations |
| `Private honcho sync` | Create host blocks for all Private profiles that don't have one yet |
| `Private honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Private + Honcho |
| `Private memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `Private memory status` | Show active memory provider and config |
| `Private memory off` | Disable external memory provider |
