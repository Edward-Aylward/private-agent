...
- Give it a name like "Private-agent"
- Select scopes:
...
- Give it a title like "Private-agent-&lt;machine-name>"
...
elif [ -f ~/.Private/.env ] && grep -q "^GITHUB_TOKEN=" ~/.Private/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.Private/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
