---
title: "Private Agent — Configure, extend, or contribute to Private Agent"
sidebar_label: "Private Agent"
description: "Configure, extend, or contribute to Private Agent"
---
...
Configure, extend, or contribute to Private Agent.
...
| Path | `skills/autonomous-ai-agents/Private-agent` |
| Version | `2.1.0` |
| Author | Private Agent + Teknium |
| License | MIT |
...
:::info
The following is the complete skill definition that Private loads when this skill is triggered. This is what the agent sees as instructions when the skill is active.
:::
...
- **Self-improving through skills** — Private learns from experience by saving reusable procedures as skills. When it solves a complex problem, discovers a workflow, or gets corrected, it can persist that knowledge as a skill document that loads into future sessions. Skills accumulate over time, making the agent better at your specific tasks and environment.
- **Persistent memory across sessions** — remembers who you are, your preferences, environment details, and lessons learned. Pluggable memory backends (built-in, Honcho, Mem0, and more) let you choose how memory works.
- **Multi-platform gateway** — the same agent runs on Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Email, and 10+ other platforms with full tool access, not just chat.
- **Provider-agnostic** — swap models and providers mid-workflow without changing anything else. Credential pools rotate across multiple API keys automatically.
- **Profiles** — run multiple independent Private instances with isolated configs, sessions, skills, and memory.
- **Extensible** — plugins, MCP servers, custom tools, webhook triggers, cron scheduling, and the full Python ecosystem.

People use Private for software development, research, system administration, data analysis, content creation, home automation, and anything else that benefits from an AI agent with persistent context and full system access.
...
(`Private mcp install <name>`): `skill_view(name="Private-agent", file_path="references/native-mcp.md")`.
...
patterns: `skill_view(name="Private-agent", file_path="references/webhooks.md")`.
...
~/.Private/sessions/         Gateway routing index, request dumps, *.jsonl transcripts (and optional per-session JSON snapshots when sessions.write_json_snapshots: true)
~/.Private/state.db          Canonical session store (SQLite + FTS5)
...
Profiles use `~/.Private/profiles/<name>/` with the same layout.

### Config Sections

Edit with `Private config edit` or `Private config set section.key value`.
...
20+ providers supported. Set via `Private model` or `Private setup`.
...
Enable/disable via `Private tools` (interactive) or `Private tools enable/disable NAME`.
...
Full enumeration lives in `toolsets.py` as the `TOOLSETS` dict; `_Private_CORE_TOOLS` is the default bundle most platforms inherit from.
...
Common "why is Private doing X to my output / tool calls / commands?" toggles — and the exact commands to change them. Most of these need a fresh session (`/reset` in chat, or start a new `Private` invocation) because they're read once at startup.
...
```bash
Private config set security.redact_secrets true       # keep enabled globally
```

**Restart required.** `security.redact_secrets` is snapshotted at import time — toggling it mid-session (e.g. via `export Private_REDACT_SECRETS=false` from a tool call) will NOT take effect for the running process. Tell the user to change it in config from a terminal, then start a new session. This is deliberate — it prevents an LLM from flipping the toggle on itself mid-task.
...
```bash
Private config set privacy.redact_pii true    # enable
Private config set privacy.redact_pii false   # disable (default)
```

### Command approval prompts

By default (`approvals.mode: manual`), Private prompts the user before running shell commands flagged as destructive (`rm -rf`, `git reset --hard`, etc.). The modes are:
...
```bash
Private config set approvals.mode smart       # recommended middle ground
Private config set approvals.mode off         # bypass everything (not recommended)
```
...
Some shell-hook integrations require explicit allowlisting before they fire. Managed via `~/.Private/shell-hooks-allowlist.json` — prompted interactively the first time a hook wants to run.

### Disabling the web/browser/image-gen tools

To keep the model away from network or media tools entirely, open `Private tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
## Spawning Additional Private Instances

Run additional Private processes as fully independent subprocesses — separate sessions, tools, and environments.

### When to Use This vs delegate_task

| | `delegate_task` | Spawning `Private` process |
|-|-----------------|--------------------------|
...
Private uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `Private chat -q` for fire-and-forget** — no PTY needed
...
the `cronjob` tool, the `Private cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `Private curator <verb>` — `status`, `run`, `pause`, `resume`,
...
- **Telemetry:** sidecar at `~/.Private/skills/.usage.json` holds
...
Users drive it via `Private kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `Private_KANBAN_TASK`, and
...
Private runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
without BOM. `Private config edit` writes without BOM; manual edits in
...
Private's own env scrubber dropping `SYSTEMROOT` / `WINDIR` / `COMSPEC`
...
POSIX venv layouts (`.venv/bin/activate`). The Private-installed venv at
...
every Private tool and most Windows APIs. Prefer forward slashes in code
...
### Tool not available
1. `Private tools` — check if toolset is enabled for your platform
2. Some tools need env vars (check `.env`)
3. `/reset` after enabling tools

### Model/provider issues
1. `Private doctor` — check config and dependencies
2. `Private auth` — re-authenticate OAuth providers (or `Private auth add <provider>`)
3. Check `.env` has the right API key
4. **Copilot 403**: `gh auth login` tokens do NOT work for Copilot API. You must use the Copilot-specific OAuth device code flow via `Private model` → GitHub Copilot.
...
### Skills not showing
1. `Private skills list` — verify installed
2. `Private skills config` — check platform enablement
3. Load explicitly: `/skill name` or `Private -s name`
...
- **Gateway crash loop**: Reset the failed state: `systemctl --user reset-failed Private-gateway`
...
Config: `~/.Private/config.yaml` (settings), `~/.Private/.env` (API keys).
...
**2. Add to `toolsets.py`** → `_Private_CORE_TOOLS` list.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual list needed.

All handlers must return JSON strings. Use `get_Private_home()` for paths, never hardcode `~/.Private`.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `Private_cli/commands.py`
...
- Tests auto-redirect `Private_HOME` to temp dirs — never touch real `~/.Private/`
- Run full suite before pushing any change
- Use `-o 'addopts='` to clear any baked-in pytest flags

**Windows contributors:** `scripts/run_tests.sh` currently looks for POSIX venvs (`.venv/bin/activate` / `venv/bin/activate`) and will error out on Windows where the layout is `venv/Scripts/activate` + `python.exe`. The Private-installed venv at `venv/Scripts/` also has no `pip` or `pytest` — it's stripped for end-user install size. Workaround: install pytest + pytest-xdist + pyyaml into a system Python 3.11 user site (`/c/Program Files/Python311/python -m pip install --user pytest pytest-xdist pyyaml`), then run tests directly:
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/Private_cli/test_auth_toctou_file_modes.py`)
- `signal.SIGALRM` → Unix-only (see `tests/conftest.py::_enforce_test_timeout`)
...
- Use `get_Private_home()` from `Private_constants` for all paths (profile-safe)
