...
Connect Private to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Private supports two WeCom integration modes:
...
> Run `Private gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Private decrypts the message, queues it for the agent
...
(Use `Private gateway start` only after `Private gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Private and the token the
...
in `~/.Private/.env` values around `=` will also break signature checks. After
fixing, restart `Private gateway run`.
...
Check `Private gateway run` logs for the bound host/port. If the adapter bound to
