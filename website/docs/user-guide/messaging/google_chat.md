...
sidebar_position: 12
title: "Google Chat"
description: "Set up Private Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Private Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Private
...
> Run `Private gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Private can read (e.g.,
`~/.Private/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a Private restart)
...
- Principal: `Private-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Private calls
...
- **App name**: whatever you want users to see ("Private" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/Private-chat-events`.
...
`ADDED_TO_SPACE` event that Private uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Private

Add the Google Chat section to `~/.Private/.env`:
...
Install the dependencies the Google Chat adapter needs (no Private extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Private is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Private detects the
...
3. Download the JSON. Move it onto the host that runs Private.
4. Register the client with Private (run under the profile you want it scoped to):
...
Private -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Private home (e.g.
`~/.Private/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.Private/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.Private/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Private isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `Private gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Private will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.Private/google_chat_user_tokens/<sanitized_email>.json` (filesystem
