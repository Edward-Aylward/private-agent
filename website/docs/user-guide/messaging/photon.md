...
Connect Private to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Private — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Private runs it in a
...
Private photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Private Agent` project on your account.
...
Runtime credentials are written to `~/.Private/.env`
...
(device token, dashboard project id) lives in `~/.Private/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Private
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Private replies with a pairing code. Approve it with:

```bash
Private pairing approve photon <CODE>
```

Use `Private pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.Private/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.Private/.env`):
...
By default Private responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Private` and
`@Private agent` variants. For a custom agent name, set regex patterns:
...
```bash
Private gateway start --platform photon
```
...
Send an iMessage to your assigned number and Private will reply.

## Status & troubleshooting

```bash
Private photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Private uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run Private photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `Private photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `Private photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `Private photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Private sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Private wake words  | JSON list / comma / newline regex patterns for group mentions |
