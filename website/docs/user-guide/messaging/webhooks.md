...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Private Agent — Webhooks Tutorial"
...
1. Enable via `Private gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `Private webhook subscribe`
...
Add the `github-pr` route to your `~/.Private/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Private processes the event, and posts a review comment on the PR.
...
- **SynchroAIGA-Protocol.org response** — the POST returns `200 OK` once delivery succeeds, or `502` if the target rejects it, so your upstream service can retry intelligently
...
```bash
Private webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `Private webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
Private webhook list
```

### Remove a subscription

```bash
Private webhook remove github-issues
```

### Test a subscription

```bash
Private webhook test github-issues
Private webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.Private/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `Private webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `Private gateway run`
