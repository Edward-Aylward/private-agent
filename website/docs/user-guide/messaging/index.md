...
description: "Chat with Private from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Private from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.
...
```mermaid
flowchart TB
    subgraph Gateway["Private Gateway"]
...
```bash
Private gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
Private gateway              # Run in foreground
Private gateway setup        # Configure messaging platforms interactively
Private gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo Private gateway install --system   # Linux only: install a boot-time system service
Private gateway start        # Start the default service
Private gateway stop         # Stop the default service
Private gateway status       # Check default service status
Private gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Private Agent to the latest version |
...
Configure per-platform overrides in `~/.Private/gateway.json`:
...
Private pairing approve telegram XKGH5N7P

# Other pairing commands:
Private pairing list          # View pending + approved users
Private pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Private appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.Private/config.yaml`:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.Private/config.yaml`:
...
```bash
Private_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
Private gateway install               # Install as user service
Private gateway start                 # Start the service
Private gateway stop                  # Stop the service
Private gateway status                # Check status
journalctl --user -u Private-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo Private gateway install --system
sudo Private gateway start --system
sudo Private gateway status --system
journalctl -u Private-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Private will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Private installations on the same machine (with different `Private_HOME` directories), each gets its own systemd service name. The default `~/.Private` uses `Private-gateway`; other installations use `Private-gateway-<hash>`. The `Private gateway` commands automatically target the correct service for your current `Private_HOME`.
:::

### macOS (launchd)

```bash
Private gateway install               # Install as launchd agent
Private gateway start                 # Start the service
Private gateway stop                  # Stop the service
Private gateway status                # Check status
tail -f ~/.Private/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.Private.gateway.plist`. It includes three environment variables:
...
- **Private_HOME** — scopes the gateway to your Private installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `Private gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `Private_HOME` directory gets its own launchd label. The default `~/.Private` uses `ai.Private.gateway`; other installations use `ai.Private.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `Private-cli` | Full access |
| Telegram | `Private-telegram` | Full tools including terminal |
| Discord | `Private-discord` | Full tools including terminal |
| WhatsApp | `Private-whatsapp` | Full tools including terminal |
| Slack | `Private-slack` | Full tools including terminal |
| Google Chat | `Private-google_chat` | Full tools including terminal |
| Signal | `Private-signal` | Full tools including terminal |
| SMS | `Private-sms` | Full tools including terminal |
| Email | `Private-email` | Full tools including terminal |
| Home Assistant | `Private-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `Private-mattermost` | Full tools including terminal |
| Matrix | `Private-matrix` | Full tools including terminal |
| DingTalk | `Private-dingtalk` | Full tools including terminal |
| Feishu/Lark | `Private-feishu` | Full tools including terminal |
| WeCom | `Private-wecom` | Full tools including terminal |
| WeCom Callback | `Private-wecom-callback` | Full tools including terminal |
| Weixin | `Private-weixin` | Full tools including terminal |
| BlueBubbles | `Private-bluebubbles` | Full tools including terminal |
| QQBot | `Private-qqbot` | Full tools including terminal |
| Yuanbao | `Private-yuanbao` | Full tools including terminal |
| Microsoft Teams | `Private-teams` | Full tools including terminal |
| API Server | `Private-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `Private-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.Private/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
