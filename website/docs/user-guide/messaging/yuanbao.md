...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Private Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Private to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.Private/.env`:
...
All standard Private commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.Private/logs/gateway.log`
...
4. Consider enabling verbose logging: `Private_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Private automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
Private_LOG_LEVEL=debug Private gateway
```
...
```bash
Private chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
