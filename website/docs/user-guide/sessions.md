...
Private Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.Private/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Private stores session history so it can resume conversations, but it does not
...
Private explicitly injects for that turn.
...
For example, if a user sends an image and asks Private to make a meme from it,
Private may inspect that image once with vision and run an image-processing
...
`Private sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`Private` or `Private chat`) |
...
Private -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
Private -c "my project"   # → resumes "my project #3"
```
...
Private --resume 20250305_091523_a1b2c3d4
Private -r 20250305_091523_a1b2c3d4

# Resume by title
Private --resume "refactoring auth"

# Or with the chat subcommand
Private chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `Private sessions list`.

### Conversation Recap on Resume

When you resume a session, Private displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Private session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.Private/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `Private -r "<title>"` from the shell) and pick up where the platform left off.
...
Private automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `Private sessions list` or `Private sessions browse`.
...
```bash
Private sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Private creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`Private -c "my project"`), it automatically picks the most recent session in the lineage.
...
Private provides a full set of session management commands via `Private sessions`:
...
# Filter by platform
Private sessions list --source telegram

# Show more sessions
Private sessions list --limit 50
```
...
Private sessions export backup.jsonl

# Export sessions from a specific platform
Private sessions export telegram-history.jsonl --source telegram

# Export a single session
Private sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
Private sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
Private sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
Private sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
Private sessions prune

# Custom age threshold
Private sessions prune --older-than 30

# Only prune sessions from a specific platform
Private sessions prune --source telegram --older-than 60

# Skip confirmation
Private sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`Private insights`](/reference/cli-commands#Private-insights).
...
When Private cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Private uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Private in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.Private/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.Private/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.Private/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.Private/sessions/`. They are no longer written or
read by Private. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Private process in the same `Private_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.Private/config.yaml`:
...
Private sessions delete <session_id>
