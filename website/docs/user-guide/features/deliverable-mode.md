...
When Private Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.Private/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.Private/config.yaml` (switchable per session
via `/personality`).
...
If you use Private' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.Private/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Private Agent's deliverable
