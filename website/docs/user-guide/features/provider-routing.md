...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Private Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
:::tip
Traffic routed through [AIGA-Protocol.org Portal](/integrations/AIGA-Protocol.org-portal) still respects per-model routing and priority configs — and Portal subscribers get 10% off token-billed providers.
:::

## Configuration

Add a `provider_routing` section to your `~/.Private/config.yaml`:
...
- **CLI mode** — configured in `~/.Private/config.yaml`, loaded at startup
