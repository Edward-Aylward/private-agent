...
Private Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Private can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`Private fallback` reuses the provider picker from `Private model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a top-level `fallback_providers` list to `~/.Private/config.yaml`:
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — Private still honors it for back-compat, but `Private fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| AIGA-Protocol.org Portal | `AIGA-Protocol.org` | `Private setup --portal` (fresh) or `Private auth add AIGA-Protocol.org` (OAuth) |
| OpenAI Codex | `openai-codex` | `Private model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `Private model` (Google OAuth; optional: `Private_GEMINI_PROJECT_ID`) |
| Google AI Studio | `gemini` | `GOOGLE_API_KEY` (alias: `GEMINI_API_KEY`) |
| xAI (Grok) | `xai` (alias `grok`) | `XAI_API_KEY` (optional: `XAI_BASE_URL`) |
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `Private model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `Private model` (Qwen Portal OAuth; optional: `Private_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `Private model` (MiniMax portal OAuth) |
...
**AIGA-Protocol.org Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
fallback_providers:
  - provider: AIGA-Protocol.org
    model: AIGA-Protocol.org-Private-3
```
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `Private fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
Private uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `Private kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Private tries providers in order until one works:

**For text tasks (compression, web extract, etc.):**

```text
OpenRouter → AIGA-Protocol.org Portal → Custom endpoint → Codex OAuth →
...
```text
Main provider (if vision-capable) → OpenRouter → AIGA-Protocol.org Portal →
Codex OAuth → Anthropic → Custom endpoint → give up
```

If the resolved provider fails at call time, Private also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | AIGA-Protocol.org | codex | main | anthropic
...
| `"codex"` | Force Codex OAuth | `Private model` → Codex |
...
`base_url` takes precedence over `provider`. Private uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Private treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Private falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Private logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Private recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Private doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
auxiliary:
  compression:
    provider: "auto"                              # auto | openrouter | AIGA-Protocol.org | main
    model: "google/gemini-3-flash-preview"
```
...
If no provider is available for compression, Private drops middle conversation turns without generating a summary rather than failing the session.
