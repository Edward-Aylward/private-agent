...
```bash
# Custom port
Private dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
Private dashboard --host 0.0.0.0

# Start without opening browser
Private dashboard --no-open
```
...
child with the selected profile's `Private_HOME`, so the conversation runs
...
processes (manage them via `Private -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `Private-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'Private-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install Private-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `Private dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `Private dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `Private --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'Private-agent[web,pty]'`, or `[all]` covers both)
...
ExecStart=/path/to/venv/bin/python -m Private_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.Private/.env` containing:

```bash
Private_DASHBOARD_BASIC_AUTH_USERNAME=admin
Private_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
Private_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `Private config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Private instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `Private profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.Private/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `Private skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `Private mcp` reads from.
...
Connect Private to any messaging platform from the browser — full parity with
`Private setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.Private/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
This re-reads `~/.Private/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
`Private_HOME`. Omitted = the dashboard's own profile. Unknown profile names
...
|-------|-----------|----------|
| `Private dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `Private dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
|---------|-----------|--------|----------------|
| `Private_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `Private dashboard register` |

Per the Private Agent convention (`~/.Private/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
|---------|-----------|-------|
| `Private_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `Private_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `Private_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `Private_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `Private_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
**1. Set credentials in `~/.Private/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
cat >> ~/.Private/.env <<EOF
Private_DASHBOARD_BASIC_AUTH_USERNAME=admin
Private_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
Private_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/Private/   # required
      client_id: Private-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `Private_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `Private_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `Private_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-Private.json` — it defines a `Private` realm, a **public PKCE client** (`Private-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "Private-dashboard",
      "name": "Private Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-Private.json:/opt/keycloak/data/import/realm-Private.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/Private/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/Private`). The admin console is at
...
```bash
export Private_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/Private"
export Private_DASHBOARD_OIDC_CLIENT_ID="Private-dashboard"
export Private_DASHBOARD_PUBLIC_URL="http://localhost:9119"
Private dashboard --host 0.0.0.0 --port 9119 --no-open
```

`Private_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → Private-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `Private_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/Private"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `Private_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `Private_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `Private_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `Private_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `Private_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$Private_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
from Private_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
```bash
# Quick env-var path.
Private_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  Private dashboard --host 0.0.0.0
...
```bash
# 1. Set the dashboard login credentials in ~/.Private/.env (secrets file, 0600).
cat >> ~/.Private/.env <<'EOF'
Private_DASHBOARD_BASIC_AUTH_USERNAME=admin
Private_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
Private dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `Private_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/Private` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `Private_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `Private_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `Private_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `Private_DASHBOARD_BASIC_AUTH_USERNAME` / `Private_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `Private_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `Private_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `Private update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `Private dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Private Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Private Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
