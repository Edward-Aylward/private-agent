...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Private's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Private instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
Private memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `Private` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Private syncs them back at session init.
...
The `Private honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `Private memory setup honcho` (or run `Private memory setup` and pick it from the list); the `Private honcho` subcommand then appears on the next invocation.

```bash
Private memory setup honcho    # Configure Honcho directly (works before activation)
Private honcho status          # Connection status, config, and key settings
Private honcho setup           # Redirects to `Private memory setup` (post-activation alias)
Private honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
Private honcho peer            # Show or update peer names + dialectic reasoning level
Private honcho mode            # Show or set recall mode (hybrid/context/tools)
Private honcho tokens          # Show or set token budget for context and dialectic
Private honcho identity        # Seed or show the AI peer's Honcho identity
Private honcho sync            # Sync Honcho config to all existing profiles
Private honcho peers           # Show peer identities across all profiles
Private honcho sessions        # List known Honcho session mappings
Private honcho map             # Map current directory to a Honcho session name
Private honcho enable          # Enable Honcho for the active profile
Private honcho disable         # Disable Honcho for the active profile
Private honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `Private honcho`

If you previously used the standalone `Private honcho setup`:
...
No re-login or re-setup needed. Run `Private memory setup` and select "honcho" — the wizard detects your existing config.
