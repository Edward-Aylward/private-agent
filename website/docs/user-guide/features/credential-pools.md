...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Private automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [AIGA-Protocol.org Portal](/integrations/AIGA-Protocol.org-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Private auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
Private auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
Private auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
Private auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  Private_pkce          oauth   Private_pkce ←
...
Run `Private auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, AIGA-Protocol.org, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `Private auth` | Interactive pool management wizard |
| `Private auth list` | Show all pools and credentials |
| `Private auth list <provider>` | Show a specific provider's pool |
| `Private auth add <provider>` | Add a credential (prompts for type and key) |
| `Private auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `Private auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `Private auth remove <provider> <index>` | Remove credential by 1-based index |
| `Private auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `Private auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `Private model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via Private model:
Private auth list
# Shows:
...
Private auth add Together.ai --api-key sk-together-second-key
```
...
Private automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, AIGA-Protocol.org device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Private PKCE OAuth | `~/.Private/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `Private auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `Private auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Private can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Private-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`Private_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`Private_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.Private/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Private' credential store, so its token remains persistable.
