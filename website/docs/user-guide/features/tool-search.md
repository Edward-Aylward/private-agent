...
**Tool Search** is Private' opt-in progressive-disclosure layer for that
...
:::info Built-in Private tools never defer
The tools that make up Private' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, `send_message`, and the rest of
`_Private_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Private **unwraps the bridge** and
...
- **No JS sandbox.** Private uses the simpler "structured tools" mode
