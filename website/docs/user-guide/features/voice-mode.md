...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Private Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Private Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Private](/guides/use-voice-mode-with-Private).

## Prerequisites

Before using voice features, make sure you have:

1. **Private Agent installed** — `pip install Private-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `Private model` or set your preferred provider credentials in `~/.Private/.env`
3. **A working base setup** — run `Private` to verify the agent responds to text before enabling voice
...
pip install "Private-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "Private-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "Private-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "Private-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`Private chat`) and the **TUI** (`Private --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.Private/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
Private                # Start the interactive CLI
```
...
1. Start the CLI with `Private` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.Private/config.yaml` (default: `ctrl+b`).
:::
...
```bash
Private gateway        # Start the gateway (connects to configured platforms)
Private gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@Privatebyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.Private/.env`:
```bash
...
```bash
Private gateway        # Start with existing configuration
```
...
If you are running Private inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.Private/.env`
...
- Review gateway logs: `tail -f ~/.Private/logs/gateway.log`
