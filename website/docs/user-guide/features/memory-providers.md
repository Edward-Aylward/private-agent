...
Private Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
Private memory setup      # interactive picker + configuration
Private memory status     # check what's active
Private memory off        # disable external provider
```

You can also select the active memory provider via `Private plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.Private/config.yaml`:
...
When a memory provider is active, Private automatically:
...
**Setup Wizard:**
```bash
Private memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `Private honcho`
If you previously used `Private honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Private profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Private profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Private profile. Host key `Private` → default; `Private.<profile>` for others. |
...
```bash
Private profile create coder --clone
```

`--clone` creates a `Private.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Private profile, creates host blocks for any profile without one, inherits settings from the default `Private` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Private memory setup    # select "openviking"
# Or manually:
Private config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.Private/.env
```
...
**Setup:**
```bash
Private memory setup    # select "mem0"
# Or manually:
Private config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.Private/.env
```

**Config:** `$Private_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `Private-user` | User identifier |
...
**Setup:**
```bash
Private memory setup    # select "hindsight"
# Or manually:
Private config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.Private/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p Private ui start`

**Config:** `$Private_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `Private` | Memory bank identifier |
...
**Setup:**
```bash
Private memory setup    # select "holographic"
# Or manually:
Private config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.Private-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$Private_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
Private memory setup    # select "retaindb"
# Or manually:
Private config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.Private/.env
```
...
# Then configure Private
Private memory setup    # select "byterover"
# Or manually:
Private config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$Private_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
Private memory setup    # select "supermemory"
# Or manually:
Private config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.Private/.env
```

**Config:** `$Private_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `Private` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `Private-{identity}` → `Private-coder`) to isolate memories per Private profile
...
| **Requires** | `pip install Private-memori` + `Private-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install Private-memori
Private-memori install
Private config set memory.provider memori
...
| **Memori** | Cloud | Free/Paid | 5 | `Private-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$Private_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$Private_HOME/` so each profile has its own credentials
