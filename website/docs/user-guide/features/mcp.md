...
description: "Connect Private Agent to external tool servers via MCP — and control exactly which MCP tools Private loads"
---

# MCP (Model Context Protocol)

MCP lets Private Agent connect to external tool servers so the agent can use tools that live outside Private itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Private to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Private tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Private to see
...
```bash
cd ~/.Private/Private-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.Private/config.yaml`:
...
4. Ask Private to use the MCP-backed capability.
...
```bash
Private mcp                # interactive picker (default)
Private mcp catalog        # plain-text list, scriptable
Private mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Private
...
- **API key** — Private prompts at install time and writes the value to
  `~/.Private/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Private points you at
  `Private auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Private probes the MCP server to list every
...
written (if not). Re-run `Private mcp configure <name>` once the server is
reachable to refine.
...
PR adds an entry with a newer `manifest_version` than your installed Private
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Private`) for that entry instead of silently hiding it. Run `Private update`
to install the latest Private when you see that.
...
from environment variables (which include everything in `~/.Private/.env`).
...
MCPs are never auto-updated. Re-run `Private mcp install <name>` to refresh
...
HTTP MCP servers are remote endpoints Private connects to directly.
...
- you do not want Private spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Private handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Private prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.Private/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Private runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Private prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.

See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `Private mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `Private mcp login` can list the tools and look like it worked, but every real tool call later times out. `Private mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `Private mcp login googledrive` — with the pre-registered client, Private skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.Private/config.yaml` from inside a running Private session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `Private mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Private passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
Private reads MCP config from `~/.Private/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Private skips the server entirely |
...
For well-known MCP servers, `Private mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
Private mcp add codex --preset codex
```
...
You can pick any local name (`Private mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Private registers MCP tools

Private prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Private sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Private also registers utility tools around MCP resources and prompts:
...
- Private only registers resource utilities if the MCP session actually supports resource operations
- Private only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Private, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Private skips the server completely and does not even attempt a connection.
...
You can also separately disable Private-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Private does not create an empty runtime MCP toolset for that server.
...
Private discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Private when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Private receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Private does not blindly pass your full shell environment.
...
Then verify your config and restart Private.
...
Because Private now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Private may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Private via the `sampling/createMessage` protocol. This allows an MCP server to ask Private to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Private as an MCP server

In addition to connecting **to** MCP servers, Private can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Private's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Private
- You want a single MCP server that bridges to all of Private's connected messaging platforms at once
- You already have a running Private gateway with connected platforms
...
Add Private to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Private in a specific location:

```json
{
  "mcpServers": {
    "Private": {
      "command": "/home/user/.Private/Private-agent/venv/bin/Private",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Private-specific channel browser:
...
The MCP server includes a live event bridge that polls Private's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
Private mcp serve              # Normal mode
Private mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Private's session store (`~/.Private/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Private agent itself.
...
- The embedded `Private mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Private, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
