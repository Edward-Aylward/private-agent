...
Private runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Private
...
1. Private captures a baseline of current diagnostics for the file.
...
…). Private auto-detects the binary on PATH or in
`<Private_HOME>/lsp/bin/`.
...
`node_modules` tree — Private installs both packages together when you
run `Private lsp install typescript` or auto-install fires on first
use.

## CLI

```
Private lsp status          # service state + per-server install status
Private lsp list            # registry, optionally --installed-only
Private lsp install <id>    # eagerly install one server
Private lsp install-all     # try every server with a known recipe
Private lsp restart         # tear down running clients
Private lsp which <id>      # print resolved binary path
```

`Private lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <Private_HOME>/lsp/bin
...
When `install_strategy: auto`, Private installs binaries into
`<Private_HOME>/lsp/bin/`. NPM packages land in
`<Private_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Private-owned and is
...
Servers are kept alive for the life of the Private process. There's
...
**`Private lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<Private_HOME>/lsp/bin/`. Run
`Private lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `Private lsp status`**
...
When `Private lsp status` shows a `Backend warnings` section, install
...
`~/.Private/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.Private/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `Private lsp restart` to clear the set;
