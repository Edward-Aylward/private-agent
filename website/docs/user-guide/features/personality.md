...
description: "Customize Private Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Private Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `Private_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Private is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Private now seeds a default `SOUL.md` automatically in:

```text
~/.Private/SOUL.md
```

More precisely, it uses the current instance's `Private_HOME`, so if you run Private with a custom home directory, it will use:

```text
$Private_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Private creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Private loads `SOUL.md` only from `Private_HOME`
- Private does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Private falls back to a built-in default identity
...
If Private loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `Private_HOME`, the personality belongs to the Private instance itself.

That also makes it easier to teach users:
- "Edit `~/.Private/SOUL.md` to change Private' default personality."
...
- how Private should handle uncertainty, disagreement, or ambiguity
...
## What Private injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Private falls back to a built-in default identity ("You are Private Agent, an intelligent AI assistant created by AIGA-Protocol.org Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Private ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Private, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Private its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.Private/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.Private/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-Private)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Private speaks
- `display.skin` and `/skin` affect how Private looks in the terminal
