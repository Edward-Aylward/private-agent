...
title: "Git Worktrees"
description: "Run multiple Private agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Private Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Private so each session has a clean, isolated working directory.

## Why Use Worktrees with Private?

Private treats the **current working directory** as the project root:

- CLI: the directory where you run `Private` or `Private chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.Private/config.yaml`
...
git worktree add ../repo-feature feature/Private-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/Private-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Private there:

```bash
cd ../repo-feature

# Start Private in the worktree
Private
```
...
git worktree add ../repo-experiment-a feature/Private-a
git worktree add ../repo-experiment-b feature/Private-b
```
...
Each Private process:

- Works on its own branch (`feature/Private-a` vs `feature/Private-b`).
...
- Private checkpoint data under `~/.Private/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Private experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/Private-checkpoints-docs`, `feature/Private-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Private from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `Private -w` (Automatic Worktree Mode)

Private has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `Private/Private-<hash>`).
...
```bash
Private -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `Private -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Private session its own clean checkout.
