...
description: "Run Private Agent directly on an Android phone with Termux"
---

# Private on Android with Termux

This is the tested path for running Private Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Private from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `Private` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `Private` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/Private" "$PREFIX/bin/Private"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `Private` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.Private/.env`.
...
### `Private doctor` says ripgrep or Node is missing
