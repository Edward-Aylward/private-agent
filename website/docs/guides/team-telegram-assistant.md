...
This tutorial walks you through setting up a Telegram bot powered by Private Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Private Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.Private/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Private itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Private Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_Private_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Private Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.Private/.env`:
...
```
[Gateway] Starting Private Gateway...
...
# View live logs
journalctl --user -u Private-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `Private gateway install` to update the plist.
:::
...
```bash
Private gateway stop && Private gateway start
```
...
Private pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.Private/.env`:
...
Control how much detail the bot shows when using tools. In `~/.Private/config.yaml`:
...
Customize how the bot communicates by editing `~/.Private/SOUL.md`:

For a full guide, see [Use SOUL.md with Private](/guides/use-soul-with-Private).
...
```bash
# From the CLI
Private cron list          # View all scheduled jobs
Private cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u Private-gateway` (Linux) or `~/.Private/logs/gateway.log` (macOS) |
| Cron job output | `~/.Private/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.Private/cron/jobs.json` |
| Pairing data | `~/.Private/pairing/` |
| Session history | `~/.Private/sessions/` |
