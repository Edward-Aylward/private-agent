---
sidebar_position: 6
title: "Use MCP with Private"
description: "A practical guide to connecting MCP servers to Private Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Private

This guide shows how to actually use MCP with Private Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Private tool
- you want Private to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Private to internal APIs, databases, or company systems without modifying Private core

Do not use MCP when:
- a built-in Private tool already solves the job well
...
- Private remains the agent
- MCP servers contribute tools
- Private discovers those tools at startup or reload time
...
If you installed Private with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Private banner/status should show MCP integration when configured
- ask Private what tools it has available
...
## WSL2: bridge Private in WSL to Windows Chrome

This is the practical setup when:

- Private runs inside WSL2
...
In this setup, Private does **not** connect to Chrome directly. Instead:

- Private runs in WSL
- Private starts a local stdio MCP server
...
```text
Private (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Private stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Private core browser transport
...
```bash
Private mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
Private mcp test chrome-devtools-win
```

Then start a fresh Private session or run:
...
Once loaded, Private can use the MCP-prefixed browser tools directly. For example:
...
If Private runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Private from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Private from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Private:
...
2. Private-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Private will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Private to reason over a bounded workspace.
...
Now Private can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Private core.
...
Because Private now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
