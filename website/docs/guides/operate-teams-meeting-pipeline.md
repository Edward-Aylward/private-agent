...
```bash
Private teams-pipeline validate
```
...
```bash
Private teams-pipeline token-health
Private teams-pipeline token-health --force-refresh
```
...
```bash
Private teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
Private teams-pipeline maintain-subscriptions
Private teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Private cron (recommended if you already run the Private gateway)

Private ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.Private/scripts/`. First create the script:

```bash
mkdir -p ~/.Private/scripts
cat > ~/.Private/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec Private teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.Private/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
Private cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/Private-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Private Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=Private
EnvironmentFile=/etc/Private/env
ExecStart=/usr/local/bin/Private teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/Private-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Private Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now Private-teams-pipeline-maintain.timer
systemctl list-timers Private-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/Private teams-pipeline maintain-subscriptions >> /var/log/Private/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.Private/.env` at the top of a wrapper script that crontab calls.
...
```bash
Private teams-pipeline subscriptions   # should show expirationDateTime advanced
Private teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
Private teams-pipeline list
Private teams-pipeline list --status failed
Private teams-pipeline show <job-id>
```

### Replay a stored job

```bash
Private teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
Private teams-pipeline fetch --meeting-id <meeting-id>
Private teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `Private teams-pipeline maintain-subscriptions --dry-run`
- inspect `Private teams-pipeline list --status failed`
...
- run `Private teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `Private teams-pipeline run`
...
- [ ] `Private teams-pipeline validate` returns an OK snapshot
- [ ] `Private teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Private cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
