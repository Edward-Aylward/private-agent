...
Private cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/Private-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/Private-agent --state open --json number,title,labels,author,createdAt --limit 30
...
Private cron create "0 9 * * 1" \
  "Scan the NousResearch/Private-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/Private-agent --state merged --json number,title,files,mergedAt --limit 30
