...
description: "Diagnose and fix common Private cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`Private gateway` for foreground, or `Private gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `Private cron tick`.
...
```bash
date
Private cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.Private/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.Private/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.Private/.env` |
...
| `local` | Write access to `~/.Private/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `Private cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `Private skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `Private skills list`.
...
2. `~/.Private/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `Private cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Private config directory). Verify:
```bash
ls ~/.Private/scripts/your-script.py   # Must exist
Private cron edit <job_id> --script ~/.Private/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `Private skills install <skill-name>`.
...
Jobs are stored in `~/.Private/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.Private/cron/jobs.json
chmod 600 ~/.Private/cron/jobs.json   # Your user should own it
```
...
```bash
Private cron list                    # Show all jobs, states, next_run times
Private cron run <job_id>            # Schedule for next tick (for testing)
Private cron edit <job_id>           # Fix configuration issues
Private logs                         # View recent Private logs
Private skills list                  # Verify installed skills
```
...
1. Run the job with `Private cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.Private/logs/agent.log` for scheduler messages and `~/.Private/logs/errors.log` for warnings
3. Open an issue at [github.com/AIGA-Protocol.orgResearch/Private-agent](https://github.com/AIGA-Protocol.orgResearch/Private-agent) with:
