...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `Private send`."
---

# Pipe Script Output to Messaging Platforms

`Private send` is a small, scriptable CLI that pushes a message to any
messaging platform Private is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | Private send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `Private
...
Private send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
Private send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
Private send --list

# Filter by platform
Private send --list telegram
```
...
Any platform Private ships adapters for works as a target:
...
`Private send` resolves the message body in this order:

1. **Positional argument** — `Private send --to telegram "hi"`
2. **`--file PATH`** — `Private send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | Private send --to telegram`

When stdin is a TTY (no pipe), Private does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  Private send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `Private send` reuses your Private config, the same script works on
any host where Private is installed — no need to export bot tokens into
...
`Private send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  Private send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | Private send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.Private/bin/Private send \
...
```bash
./train.py --epochs 200 && \
  Private send --to telegram "training done" || \
  Private send --to telegram "training failed (exit $?)"
```
...
Private send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(Private send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `Private send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `Private send` calls
the platform's REST endpoint directly using credentials from
`~/.Private/.env` and `~/.Private/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `Private gateway start` and retry.
...
# Machine-readable
Private send --list --json
```

The listing is built from `~/.Private/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`Private gateway
...
| Approach | Multi-platform | Reuses Private creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `Private send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`Private send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `Private send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `Private send` shares with cron delivery.
