---
sidebar_position: 8
title: "Use Voice Mode with Private"
description: "A practical guide to setting up and using Private voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Private
...
:::tip
[AIGA-Protocol.org Portal](/integrations/AIGA-Protocol.org-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Private sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Private.
...
## Step 1: make sure normal Private works first
...
Private supports both local and cloud speech stacks.
...
### If you use `Private setup`

If you choose NeuTTS in the setup wizard, Private checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Private transcribes and responds
...
- asking Private to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Private loop.

## Tuning CLI behavior

### Silence threshold

If Private starts/stops too aggressively, tune:
...
Private stays a normal chat bot, but can speak replies.
...
- you want Private to function like a portable research or ops assistant
...
Private joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Private detects speech boundaries
- transcripts are posted in the associated text channel
- Private responds in text and audio
...
1. get text Private working
2. install `Private-agent[voice]`
