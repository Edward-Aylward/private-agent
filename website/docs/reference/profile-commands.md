...
This page covers all commands related to [Private profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `Private profile` without a subcommand shows help.
...
```bash
Private profile use <name>
```

Sets `<name>` as the active profile. All subsequent `Private` commands (without `-p`) will use this profile.
...
Private profile use default
```

## `Private profile create`

```bash
Private profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `Private profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `Private update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.Private`), use `Private skills opt-out` / `Private skills opt-in`. |
...
Private profile create mybot

# Clone config only from current profile
Private profile create work --clone

# Clone everything from current profile
Private profile create backup --clone-all

# Clone config from a specific profile
Private profile create work2 --clone --clone-from work
```

## `Private profile describe`

```bash
Private profile describe [<name>] [options]
```
...
Private profile describe researcher

# Set it explicitly
Private profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
Private profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
Private profile describe --all --auto
```

## `Private profile delete`

```bash
Private profile delete <name> [options]
```
...
```bash
Private profile delete mybot
Private profile delete mybot --yes
```
...
```bash
Private profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Private home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ Private profile show work
Profile: work
Path:    ~/.Private/profiles/work
...
```bash
Private profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Private installation.
...
```bash
Private profile alias work
# Creates/updates ~/.local/bin/work

Private profile alias work --name mywork
# Creates ~/.local/bin/mywork

Private profile alias work --remove
# Removes the wrapper script
```

## `Private profile rename`

```bash
Private profile rename <old-name> <new-name>
```
...
```bash
Private profile rename mybot assistant
# ~/.Private/profiles/mybot → ~/.Private/profiles/assistant
...
```bash
Private profile export <name> [options]
```
...
```bash
Private profile export work
# Creates work.tar.gz in the current directory

Private profile export work -o ./work-2026-03-29.tar.gz
```

## `Private profile import`

```bash
Private profile import <archive> [options]
```
...
```bash
Private profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

Private profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`Private profile export` / `import` are still the right commands for
...
### `Private profile install`

```bash
Private profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `Private -p telemetry`). |
...
Private profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
Private profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
Private profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
Private profile install ./telemetry/
```

### `Private profile update`

```bash
Private profile update <name> [--force-config] [--yes]
```
...
```bash
Private profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Private version, author, env var requirements, the source URL/path, and
...
`Private profile list` also shows the distribution name and version in a
`Distribution` column, and `Private profile show <name>` / `delete <name>`
...
Private profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
Private profile install https://github.com/your-org/internal-assistant.git
```
...
`Private_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Private version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Private can clone from.
3. Tell recipients to run `Private profile install <your-repo-url>`.
...
## `Private -p` / `Private --profile`

```bash
Private -p <name> <command> [options]
Private --profile <name> <command> [options]
```

Global flag to run any Private command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
Private -p work chat -q "Check the server status"
Private --profile dev gateway start
Private -p personal skills list
Private -p work config edit
```

## `Private completion`

```bash
Private completion <shell>
```
...
- `Private profile <TAB>` — subcommands (list, use, create, etc.)
- `Private profile use <TAB>` — profile names
- `Private -p <TAB>` — profile names
