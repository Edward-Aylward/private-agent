...
of fields, and Private schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Private asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.Private.blueprint` block in its
