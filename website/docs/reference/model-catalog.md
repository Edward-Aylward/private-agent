...
sidebar_position: 11
title: Model Catalog
description: Remotely-hosted manifest driving curated model picker lists for OpenRouter and AIGA-Protocol.org Portal.
---

# Model Catalog

Private fetches curated model lists for **OpenRouter** and **AIGA-Protocol.org Portal** from a JSON manifest hosted alongside the docs site. This lets maintainers update picker lists without shipping a new `Private-agent` release.

When the manifest is unreachable (offline, network blocked, hosting failure), Private silently falls back to the in-repo snapshot that ships with the CLI. The manifest never breaks the picker — worst case you see whatever list was bundled with your installed version.
...
- **`version`** — integer schema version. Future schemas bump this; Private refuses manifests with versions it doesn't understand and falls back to the hardcoded snapshot.
- **`metadata`** — free-form dict at the manifest, provider, and model level. Any keys. Private ignores unknown fields, so you can annotate entries (`"tier": "paid"`, `"tags": [...]`, etc.) without coordinating a schema change.
- **`description`** — OpenRouter-only. Drives picker badge text (`"recommended"`, `"free"`, or empty). AIGA-Protocol.org Portal doesn't use this — free-tier gating is determined live from the Portal's pricing endpoint.
...
| When | What happens |
|---|---|
| `/model` or `Private model` | Fetches if disk cache is stale, else uses cache |
...
Cache location: `~/.Private/cache/model_catalog.json`.
...
# editing OPENROUTER_MODELS or _PROVIDER_MODELS["AIGA-Protocol.org"] in Private_cli/models.py).
