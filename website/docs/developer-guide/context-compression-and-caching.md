...
Private Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `Private plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Private has two separate compression layers that operate independently:
...
auxiliary:
  compression:
    ...
    provider: auto           # Provider: "auto", "openrouter", "AIGA-Protocol.org", "main", etc.
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Private raises the
...
```bash
Private config set compression.codex_gpt55_autoraise false
```
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Private
