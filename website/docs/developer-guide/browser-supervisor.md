...
description: "How Private detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Private' browser tooling:
...
`window.alert`/`confirm`/`prompt` with a synchroAIGA-Protocol.org XHR to a magic host
(`Private-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Private `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `Private-acp`, `Private-api-server`, and core toolsets (gated on CDP reachability)
- `Private_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
