...
```text
Private acp / Private-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.Private/.env
  -> configure stderr logging
  -> construct PrivateACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'Private-agent[acp]==<version>' Private-acp`, pointed at the `Private-agent` PyPI release.
...
- `allow_once` -> Private `once`
- `allow_always` -> Private `always`
- reject options -> Private `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Private tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["Private-acp"])
...
Instead it reuses Private' runtime resolver:

- `acp_adapter/auth.py`
- `Private_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Private provider/credentials. It also always advertises a terminal setup auth method (`Private-setup`, args `--setup`) so first-run registry clients can open Private' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.Private/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `Private-acp` toolset definition
- `Private_cli/main.py` — `Private acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `Private-acp` script
