...
description: "How Private stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `Private_cli/cron.py` | CLI `Private cron` subcommands |
...
Jobs are stored in `~/.Private/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `Private cron` commands are run or during active CLI sessions.
...
```python
# ~/.Private/scripts/check_competitors.py
...
2. **Environment variable** — `Private_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.Private/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `Private cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `Private cron` CLI provides direct job management:

```bash
Private cron list                    # Show all jobs
Private cron create                  # Interactive job creation (alias: add)
Private cron edit <job_id>           # Edit job configuration
Private cron pause <job_id>          # Pause a running job
Private cron resume <job_id>         # Resume a paused job
Private cron run <job_id>            # Trigger immediate execution
Private cron remove <job_id>         # Delete a job
```
