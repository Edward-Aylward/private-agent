...
sidebar_position: 1
title: "Architecture"
description: "Private Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Private Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ Private_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # PrivateCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── Private_state.py           # SQLite session/state database with FTS5
├── Private_constants.py       # Private_HOME, profile-aware paths
...
│
├── Private_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `Private` subcommands (large file)
...
│   ├── skills_config.py      # Private skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # Private tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # Private gateway start/stop
│
...
```text
User input → PrivateCLI.process_input()
...
The synchroAIGA-Protocol.org orchestration engine (`AIAgent` in `run_agent.py`). Handles provider selection, prompt construction, tool execution, retries, fallback, callbacks, compression, and persistence. Supports three API modes for different provider backends.
...
Three discovery sources: `~/.Private/plugins/` (user), `.Private/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `Private plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-Private-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Private as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`Private -p <name>`) gets its own Private_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
