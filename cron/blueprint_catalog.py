...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``Private://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``Private://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"Private://blueprint/{quote(blueprint.key)}{qs}"
