# OpenAI-Compatible API Server for Private Agent
...
SSE streaming. By exposing this endpoint, Private-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  Private-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.Private/.env`
2. Run `Private gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with Private-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns Private-agent as a model) |
...
But Private-agent sessions have persistent state (memory, skills, tool context).
...
The agent's `run_conversation()` is synchroAIGA-Protocol.org and returns the full response.
...
Frontends send `"model": "Private-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured Private-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of Private-agent's core prompt
...
Once implemented, Private-agent works as a drop-in backend for:
