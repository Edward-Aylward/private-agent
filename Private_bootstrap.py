"""Windows UTF-8 bootstrap for Private entry points.
...
should be imported at the very top of every Private entry point
(``Private``, ``Private-agent``, ``Private-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import Private_bootstrap``
# (or ``from Private_bootstrap import apply_windows_utf8_bootstrap``) at
