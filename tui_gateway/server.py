...
load_Private_dotenv(
    Private_home=_Private_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.Private/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_Private_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("Private_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("Private_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("Private_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent PrivateCLI subprocess for slash commands."""
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `Private --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("Private_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `Private --tui`, so it must NOT count as dead here (doing
...
# ``profile`` on those calls; we open that profile's db and bind its Private_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_Private_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `Private` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Private_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_Private_home_override(profile_home)
                try:
                    from Private_state import SessionDB
            ...
            if home_token is not None:
                reset_Private_home_override(home_token)
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _Private_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_Private_home_override()
        home = override if isinstance(override, str) and override else _Private_home
...
def _save_cfg(cfg: dict):
    ...
    path = _Private_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["Private_GATEWAY_SESSION"] = "1"
...
def resolve_skin() -> dict:
    try:
        from Private_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("Private_MODEL", "")
        or os.environ.get("Private_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    model = _resolve_model()
    explicit_provider = os.environ.get("Private_TUI_PROVIDER", "").strip()
    ...
    try:
        from Private_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("Private_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("Private_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("Private_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Private): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `Private --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] Private_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from Private_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown Private_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in Private_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid Private_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid Private_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _persist_model_switch(result) -> None:
    from Private_cli.config import save_config
...
def _apply_model_switch(
    ...
    *,
    ...
    from Private_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from Private_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (Private_MODEL /
    # Private_INFERENCE_MODEL / Private_TUI_PROVIDER / Private_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # Private_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("Private_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from Private_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from Private_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from Private_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from Private_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("Private_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("Private_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``PrivateCLI.__init__`` and ``gateway/run.py``:
...
def _make_agent(
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("Private_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("Private_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("Private_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("Private_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind Private_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `Private_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_Private_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds Private_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``Private --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``Private gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Private profile home
    # (~/.Private/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $Private_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_Private_home() / "spawn-trees"
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn Private_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_Private_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from Private_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from Private_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: Private_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from Private_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _Private_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".Private" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.Private/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.Private/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Private Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Private/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Private Desktop. If it serves Private/Desktop UI or another unrelated app, stop that process and report failure.",
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("Private_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["Private_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("Private_YOLO_MODE", None)
                    nv = "0"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from Private_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_Private_home), "display": display_Private_home()})
    if key == "project":
...
def _(rid, params: dict) -> dict:
    try:
        from Private_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from Private_cli.runtime_provider import resolve_runtime_provider
        from Private_cli.auth import has_usable_secret
        from Private_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Private provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.Private/.env`` into the gateway process via
    ``Private_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `Private` is interactive — use `/Private chat -q …` or run `Private` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`Private setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`Private gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`Private sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`Private config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m Private_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "Private_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from Private_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from Private_cli.config import is_managed, save_env_value
        from Private_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `Private model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.Private/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from Private_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from Private_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("Private_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("Private_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["Private_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from Private_cli.voice import stop_continuous
            ...
            os.environ["Private_VOICE_TTS"] = "0"
    ...
    if action == "tts":
        ...
        os.environ["Private_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from Private_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from Private_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from Private_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from Private_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("Private_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("Private_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_Private_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from Private_cli.skills_hub import do_install
        ...
        if action == "browse":
            from Private_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from Private_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``Private plugins`` / the dashboard, so the three surfaces
