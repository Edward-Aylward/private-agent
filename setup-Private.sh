...
# 5. Symlinks the 'Private' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Private Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/Private-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink Private into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up Private command..."

Private_BIN="$SCRIPT_DIR/venv/bin/Private"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$Private_BIN" "$COMMAND_LINK_DIR/Private"
echo -e "${GREEN}✓${NC} Symlinked Private → $COMMAND_LINK_DISPLAY_DIR/Private"
...
# ============================================================================
# Seed bundled skills into ~/.Private/skills/
# ============================================================================

Private_SKILLS_DIR="${Private_HOME:-$HOME/.Private}/skills"
mkdir -p "$Private_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.Private/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$Private_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  Private status        # Check configuration"
if is_termux; then
    echo "  Private gateway       # Run gateway in foreground"
else
    echo "  Private gateway install # Install gateway service (messaging + cron)"
fi
echo "  Private cron list     # View scheduled jobs"
echo "  Private doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m Private_cli.main setup
fi
