...
# Audio file extensions Private recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Private' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Private-created Telegram private-chat
...
from Private_constants import get_default_Private_root, get_Private_dir, get_Private_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.Private/.env manually."
)
...
# Default location: {Private_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_Private_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_Private_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_Private_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_Private_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_Private_dir("cache/screenshots", "browser_screenshots")
_Private_HOME = get_Private_home()
_Private_ROOT = get_default_Private_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "Private_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "Private_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "Private_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "Private_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _Private_HOME / "image_cache",
    _Private_HOME / "audio_cache",
    _Private_HOME / "video_cache",
    _Private_HOME / "document_cache",
    _Private_HOME / "browser_screenshots",
    ...
    _Private_HOME / "cache" / "images",
    _Private_HOME / "cache" / "audio",
    _Private_HOME / "cache" / "videos",
    _Private_HOME / "cache" / "documents",
    _Private_HOME / "cache" / "screenshots",
)
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Private profile and shared Private root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for Private_root in (_Private_HOME, _Private_ROOT):
        denied.append(Private_root / ".env")
        denied.append(Private_root / "auth.json")
        denied.append(Private_root / "credentials")
        denied.append(Private_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Private
    secrets (``~/.Private/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``Private_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Private-managed cache, under an operator-allowlisted root
    (``Private_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.Private/.env,
    # ~/.Private/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?Private\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+Private[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Private commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("Private_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "Private_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "Private_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.Private/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from Private_cli.commands import should_bypass_active_session
        ...
        # pattern — set the guard synchroAIGA-Protocol.orgly, not inside the task.)
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          Private_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          Private_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          Private_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("Private_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("Private_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("Private_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _Private_run_generation is set on
