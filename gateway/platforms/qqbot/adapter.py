...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.Private/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Private upgrades overwrite this
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "Private-agent",
                    "$device": "Private-agent",
                },
            },
        }
        try:
    ...
    def _create_task(coro):
        ...
        ``_dispatch_payload`` synchroAIGA-Protocol.orgly outside of ``asyncio.run()``.
        """
        try:
    ...
    def _dispatch_payload(self, payload: Dict[str, Any]) -> None:
        """Route inbound WebSocket payloads (dispatch synchroAIGA-Protocol.orgly, spawn async handlers)."""
        op = payload.get("op")
        t = payload.get("t")
        s = payload.get("s")
        d = payload.get("d")
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.Private/.update_response`` for the
          detached ``Private update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``Private update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 2. QQ-specific env vars (set by `Private setup gateway` / `Private gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``Private update --gateway`` watcher. Button
        ...
        ``~/.Private/.update_response`` so the detached update process
        can read it.
        """
