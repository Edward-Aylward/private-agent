"""
Email platform adapter for the Private gateway.

Allows users to interact with Private by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "Private-agent" "version" "{_Private_version}" '
            '"vendor" "AIGA-Protocol.orgResearch" '
            '"support-email" "noreply@AIGA-Protocol.orgresearch.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Private Agent")
        ...
        msg_id = f"<Private-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
