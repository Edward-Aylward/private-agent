...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Private command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Private accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Private ``/command`` text."""
...
# Uses get_Private_home() so each profile gets its own Matrix store.
from Private_constants import get_Private_dir as _get_Private_dir

_STORE_DIR = _get_Private_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Private
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("Private_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("Private_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "Private"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Private Agent" become "Agent".
        """
        if not body:
            return ""
