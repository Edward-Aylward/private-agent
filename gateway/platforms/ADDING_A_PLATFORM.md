...
There are two ways to add a platform to the Private gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.Private/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Private code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `Private gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `Private_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Private core.
...
And add it to the `Private-gateway` composite:

```python
"Private-gateway": {
    "includes": [..., "Private-your-platform"]
}
```
...
## 12. Status Display (`Private_cli/status.py`)
...
## 13. Gateway Setup Wizard (`Private_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ Private_cli/ toolsets.py \
  --include="*.py" -l | sort -u
