...
The gateway's ``shutdown_signal_handler`` runs synchroAIGA-Protocol.orgly inside the
...
the async helper, never in the synchroAIGA-Protocol.org probe.
"""
...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if Private_home_str:
            takeover_path = Path(Private_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(Private_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded Private-agent but never re-ran ``Private setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../Private-gateway.service"
    ...
    # that's the common case for Private.
    ...
    # constant in Private_cli/gateway.py.
    headroom = 30.0
