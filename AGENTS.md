# Private Agent - Development Guide

Instructions for AI coding assistants and developers working on the Private-agent codebase.

**Never give up on the right solution.**

## What Private Is

Private is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Private ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`Private tools`, `Private setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `Private_HOME`. Mocks hide integration bugs.
...
- **New `Private_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `Private tools`
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `Private <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `Private webhook`, `Private cron`, `Private tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.Private/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.Private/Private-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _Private_CORE_TOOLS list
├── cli.py                # PrivateCLI class — interactive CLI orchestrator (~11k LOC)
├── Private_state.py       # SessionDB — SQLite session store (FTS5 search)
├── Private_constants.py   # get_Private_home(), display_Private_home() — profile-aware paths
├── Private_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── Private_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── Private-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `Private --tui`
...
**User config:** `~/.Private/config.yaml` (settings), `~/.Private/.env` (API keys only).
**Logs:** `~/.Private/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_Private_home()`.
Browse with `Private logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Private: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`Private_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `PrivateCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.Private/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`Private_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/Private` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `Private_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `PrivateCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `Private --tui` or `Private_TUI=1`.
...
npm run dev       # watch mode (rebuilds Private-ink + tsx --watch)
npm start         # production
npm run build     # full build (Private-ink + tsc)
...
### TUI in the Dashboard (`Private dashboard` → `/chat`)

The dashboard embeds the real `Private --tui` — **not** a rewrite.  See `Private_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `Private_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `Private --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `Private --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `Private --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `Private-desktop-app-work` skill, not `Private-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Private built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Private core.
Use the plugin route instead: create `~/.Private/plugins/<name>/plugin.yaml`
and `~/.Private/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Private tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_Private_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_Private_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_Private_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `Private_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_Private_home()` for the base directory — never `Path.home() / ".Private"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `Private_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `Private_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `Private tools`, `Private setup`, most CLI subcommands | `Private_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`Private_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
Private_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.Private/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Private gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `Private_cli/skin_engine.py`:
...
Users create `~/.Private/skins/<name>.yaml`:
...
Private has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.Private/plugins/` and pip-installed entry points.

### General plugins (`Private_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.Private/plugins/`, `./.Private/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `Private` at startup so
  `Private <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(Private_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `Private <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `Private --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `Private_cli/main.py`, etc.).
...
into `~/.Private/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `Private memory setup` / `post_setup()` without
...
2. User: `$Private_HOME/plugins/model-providers/<name>/`
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `Private skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.Private.tags`, `metadata.Private.category`,
`metadata.Private.related_skills`, `metadata.Private.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.Private.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Private tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Private Agent" is the secondary collaborator. If the
   contributor's commit shows "Private Agent" as author (because they
   used Private to draft the skill), replace it with their actual name
...
lives in the `Private-agent-dev` skill at
...
`"messaging"`); `_Private_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `Private tools` (the curses UI) or the
...
go to `~/.Private/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `Private_cli/curator.py` wires `Private curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.Private/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `Private cron <verb>`
...
- File lock at `~/.Private/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `Private kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `Private_cli/kanban.py` wires `Private kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`Private-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `Private_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Private-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `Private_BACKGROUND_NOTIFICATIONS` env var):
...
Private supports **profiles** — multiple fully isolated instances, each with its own
`Private_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `Private_cli/main.py` sets
`Private_HOME` before any module imports. All `get_Private_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_Private_home()` for all Private_HOME paths.** Import from `Private_constants`.
   NEVER hardcode `~/.Private` or `Path.home() / ".Private"` in code that reads/writes state.
   ```python
   # GOOD
   from Private_constants import get_Private_home
   config_path = get_Private_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".Private" / "config.yaml"
   ```

2. **Use `display_Private_home()` for user-facing messages.** Import from `Private_constants`.
   This returns `~/.Private` for default or `~/.Private/profiles/<name>` for profiles.
   ```python
   # GOOD
   from Private_constants import display_Private_home
   print(f"Config saved to {display_Private_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.Private/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_Private_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_Private_home()`,
   not `Path.home() / ".Private"`.

4. **Tests that mock `Path.home()` must also set `Private_HOME`** — since code now uses
   `get_Private_home()` (reads env var), not `Path.home() / ".Private"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"Private_HOME": str(tmp_path / ".Private")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not Private_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".Private" / "profiles"`, NOT `get_Private_home() / "profiles"`.
   This is intentional — it lets `Private -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.Private` paths
Use `get_Private_home()` from `Private_constants` for code paths. Use `display_Private_home()`
for user-facing print/log messages. Hardcoding `~/.Private` breaks profiles — each profile
has its own `Private_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `Private_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `Private_cli/curses_ui.py` — see
`Private_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `Private_HOME`.

### Tests must not write to `~/.Private/`
The `_isolate_Private_home` autouse fixture in `tests/conftest.py` redirects `Private_HOME` to a temp dir. Never hardcode `~/.Private/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_Private_home()` resolve within the temp dir.
Use the pattern from `tests/Private_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("Private_HOME", str(home))
    return home
```
...
- The plugin disables itself in child processes (sentinel envvar
  `Private_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.Private/` | Your real config+auth.json | Temp dir per test |
