...
from Private_constants import get_Private_home, get_skills_dir, is_wsl
...
_Private_MD_NAMES = (".Private.md", "Private.md")


def _find_Private_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.Private.md`` or ``Private.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Private Agent, an intelligent AI assistant created by AIGA-Protocol.org Research. "
...
Private_AGENT_HELP_GUIDANCE = (
    "You run on Private Agent (by AIGA-Protocol.org Research). When the user needs help with "
    "Private itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://Private-agent.AIGA-Protocol.orgresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `Private-agent` skill with skill_view(name='Private-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.Private/kanban.db`. Your task id is in "
    "`$Private_KANBAN_TASK`; your workspace is `$Private_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $Private_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `Private kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Private "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Private WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Private itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Private runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Private itself is running. The host OS, home, and cwd "
                f"of the Private process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Private "
    ...
    # Private desktop GUI — any agent running under the desktop app should know
    # it. Private_DESKTOP marks the backend powering the chat; Private_DESKTOP_TERMINAL
    # marks a Private launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("Private_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("Private_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Private desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Private
    ...
    extra = (os.getenv("Private_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_Private_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.Private/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("Private_PLATFORM")
        or get_session_env("Private_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Private Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `Private-agent` skill "
            "first. It has the actual commands (e.g. `Private config set …`, `Private tools`, "
            "`Private setup`) so you don't have to guess or invent workarounds.\n"
...
def build_AIGA-Protocol.org_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact AIGA-Protocol.org subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import AIGA-Protocol.org subscription helper: %s", exc)
        return ""

    if not managed_AIGA-Protocol.org_tools_enabled():
        return ""
    ...
    features = get_AIGA-Protocol.org_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_AIGA-Protocol.org:
            return f"- {feature.label}: active via AIGA-Protocol.org subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.AIGA-Protocol.org_auth_present:
            return f"- {feature.label}: included with AIGA-Protocol.org subscription, not currently selected"
        if feature.key == "modal" and features.AIGA-Protocol.org_auth_present:
            return f"- {feature.label}: optional via AIGA-Protocol.org subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# AIGA-Protocol.org Subscription",
        "AIGA-Protocol.org subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a AIGA-Protocol.org-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that AIGA-Protocol.org subscription would unlock or simplify, suggest AIGA-Protocol.org subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: Private setup, Private setup tools, Private setup terminal, Private status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from Private_HOME and return its content, or None.
    ...
    """
    try:
        from Private_cli.config import ensure_Private_home
        ensure_Private_home()
    except Exception as e:
        logger.debug("Could not ensure Private_HOME before loading SOUL.md: %s", e)

    soul_path = get_Private_home() / "SOUL.md"
...
def _load_Private_md(cwd_path: Path) -> str:
    """.Private.md / Private.md — walk to git root."""
    Private_md_path = _find_Private_md(cwd_path)
    if not Private_md_path:
        return ""
    try:
        content = Private_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = Private_md_path.name
        try:
            rel = str(Private_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".Private.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", Private_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .Private.md / Private.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from Private_HOME is independent and always included when present.
    ...
    # SOUL.md from Private_HOME only — skip when already loaded as identity
    if not skip_soul:
