...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Private starts and finds ``~/.openclaw/``.

    Points users at ``Private claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``Private claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Private, run "
        "`Private claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `Private claw cleanup` (renames it to ~/.openclaw.pre-migration — "
