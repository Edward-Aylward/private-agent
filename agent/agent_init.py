...
from Private_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: Private config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.Private/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from Private_cli.config import load_config as _load_pc_cfg
    ...
    # both live under ~/.Private/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from Private_logging import setup_logging, setup_verbose_logging
    setup_logging(Private_home=_ra()._Private_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that Private_logging.setup_logging() does not
        ...
        # handler level inside Private_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from Private_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from Private_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `Private model` to "
                        "select a provider, or run `Private setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff Private_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["Private_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.Private/sessions/ alongside gateway sessions
    Private_home = get_Private_home()
    agent.logs_dir = Private_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.Private/sessions/session_{sid}.json)
    ...
    try:
        from Private_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from Private_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "Private"
                    except Exception:
                        pass
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from Private_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Private Agent.  Choose a model with at least "
