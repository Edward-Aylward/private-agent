...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Private with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.Private/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Private
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``Private tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``Private setup`` for
