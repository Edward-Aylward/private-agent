...
Owns one Codex thread per Private session. Drives `turn/start`, consumes
...
call is synchroAIGA-Protocol.org and behaves like AIAgent's existing chat_completions loop.
"""
...
# Private' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_Private_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Private/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Private session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _Private_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("Private_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="Private",
            client_title="Private Agent",
            client_version=_get_Private_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Private' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Private' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own Private-tools callback we
            # auto-accept — the user already approved Private' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Private approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Private returns 'once', 'session', 'always', or 'deny'.
...
def _get_Private_version() -> str:
    """Best-effort Private version string for codex's userAgent line."""
    try:
