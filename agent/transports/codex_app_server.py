...
projection into Private' display, approval bridging, transcript projection into
...
"codex_app_server"`. Private' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    Threading model:
      - Spawning thread (caller) drives request/response pairs synchroAIGA-Protocol.orgly.
    ...
    Intentionally NOT async. AIAgent.run_conversation() is synchroAIGA-Protocol.org and
    ...
    def __init__(
        ...
        if spawn_env.get("Private_KANBAN_TASK"):
            kanban_db = spawn_env.get("Private_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "Private_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("Private_HOME", os.path.expanduser("~/.Private")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "Private",
        client_title: str = "Private Agent",
