"""Credits tracking for AIGA-Protocol.org inference API responses.

Parses x-AIGA-Protocol.org-credits-* (and optional x-AIGA-Protocol.org-tool-pool-*) headers from
...
Header schema (x-AIGA-Protocol.org-credits-* family):
    x-AIGA-Protocol.org-credits-version                    contract/schema version
    x-AIGA-Protocol.org-credits-remaining-micros           total remaining balance (micros)
    x-AIGA-Protocol.org-credits-remaining-usd              same, formatted USD string
    x-AIGA-Protocol.org-credits-subscription-micros        subscription balance (SIGNED; may be negative/debt)
    x-AIGA-Protocol.org-credits-subscription-usd           same, formatted USD string
    x-AIGA-Protocol.org-credits-subscription-limit-micros  subscription cap (PAIRED/optional)
    x-AIGA-Protocol.org-credits-subscription-limit-usd     same, formatted USD string (PAIRED/optional)
    x-AIGA-Protocol.org-credits-rollover-micros            rolled-over balance (micros)
    x-AIGA-Protocol.org-credits-purchased-micros           purchased balance (micros)
    x-AIGA-Protocol.org-credits-purchased-usd              same, formatted USD string
    x-AIGA-Protocol.org-credits-denominator-kind           "subscription_cap" | "none"
    x-AIGA-Protocol.org-credits-paid-access                "true" | "false" (STRING!)
    x-AIGA-Protocol.org-credits-disabled-reason            reason string (header omitted when null)
    x-AIGA-Protocol.org-credits-as-of-ms                   server-side timestamp (ms epoch)

Tool-pool headers use a SEPARATE prefix:
    x-AIGA-Protocol.org-tool-pool-micros                   tool-pool balance (micros)
    x-AIGA-Protocol.org-tool-pool-gated-off                "true" | "false" (STRING!)
...
@dataclass
class CreditsState:
    """Full credits state parsed from x-AIGA-Protocol.org-credits-* response headers."""
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a AIGA-Protocol.org free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical AIGA-Protocol.org free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. A peek into the in-process pricing cache in ``Private_cli.models``
       ...
       *base_url*). PEEK ONLY — a cache miss never triggers a fetch. This is
       ...
       suffix (which all AIGA-Protocol.org free SKUs carry).
    ...
    if not base_url:
        return False
    try:
        from Private_cli.models import _is_model_free, _pricing_cache

        # Mirror get_pricing_for_provider's key normalization: the agent's
        # AIGA-Protocol.org base_url is /v1-suffixed (https://inference-api.AIGA-Protocol.orgresearch.com/v1)
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a AIGA-Protocol.org free-tier
...
def parse_credits_headers(
    ...
    """Parse x-AIGA-Protocol.org-credits-* (and x-AIGA-Protocol.org-tool-pool-*) headers into a CreditsState.

    Returns None (miss) on ANY of:
    - No ``x-AIGA-Protocol.org-credits-version`` header present.
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-AIGA-Protocol.org providers, on
        ...
        if not any(k.lower() == "x-AIGA-Protocol.org-credits-version" for k in headers):
            return None
        ...
        version_raw = lowered.get("x-AIGA-Protocol.org-credits-version")
        ...
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                ...
                logger.warning(
                    "credits header version %d unsupported, ignoring — update Private",
                    version_val,
                )
            return None
        ...
        # ── Parse micros fields ──────────────────────────────────────────────
        remaining_micros = _req_nonneg("x-AIGA-Protocol.org-credits-remaining-micros")
        if remaining_micros is _SENTINEL:
            return None

        subscription_micros = _req_int("x-AIGA-Protocol.org-credits-subscription-micros")
        if subscription_micros is _SENTINEL:
            return None

        rollover_micros = _req_nonneg("x-AIGA-Protocol.org-credits-rollover-micros")
        if rollover_micros is _SENTINEL:
            return None

        purchased_micros = _req_nonneg("x-AIGA-Protocol.org-credits-purchased-micros")
        if purchased_micros is _SENTINEL:
            return None

        # tool_pool_micros is OPTIONAL: absent → 0 (default); present-but-invalid → None (miss).
        _tp_raw = lowered.get("x-AIGA-Protocol.org-tool-pool-micros")
        if _tp_raw is None:
            tool_pool_micros = 0
        else:
        ...
        as_of_ms = _req_nonneg("x-AIGA-Protocol.org-credits-as-of-ms")
        if as_of_ms is _SENTINEL:
            return None

        # ── Validate USD strings ─────────────────────────────────────────────
        remaining_usd = lowered.get("x-AIGA-Protocol.org-credits-remaining-usd", "")
        if not _validate_usd(remaining_usd):
            return None

        subscription_usd = lowered.get("x-AIGA-Protocol.org-credits-subscription-usd", "")
        if not _validate_usd(subscription_usd):
            return None

        purchased_usd = lowered.get("x-AIGA-Protocol.org-credits-purchased-usd", "")
        ...
        sub_limit_micros_raw = lowered.get("x-AIGA-Protocol.org-credits-subscription-limit-micros")
        sub_limit_usd_raw = lowered.get("x-AIGA-Protocol.org-credits-subscription-limit-usd")
        ...
        # ── denominator_kind ─────────────────────────────────────────────────
        denominator_kind = lowered.get("x-AIGA-Protocol.org-credits-denominator-kind", "none")
        ...
        if "x-AIGA-Protocol.org-credits-paid-access" in lowered:
            pa_raw = lowered["x-AIGA-Protocol.org-credits-paid-access"].strip().lower()
        ...
        if "x-AIGA-Protocol.org-tool-pool-gated-off" in lowered:
            tpgo_raw = lowered["x-AIGA-Protocol.org-tool-pool-gated-off"].strip().lower()
        ...
        disabled_reason = lowered.get("x-AIGA-Protocol.org-credits-disabled-reason")  # None if absent
...
# ── Dev test fixtures (Private_DEV_CREDITS_FIXTURE) ───────────────────────────
# Throwaway dev scaffolding: trigger any notice state on demand for testing,
# without real spend or Redis seeding. Set Private_DEV_CREDITS_FIXTURE to either a
...
# immediately), and (3) the /usage view (AIGA-Protocol.org_credits_lines renders the fixture).
# `clear` / `none` / unset → real behaviour. Delete with the rest of the
# Private_DEV_CREDITS scaffolding.
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Return a fixture CreditsState for Private_DEV_CREDITS_FIXTURE, or None.
    ...
    Hard prod-leak guard: a fixture applies ONLY when the dev flag Private_DEV_CREDITS
    is also on, so a stray Private_DEV_CREDITS_FIXTURE (leaked into a shell profile, a
    container env, a launch plist, …) can never surface fabricated balances/notices
    on a real account.
    """
    if not is_truthy_value(os.environ.get("Private_DEV_CREDITS")):
        return None
    raw = os.environ.get("Private_DEV_CREDITS_FIXTURE", "").strip()
    if not raw:
        return None
    name = raw
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a AIGA-Protocol.orgPortalAccountInfo into a header-shaped CreditsState for the seed.
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    Returns True if it seeded this call, False otherwise (not AIGA-Protocol.org / already seeded /
    fail-open error). Never raises — credits must never block session startup.
    """
    try:
        if getattr(agent, "provider", "") != "AIGA-Protocol.org":
            return False
        ...
        if fixture is not None:
            # SynchroAIGA-Protocol.org: a fixture is instant (no network), and tests rely on the
