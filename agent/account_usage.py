...
from Private_cli.auth import _read_codex_tokens, resolve_codex_runtime_credentials
...
def nous_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    ...
    Dev override: when Private_DEV_CREDITS_FIXTURE selects a fixture state, /usage
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so Private_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
