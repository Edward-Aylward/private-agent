...
def _Private_home_path() -> Path:
    """Resolve the active Private_HOME (profile-aware) without circular imports."""
    try:
        from Private_constants import get_Private_home  # local import to avoid cycles
        return get_Private_home()
    except Exception:
        return Path(os.path.expanduser("~/.Private"))


def _Private_root_path() -> Path:
    """Resolve the Private root dir (always the parent of any profile, never per-profile)."""
    try:
        from Private_constants import get_default_Private_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    Private_home = _Private_home_path()
    Private_root = _Private_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(Private_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(Private_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(Private_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(Private_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved Private_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("Private_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Private control-plane files: block both the ACTIVE profile's view
    # (Private_home) AND the global root view. Without the root pass, a
    ...
    Private_dirs = []
    for base in (_Private_home_path(), _Private_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in Private_dirs:
                Private_dirs.append(real)
        except Exception:
            continue

    for base_real in Private_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Private path.

    Three categories are blocked:

      * Internal Private cache files under ``Private_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under Private_HOME and the global Private
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.Private/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active Private_HOME (profile-aware) AND the global
    # Private root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (Private_HOME points at
    ...
    for hd in Private_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Private credential store "
    ...
    for hd in Private_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Private MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Private MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Private profiles are separate Private_HOME dirs under
...
#
# Reference: May 2026 incident where a Private-security profile session
# edited skills under both ``~/.Private/profiles/Private-security/skills/``
# AND ``~/.Private/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under Private_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from Private_HOME.

    ``~/.Private``              -> ``"default"``
    ``~/.Private/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _Private_home_path().resolve()
        root_real = _Private_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Private scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Private entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Private profile {info['target_profile']!r}, but the "
...
#
#   <Private_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.Private/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.Private/…`` segment and warns
# regardless of which Private profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.Private/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.Private`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.Private/…`` and returns the
    index where the inner Private-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .Private / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".Private":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Private state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.Private``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.Private`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Private resolver to
    succeed, so it works correctly even when called from contexts where
    Private_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Private process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real Private_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.Private/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.Private/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host Private_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Private state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Private "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real Private_HOME. Use the "
