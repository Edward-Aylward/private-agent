"""Unified removal contract for every credential source Private reads from.

Private seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.Private/.env
    claude_code   — ~/.claude/.credentials.json
    Private_pkce   — ~/.Private/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (AIGA-Protocol.org, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `Private auth add`
...
unify here is **removal**:
    ...
    ``Private auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, AIGA-Protocol.org device_code (partial), Private_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"AIGA-Protocol.org"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.Private/.env  → clear it
    ...
    """
    from Private_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.Private/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Private.",
            f"  The pool entry is now suppressed — Private will ignore "
            f"{env_var} until you run `Private auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Private stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `Private auth add anthropic` to re-enable if needed.",
    ])


def _remove_Private_pkce(provider: str, removed) -> RemovalResult:
    """~/.Private/.anthropic_oauth.json is ours — delete it outright."""
    from Private_constants import get_Private_home

    result = RemovalResult()
    oauth_file = get_Private_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Private Anthropic OAuth credentials")
...
def _remove_AIGA-Protocol.org_device_code(provider: str, removed) -> RemovalResult:
    """AIGA-Protocol.org OAuth lives in auth.json providers.AIGA-Protocol.org — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `Private auth add AIGA-Protocol.org` (or any other path that writes providers.AIGA-Protocol.org)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `Private auth add AIGA-Protocol.org` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as AIGA-Protocol.org: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``Private auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``Private auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `Private model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Private auth store is not enough — _seed_from_singletons() would
    ...
    via ``Private auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `Private auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `Private auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Private stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `Private auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="Private_pkce",
        remove_fn=_remove_Private_pkce,
        description="~/.Private/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="AIGA-Protocol.org", source_id="device_code",
        remove_fn=_remove_AIGA-Protocol.org_device_code,
        description="auth.json providers.AIGA-Protocol.org",
    ))
    register(RemovalStep(
