...
qwen-cli, nous device_code (partial), Private_pkce, copilot gh_cli, and
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `Private auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `Private auth add nous` to re-engage, which is the
