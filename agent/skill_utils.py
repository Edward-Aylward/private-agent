...
from Private_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``Private_KANBAN_TASK`` / ``Private_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("Private_KANBAN_TASK") or os.getenv("Private_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Private Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``Private_PLATFORM`` or
            ``Private_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``Private`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.Private/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``Private`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against Private_HOME, not cwd
        if not p.is_absolute():
            p = (Private_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.Private/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    Private = metadata.get("Private") or {}
    if not isinstance(Private, dict):
        Private = {}
    return {
        "fallback_for_toolsets": Private.get("fallback_for_toolsets", []),
        "requires_toolsets": Private.get("requires_toolsets", []),
        "fallback_for_tools": Private.get("fallback_for_tools", []),
        "requires_tools": Private.get("requires_tools", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Private metadata, VCS, virtualenv/dependency, and cache
