...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_Private_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``Private_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_Private_home()`` themselves.
        """
        if "Private_home" not in kwargs:
            from Private_constants import get_Private_home
            kwargs["Private_home"] = str(get_Private_home())
        for provider in self._providers:
            try:
