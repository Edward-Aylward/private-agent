...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.Private/auth.json active provider)
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://Private-agent.nousresearch.com",
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.Private/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from Private_cli.auth import _nous_invoke_jwt_is_usable
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: Private auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
