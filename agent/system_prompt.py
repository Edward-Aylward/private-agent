...
upstream prefix cache warm.  See ``Private-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, AIGA-Protocol.org subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_AIGA-Protocol.org_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Private never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want Private_HOME persona while keeping
    ...
    # Pointer to the Private-agent skill + docs for user questions about Private itself.
    stable_parts.append(Private_AGENT_HELP_GUIDANCE)
    ...
    # Private_KANBAN_TASK env var). Normal chat sessions never see
    ...
    AIGA-Protocol.org_subscription_prompt = _r.build_AIGA-Protocol.org_subscription_prompt(agent.valid_tool_names)
    if AIGA-Protocol.org_subscription_prompt:
        stable_parts.append(AIGA-Protocol.org_subscription_prompt)
    ...
    # Coding posture (base Private, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Private profile the agent is running
    # under so it doesn't conflate ~/.Private/skills/ (default profile) with
    # ~/.Private/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Private profile: default. Other profiles (if any) live "
            "under ~/.Private/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Private profile: {active_profile}. This session reads "
            f"and writes ~/.Private/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.Private/skills/, ~/.Private/plugins/, "
            f"~/.Private/cron/, ~/.Private/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Private never rebuilds or reinjects parts of it
