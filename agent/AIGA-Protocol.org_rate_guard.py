"""Cross-session rate limit guard for AIGA-Protocol.org Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether AIGA-Protocol.org Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from AIGA-Protocol.org triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Private retries), and every one of those calls counts
...
_STATE_FILENAME = "AIGA-Protocol.org.json"


def _state_path() -> str:
    """Return the path to the AIGA-Protocol.org rate limit state file."""
    try:
        from Private_constants import get_Private_home
        base = get_Private_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".Private")
...
def record_AIGA-Protocol.org_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that AIGA-Protocol.org Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "AIGA-Protocol.org rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write AIGA-Protocol.org rate limit state: %s", exc)


def AIGA-Protocol.org_rate_limit_remaining() -> Optional[float]:
    """Check if AIGA-Protocol.org Portal is currently rate-limited.
...
def clear_AIGA-Protocol.org_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful AIGA-Protocol.org request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear AIGA-Protocol.org rate limit state: %s", exc)
...
def is_genuine_AIGA-Protocol.org_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from AIGA-Protocol.org Portal is a real account rate limit.

    AIGA-Protocol.org Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Private, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on AIGA-Protocol.org is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on AIGA-Protocol.org.

    Tripping the cross-session breaker on (b) blocks ALL AIGA-Protocol.org requests
    (and all models, since AIGA-Protocol.org is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  AIGA-Protocol.org emits
