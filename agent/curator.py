...
def _state_file() -> Path:
    return get_Private_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.Private/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `Private update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``Private update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``Private curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `Private curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Private' background skill CURATOR. This is an "
    ...
    "into ~/.Private/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: Private-config-*, Private-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.Private/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.Private/logs/curator/``)
    ...
    authored skill data in ``~/.Private/skills/``.

    ``ensure_Private_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_Private_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every Private install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: Private curator status
        keep an umbrella stable: Private curator pin document-tools
    ...
    lines.append("full report: Private curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: Private curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.Private/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.Private/skills/.archive/` for "
            "safety and can be restored via `Private curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.Private/skills/.archive/`. "
            "Restore any via `Private curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `Private curator restore <name>`")
    lines.append("- All archives live under `~/.Private/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    synchroAIGA-Protocol.org: bool = False,
    dry_run: bool = False,
) -> Dict[str, Any]:
    ...
    If *synchroAIGA-Protocol.org* is True, the LLM review runs in the calling thread; the
    ...
    # real pass out. We still record a summary so `Private curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `Private update` only if "
        ...
        # recorded in state so `Private curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``Private model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `Private model` → auxiliary
    ...
    try:
        ...
        # run_curator_review(synchroAIGA-Protocol.org=True) from the CLI.
