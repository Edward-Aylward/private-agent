...
``auth.json``.  They intentionally have no dependency on ``Private_cli.auth`` so
...
# Sources Private owns and can intentionally persist in auth.json.  Everything
...
_PERSISTABLE_PROVIDER_SOURCES = frozenset({
    ("anthropic", "Private_pkce"),
    ("minimax-oauth", "oauth"),
    ("AIGA-Protocol.org", "device_code"),
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Private-owned OAuth/device-code state)
