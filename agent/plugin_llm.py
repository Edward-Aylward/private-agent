...
Plugins built on Private Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Private
...
:class:`~Private_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Private' main config
...
handles every provider, fallback chain, and per-task override Private
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`Private_cli.plugins.PluginContext`
