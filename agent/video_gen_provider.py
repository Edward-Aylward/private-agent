...
as ``kind: backend``) or ``~/.Private/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``Private tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$Private_HOME/cache/videos/``, creating parents as needed."""
    from Private_constants import get_Private_home

    path = get_Private_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$Private_HOME/cache/videos/``.
