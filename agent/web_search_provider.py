...
``kind: backend``) or ``~/.Private/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``Private tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by ``Private_cli/tools_config.py`` to inject this provider as a
