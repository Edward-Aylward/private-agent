...
Storage
-------
Bundles live in ``~/.Private/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``Private_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``Private bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under Private_HOME.

    Honors ``Private_BUNDLES_DIR`` for tests; falls back to
    ``<Private_HOME>/skill-bundles``.
    """
    override = os.environ.get("Private_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_Private_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `Private bundles` CLI subcommand.
# ---------------------------------------------------------------------------
