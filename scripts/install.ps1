# ============================================================================
# Private Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$PrivateHome = $(if ($env:Private_HOME) { $env:Private_HOME } else { "$env:LOCALAPPDATA\Private" }),
    [string]$InstallDir = $(if ($env:Private_HOME) { "$env:Private_HOME\Private-agent" } else { "$env:LOCALAPPDATA\Private\Private-agent" }),
    ...
    # builds apps/desktop into a launchable Private.exe.
    #
    # Why opt-in:
    #   * Private-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Private.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Private.exe
    ...
    #     `Private desktop` already builds on demand.
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $PrivateHome)) {
        New-Item -ItemType Directory -Force -Path $PrivateHome | Out-Null
    }
    $envFile = Join-Path $PrivateHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $PrivateHome "node"
...
function Install-Uv {
    # Private owns its own uv at $PrivateHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (Private_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `Private update` stay in sync.
    $managedUv = Join-Path $PrivateHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $PrivateHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $PrivateHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $PrivateHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $PrivateHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and Private_HOME wasn't propagated).
...
function Install-Git {
    ...
    which Private uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\Private\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Private needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Private copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\Private\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``Private_GIT_BASH_PATH`` (User scope) so Private can find it in a fresh
    ...
    # Download PortableGit into $PrivateHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $PrivateHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Private features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Private needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``Private_GIT_BASH_PATH`` (User env scope) so Private can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $PrivateHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $PrivateHome\git\usr\bin\bash.exe
    $candidates += "$PrivateHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$PrivateHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("Private_GIT_BASH_PATH", $candidate, "User")
            $env:Private_GIT_BASH_PATH = $candidate
            Write-Info "Set Private_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Private may not find Git Bash."
    Write-Info "If needed, set Private_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$PrivateHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$PrivateHome\node;$env:Path"
        Write-Success "Node.js $version found (Private-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Private-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $PrivateHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $PrivateHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\Private-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$PrivateHome\node") { Remove-Item -Recurse -Force "$PrivateHome\node" }
                Move-Item $extractedDir.FullName "$PrivateHome\node"
                ...
                $version = & "$PrivateHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $PrivateHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\Private-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "Private-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Private behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running Private processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\Private-agent-extract"
    ...
    # next `Private update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `Private.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install Private-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and Private.exe broken with
    ...
    # here, not 30 seconds later when the user runs `Private`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `Private dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `Private dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `Private dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up Private command..."
    
    if ($NoVenv) {
        $PrivateBin = "$InstallDir"
    } else {
        $PrivateBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so Private is globally available
    # On Windows, the Private.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$PrivateBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$PrivateBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $PrivateBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set Private_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\Private instead
    # of the Unix default ~/.Private
    $currentPrivateHome = [Environment]::GetEnvironmentVariable("Private_HOME", "User")
    if (-not $currentPrivateHome -or $currentPrivateHome -ne $PrivateHome) {
        [Environment]::SetEnvironmentVariable("Private_HOME", $PrivateHome, "User")
        Write-Success "Set Private_HOME=$PrivateHome"
    }
    $env:Private_HOME = $PrivateHome
    
    # Update current session
    $env:Path = "$PrivateBin;$env:Path"
    
    Write-Success "Private command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.Private-bootstrap-complete which tells the Private
    ...
    # Private-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `Private update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".Private-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the Private_HOME directory structure ($PrivateHome, default %LOCALAPPDATA%\Private)
    New-Item -ItemType Directory -Force -Path "$PrivateHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$PrivateHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$PrivateHome\.env"
    ...
    # Create config.yaml
    $configPath = "$PrivateHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Private's prompt-injection scanner
...
Edit this to customize how Private communicates with you.
...
"@
    ...
    Write-Success "Configuration directory ready: $PrivateHome"
    
    # Seed bundled skills into $PrivateHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $PrivateHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $PrivateHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$PrivateHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $PrivateHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Private-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'Private setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\Private-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\Private-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\Private-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Private` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Private.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_Private_desktop command
    # resolves apps/desktop/release/win-unpacked/Private.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Private-managed
    ...
    # The --dir mode produces an unpacked Private.exe in
    ...
    # are NOT trying to work around). The Private icon + product name are
    # stamped onto Private.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\Private-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Private` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Private.exe",
        "$desktopDir\release\win-arm64-unpacked\Private.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Private.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Private icon + identity are stamped onto Private.exe by the
    ...
    #    Private.exe. We deliberately do NOT point them at `Private desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Private.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Private.lnk')
        )
        ...
        # with the Private icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.Private/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'Private setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m Private_cli.main setup
    } else {
        python -m Private_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $PrivateCmd = "$InstallDir\venv\Scripts\Private.exe"
    if (-not (Test-Path $PrivateCmd)) {
        $PrivateCmd = "Private"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$PrivateHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'Private whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: Private gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$PrivateHome\logs\gateway.log"
            Start-Process -FilePath $PrivateCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$PrivateHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: Private gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: Private gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$PrivateHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$PrivateHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PrivateHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$PrivateHome\Private-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   Private              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   Private setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   Private config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   Private config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   Private gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   Private update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Private repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Private-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Private to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
