#!/usr/bin/env python3
"""Live test harness for Private Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any Private imports.
ORIGINAL_HOME = os.environ.get("Private_HOME")
ORIGINAL_AUTH = Path.home() / ".Private" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.Private/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.Private/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="Private_ts_live_"))
    Private_home = home_dir / ".Private"
    Private_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, Private_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".Private" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, Private_home / ".env")
        ...
        load_Private_dotenv(Private_home=str(Path.home() / ".Private"))
    ...
    (Private_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return Private_home
...
def reset_module_state():
    """Drop cached modules so the new Private_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "Private_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["Private_HOME"] = str(home)
...
def main():
    ...
    # Restore original Private_HOME
    if ORIGINAL_HOME is not None:
        os.environ["Private_HOME"] = ORIGINAL_HOME
    else:
