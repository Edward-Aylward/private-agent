...
# Bootstrap Open WebUI against Private Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.Private/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-Private.sh
...
#   OPEN_WEBUI_NAME='Johnny Private'
...
#   Private_API_PORT=8642
#   Private_API_HOST=127.0.0.1
#   Private_API_MODEL_NAME='Private Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Private Agent WebUI}"
...
Private_ENV_FILE="${Private_ENV_FILE:-$HOME/.Private/.env}"
Private_API_PORT="${Private_API_PORT:-8642}"
Private_API_HOST="${Private_API_HOST:-127.0.0.1}"
Private_API_CONNECT_HOST="${Private_API_CONNECT_HOST:-127.0.0.1}"
Private_API_MODEL_NAME="${Private_API_MODEL_NAME:-Private Agent}"
Private_API_BASE_URL="http://${Private_API_CONNECT_HOST}:${Private_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-Private.sh"
LOG_DIR="$HOME/.Private/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$Private_API_BASE_URL")"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.Private.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.Private</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.Private"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.Private"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-Private.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Private Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-Private.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.Private/logs/openwebui.log
StandardError=append:%h/.Private/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-Private.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$Private_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Private API server is configured...'
  upsert_env API_SERVER_ENABLED true "$Private_ENV_FILE"
  upsert_env API_SERVER_HOST "$Private_API_HOST" "$Private_ENV_FILE"
  upsert_env API_SERVER_PORT "$Private_API_PORT" "$Private_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$Private_API_MODEL_NAME" "$Private_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$Private_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Private gateway so API server settings take effect...'
  Private gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${Private_API_CONNECT_HOST}:${Private_API_PORT}/health" >/dev/null; then
    log 'Private API server did not answer on the first check. Trying to start gateway in the background...'
    nohup Private gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${Private_API_CONNECT_HOST}:${Private_API_PORT}/health" >/dev/null
  ...
  log "Private API endpoint: ${Private_API_BASE_URL}"
