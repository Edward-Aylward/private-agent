...
it is automatically downloaded from GitHub releases to $Private_HOME/bin/tirith.
...
def _get_Private_home() -> str:
    """Return the Private home directory, respecting Private_HOME env var."""
    return str(get_Private_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_Private_home(), ".tirith-install-failed")
...
def _Private_bin_dir() -> str:
    """Return $Private_HOME/bin, creating it if needed."""
    d = os.path.join(_get_Private_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $Private_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_Private_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $Private_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $Private_HOME/bin/tirith
    ...
    Private_bin = os.path.join(_Private_bin_dir(), "tirith")
    if os.path.isfile(Private_bin) and os.access(Private_bin, os.X_OK):
        _resolved_path = Private_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return Private_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchroAIGA-Protocol.org; network download runs in a
    ...
    # Explicit path: synchroAIGA-Protocol.org check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
