...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("Private_YOLO_MODE", ""))
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from Private_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("Private_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("Private_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("Private_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set Private_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind Private_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds Private_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("Private_CRON_SESSION"):
        return False
    if env_var_enabled("Private_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $Private_HOME, or by the resolved absolute
# active profile home path such as /home/Private/.Private/config.yaml. The
# resolved-absolute form is folded into the ~/.Private/ patterns at detection
...
# go stale when Private_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_Private_ENV_PATH = (
    r'(?:~\/\.Private/|'
    r'(?:\$home|\$\{home\})/\.Private/|'
    r'(?:\$Private_home|\$\{Private_home\})/)'
    r'\.env\b'
)
# ~/.Private/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _Private_ENV_PATH; matches the Private_HOME override form as
# well as ~/.Private/.
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_Private_ENV_PATH}|'
    rf'{_Private_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bPrivate\s+gateway\s+(stop|restart)\b', "stop/restart Private gateway (kills running agents)"),
    (r'\bPrivate\s+update\b', "Private update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `Private gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart Private-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart Private-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(Private|gateway|cli\.py)\b', "kill Private/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill Private` but not
    # `kill -9 $(pgrep -f Private)` because the substitution is opaque
    ...
    # In-place edit of a Private-managed security file (~/.Private/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_Private_CONFIG_PATH}|{_Private_ENV_PATH})', "in-place edit of Private config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_Private_CONFIG_PATH}|{_Private_ENV_PATH})', "in-place edit of Private config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_Private_CONFIG_PATH}|{_Private_ENV_PATH})', "in-place edit of Private config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.Private/ form so the Private config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/Private/.Private/config.yaml`) rather than
    # ~, $HOME, or $Private_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live Private_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_Private_home(command)
    return command


def _rewrite_resolved_Private_home(command: str) -> str:
    """Rewrite the resolved absolute Private home prefix to ``~/.Private/``.

    Resolves the active ``Private_HOME`` at call time (and its symlink-resolved
    form) and replaces an occurrence of ``<home>/`` in *command* with
    ``~/.Private/`` so the static ``_Private_CONFIG_PATH`` / ``_Private_ENV_PATH``
    patterns match. No-op when the path can't be resolved or doesn't appear.
    """
    try:
        from Private_constants import get_Private_home
        home = get_Private_home().expanduser()
        candidates = [
            str(home).rstrip("/"),
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate Private_HOME (e.g. "/" or "") rewriting
        ...
        # /home/Private/.Private or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.Private/")
    return command
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchroAIGA-Protocol.org input() flow)
# =========================================================================
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["Private_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "Private_SPINNER_PAUSE" in os.environ:
            del os.environ["Private_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("Private_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set Private_INTERACTIVE or Private_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("Private_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("Private_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("Private_EXEC_ASK")
    ...
    # responds with /approve or /deny, mirroring the CLI's synchroAIGA-Protocol.org
