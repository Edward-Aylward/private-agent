...
`Private skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.Private.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.Private.blueprint`` is a mapping containing
    ...
    Private = meta.get("Private") if isinstance(meta, dict) else None
    blueprint = Private.get("blueprint") if isinstance(Private, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.Private.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.Private.blueprint`` block) they can hand
    to ``Private skills publish`` to share. ``body`` is the plain-language
