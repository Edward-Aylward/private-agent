...
One ``CDPSupervisor`` runs per Private ``task_id`` that has a reachable CDP
...
thread-safe snapshot object that tool handlers consume synchroAIGA-Protocol.orgly.
...
DIALOG_BRIDGE_HOST = "Private-dialog-bridge.invalid"
...
(() => {
  if (window.__PrivateDialogBridgeInstalled) return;
  window.__PrivateDialogBridgeInstalled = true;
  const ENDPOINT = "http://Private-dialog-bridge.invalid/";
  ...
  // onbeforeunload — we can't really synchroAIGA-Protocol.orgly prompt the user from this
...
class CDPSupervisor:
    ...
    async def _attach_initial_page(self) -> None:
        ...
        # a synchroAIGA-Protocol.org XHR we intercept via Fetch domain. This is how we make
    ...
    async def _on_fetch_paused(
        ...
        The injected script (``_DIALOG_BRIDGE_SCRIPT``) fires a synchroAIGA-Protocol.org
