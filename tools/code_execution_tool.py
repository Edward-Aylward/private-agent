...
Lets the LLM write a Python script that calls Private tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `Private_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `Private_tools.py` with file-based RPC stubs
...
# available on every platform Private itself runs on.
...
# match a safe prefix, the operational Private_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "Private_" prefix was deliberately removed (#27303) — it leaked
# Private_*-named config that lacks a secret substring (e.g. Private_BASE_URL,
# Private_KANBAN_DB, Private_*_WEBHOOK).  The child only needs the few
# location/profile vars in _Private_CHILD_ALLOWED below; Private_RPC_SOCKET /
# Private_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational Private_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set Private_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_Private_CHILD_ALLOWED = frozenset({
    "Private_HOME",
    "Private_PROFILE",
    "Private_CONFIG",
    "Private_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational Private_* vars (_Private_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret Private_* vars dropped by the tightened allowlist (#27303). The
    # broad "Private_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like Private_KANBAN_DB / Private_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _Private_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        ...
        if k.startswith("Private_"):
            ...
            # allowlist — a deliberately-dropped Private_* var.
            _dropped_Private.append(k)
    if _dropped_Private:
        logger.debug(
            "execute_code: dropped %d non-allowlisted Private_* var(s) from the "
            ...
            len(_dropped_Private),
            ", ".join(sorted(_dropped_Private)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# Private_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Private-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Private-profile soft guard."""',
...
def generate_Private_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the Private_tools.py stub module.
...
"""Auto-generated Private tools RPC stubs."""
...
def _connect():
    ...
    Private_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["Private_RPC_SOCKET"]
...
"""Auto-generated Private tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("Private_RPC_DIR") or os.path.join(tempfile.gettempdir(), "Private_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated Private_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/Private_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_Private_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/Private_tools.py", tools_src)
        ...
        env_prefix = (
            f"Private_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("Private_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # --- Set up temp directory with Private_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="Private_sandbox_")
    ...
    # selector from Private_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"Private_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated Private_tools module.
        ...
        tools_src = generate_Private_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "Private_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  Private_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["Private_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the Private-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from Private_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _Private_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _Private_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # Private_TIMEZONE is an internal Private setting and must not leak
        # into child processes.
        _tz_name = os.getenv("Private_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("Private_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {Private_HOME}/home/ when that directory exists.
        from Private_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.Private/.env')).
...
def _load_config() -> dict:
    ...
    ``Private --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (Private-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``Private tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and Private-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.Private/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Private tools programmatically. "
        ...
        f"Available via `from Private_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into Private_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from Private_tools import {import_str}` "
