...
This module provides the glue:
    - ``PrivateTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Private Agent"
"""
...
from Private_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__Private_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses Private_HOME so each profile gets its own OAuth tokens.
    Layout: ``Private_HOME/mcp-tokens/``
    """
    try:
        from Private_constants import get_Private_home
        base = Path(get_Private_home())
    except ImportError:
        base = Path(os.environ.get("Private_HOME", str(Path.home() / ".Private")))
...
# ---------------------------------------------------------------------------
# PrivateTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------
...
class PrivateTokenStorage:
    ...
    File layout::
        ...
        Private_HOME/mcp-tokens/<server_name>.json         -- tokens
        Private_HOME/mcp-tokens/<server_name>.client.json   -- client info
        Private_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Private records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Private.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://Private-agent.AIGA-Protocol.orgresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `Private mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Private Agent")
    scope = cfg.get("scope")
