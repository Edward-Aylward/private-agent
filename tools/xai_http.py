...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``Private tools`` repaint, tool-registration scans,
    ...
    2. ``~/.Private/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_Private_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.Private/.env`` first, then ``os.environ``.

    Wraps :func:`Private_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from Private_cli.config import get_env_value as _Private_get_env_value

        value = _Private_get_env_value(name)
...
def Private_xai_user_agent() -> str:
    """Return a stable Private-specific User-Agent for xAI HTTP calls."""
    try:
        from Private_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Private-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Private-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``Private_cli.config.get_env_value`` so keys
    stored in ``~/.Private/.env`` (the standard Private location) are honored —
