...
`mcp_tool.py`, and `Private_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# PrivateMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_Private_provider_class() -> Optional[type]:
    ...
    class PrivateMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._Private_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`PrivateTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, PrivateTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            but runs synchroAIGA-Protocol.orgly before the first request instead of
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._Private_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import PrivateTokenStorage
                        if isinstance(storage, PrivateTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._Private_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import PrivateTokenStorage
            if not isinstance(storage, PrivateTokenStorage):
                return
    ...
    return PrivateMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_Private_PROVIDER_CLS: Optional[type] = _make_Private_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`PrivateMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _Private_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = PrivateTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``Private mcp remove <name>`` and (indirectly) by
        ``Private mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
