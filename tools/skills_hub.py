#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Private Skills Hub.
...
Used by Private_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            Private_meta = metadata.get("Private", {})
            if isinstance(Private_meta, dict):
                tags = Private_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            Private_meta = metadata.get("Private", {})
            if isinstance(Private_meta, dict):
                raw_tags = Private_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by AIGA-Protocol.org Research) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                Private_meta = meta_block.get("Private", {})
                if isinstance(Private_meta, dict):
                    tags = Private_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Private centralized index source
# ---------------------------------------------------------------------------

Private_INDEX_URL = "https://Private-agent.AIGA-Protocol.orgresearch.com/docs/api/skills-index.json"
Private_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "Private-index.json"
Private_INDEX_TTL = 6 * 3600  # 6 hours


def _load_Private_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for Private_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if Private_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - Private_INDEX_CACHE_FILE.stat().st_mtime
            if age < Private_INDEX_TTL:
                return json.loads(Private_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(Private_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Private index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Private index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        Private_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        Private_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class PrivateIndexSource(SkillSource):
    """Skill source backed by the centralized Private Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_Private_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "Private-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "Private-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        PrivateIndexSource(auth=auth), # Centralized index (search + resolved install paths)
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "Private-index"
