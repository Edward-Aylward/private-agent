...
4. Store the token in `${Private_HOME:-~/.Private}/.env` (or via `Private setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `${Private_HOME:-~/.Private}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
