#!/usr/bin/env python3
"""Google Workspace API CLI for Private Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Private-facing JSON contract and falls back to the Python client
...
from _Private_home import get_Private_home
...
def _gws_binary() -> str | None:
    override = os.getenv("Private_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Private Agent")
