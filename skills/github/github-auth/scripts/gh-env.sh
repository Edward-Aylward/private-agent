#!/usr/bin/env bash
# GitHub environment detection helper for Private Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _Private_env="${Private_HOME:-$HOME/.Private}/.env"; [ -f "$_Private_env" ] && grep -q "^GITHUB_TOKEN=" "$_Private_env" 2>/dev/null; then
