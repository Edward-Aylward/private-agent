...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _Private_env="${Private_HOME:-$HOME/.Private}/.env"; [ -f "$_Private_env" ] && grep -q "^GITHUB_TOKEN=" "$_Private_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_Private_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
