...
~/.Private/.env              API keys and secrets (under $Private_HOME if set)
...
Config: `~/.Private/config.yaml` (settings), `~/.Private/.env` (API keys) — both under `$Private_HOME` when it is set.
