...
Private_HOME_DIR="${Private_HOME:-$HOME/.Private}"
Private_CFG="${Private_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Private config has twozero_td MCP entry ──
if [[ ! -f "$Private_CFG" ]]; then
    echo -e " ${FAIL} Private config not found at ${Private_CFG}"
    manual_steps+=("Create ${Private_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$Private_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Private config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Private config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$Private_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${Private_CFG} manually"); }
    manual_steps+=("Restart Private session to pick up config change")
fi
