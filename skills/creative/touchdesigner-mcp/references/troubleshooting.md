...
`$Private_HOME/config.yaml` (defaults to `~/.Private/config.yaml` when `Private_HOME` is unset)
...
Restart the Private session for changes to take effect. The MCP connection is
