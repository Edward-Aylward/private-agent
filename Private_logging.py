"""Centralized logging setup for Private Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.Private/logs/`` (profile-aware via ``get_Private_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``Private_cli.web_server``,
    ``Private_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
from Private_constants import get_config_path, get_Private_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_Private_session_injector", False):
    ...
    _session_record_factory._Private_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``Private logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "Private_plugins"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("Private_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "Private_cli.web_server",
        "Private_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Private logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    Private_home
        Override for the Private home directory.  Falls back to
        ``get_Private_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_Private_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._Private_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the Private group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from Private_cli.config import is_managed
