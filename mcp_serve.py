"""
Private MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Private-specific extra)

Usage:
    Private mcp serve
    Private mcp serve --verbose
...
logger = logging.getLogger("Private.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using Private_HOME."""
    try:
        from Private_constants import get_Private_home
        return get_Private_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("Private_HOME", Path.home() / ".Private")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from Private_constants import get_Private_home
        directory_file = get_Private_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("Private_HOME", Path.home() / ".Private")
...
class EventBridge:
    ...
    This is the Private equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from Private_constants import get_Private_home
            db_file = get_Private_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("Private_HOME", Path.home() / ".Private")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Private MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "Private",
        instructions=(
            "Private Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Private MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
