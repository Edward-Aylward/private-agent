...
## Client ID
- Need to register a "Desktop app" OAuth client on a AIGA-Protocol.org Research GCP project
...
## Token Lifecycle
- Store at `~/.Private/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `Private_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `Private_cli/models.py` — Add Gemini model catalog
4. `Private_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `Private_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `Private_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- AIGA-Protocol.org Research GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via Private_GEMINI_CLIENT_ID env var
...
- Private-agent Copilot OAuth: `Private_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
