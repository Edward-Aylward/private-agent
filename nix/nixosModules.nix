# nix/nixosModules.nix — NixOS module for Private-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: Private runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $Private_HOME/.env
# and read by Private at startup — no container recreation needed for env changes.
#
# Tool resolution: the Private wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.Private-agent;
    effectivePackage =
    ...
    Private-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-Private-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "Private-config-attrs";
      description = "Private YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "Private-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # Private_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "Private-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'Private_DOC_EOF'\n${value}\nPrivate_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "Private-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/Private";
    ...
    # Private user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "Private-container-entrypoint" ''
      set -eu

      Private_UID="''${Private_UID:?Private_UID must be set}"
      Private_GID="''${Private_GID:?Private_GID must be set}"

      # ── Group: ensure a group with GID=$Private_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$Private_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="Private"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$Private_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$Private_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$Private_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$Private_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="Private"
        TARGET_HOME="/home/Private"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$Private_UID" -g "$Private_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$Private_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$Private_UID:$Private_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure Private_HOME is owned by the target user.
      ...
      if [ -n "''${Private_HOME:-}" ] && [ -d "$Private_HOME" ]; then
        find "$Private_HOME" \! -user "$Private_UID" -exec chown "$Private_UID:$Private_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/Private-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/Private-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/Private ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/Private
        chmod 0440 /etc/sudoers.d/Private
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$Private_UID" --regid="$Private_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $Private_HOME/.env.
    ...
    # Default: /var/lib/Private/workspace → /data/workspace.
  ...
  in {
    options.services.Private-agent = with lib; {
      enable = mkEnableOption "Private Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = Private-agent;
        description = "The Private-agent package to use.";
      };
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .Private/ subdir (Private_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Private config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $Private_HOME/.env at activation time.
          Private reads this file on every startup via load_Private_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $Private_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $Private_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `Private gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the Private user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the Private plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Private discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          Private_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as Private (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in Private-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the Private CLI to environment.systemPackages and export
          Private_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.Private symlink to the service
            stateDir. These users are automatically added to the Private group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.Private-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the Private CLI to system PATH and export Private_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.Private/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.Private_HOME = "${cfg.stateDir}/.Private";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.Private-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the Private user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.Private-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed Private
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure Private is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.Private        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Private/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Private/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Private/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Private/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.Private/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."Private-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.Private
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.Private ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.Private ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.Private -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.Private/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.Private/$_subdir"
            chmod 2770 "${cfg.stateDir}/.Private/$_subdir"
            find "${cfg.stateDir}/.Private/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # Private-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.Private/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.Private/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Private/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.Private/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.Private/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Private/.managed
          chmod 0644 ${cfg.stateDir}/.Private/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.Private/.container-mode <<'Private_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    Private_bin=${containerDataDir}/current-package/bin/Private
    Private_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Private/.container-mode
            chmod 0644 ${cfg.stateDir}/.Private/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.Private/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.Private";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.Private" ]; then
                  rm -f "${symlinkPath}"
                  echo "Private-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.Private -> stateDir/.Private for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.Private";
                target = "${cfg.stateDir}/.Private";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "Private-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.Private/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.Private/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.Private/auth.json
              fi
            ''}
          ''}
          ...
          # Private reads $Private_HOME/.env at startup via load_Private_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.Private/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'Private_NIX_ENV_EOF'
    ${envFileContent}
    Private_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.Private/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.Private/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.Private/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.Private-agent = {
          description = "Private Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            Private_HOME = "${cfg.stateDir}/.Private";
          ...
          serviceConfig = {
            ...
            # $Private_HOME/.env by the activation script. load_Private_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/Private"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the Private group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.Private-agent = {
          description = "Private Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              Private_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              Private_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env Private_UID="$Private_UID" \
                --env Private_GID="$Private_GID" \
                --env Private_HOME=${containerDataDir}/.Private \
                --env Private_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/Private gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
