...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      PrivateNpmLib = self'.packages.default.passthru.PrivateNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. Private-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Private Agent dev shell"
          ${combinedNonNpm}
          ${PrivateNpmLib.mkNpmDevShellHook npmPackageJsonPaths fixLockfilesExe}
          echo "Ready. Run 'Private' to start."
        '';
      };
    };
}
