...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      Private-agent = self'.packages.default;
      PrivateVenv = Private-agent.PrivateVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "Private-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${PrivateVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "Private-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "Private-build-package" { } ''
          echo "PASS: package built at ${Private-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "Private-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "Private-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${Private-agent}/bin/Private || (echo "FAIL: Private binary missing"; exit 1)
          test -x ${Private-agent}/bin/Private-agent || (echo "FAIL: Private-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${Private-agent}/bin/Private version 2>&1 | grep -qi "Private" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "Private-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in Private Private-agent Private-acp; do
            test -x ${Private-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "Private-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking Private --help ==="
          ${Private-agent}/bin/Private --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${Private-agent}/bin/Private --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "Private-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${Private-agent}/share/Private-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${Private-agent}/share/Private-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "Private_BUNDLED_SKILLS" ${Private-agent}/bin/Private || \
            (echo "FAIL: Private_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: Private_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "Private-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${Private-agent}/share/Private-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${Private-agent}/share/Private-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "Private_BUNDLED_PLUGINS" ${Private-agent}/bin/Private || \
            (echo "FAIL: Private_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: Private_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "Private-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${Private-agent}/share/Private-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${Private-agent}/share/Private-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${Private-agent}/share/Private-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "Private_BUNDLED_LOCALES" ${Private-agent}/bin/Private || \
            (echo "FAIL: Private_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: Private_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (Private_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && Private_BUNDLED_LOCALES=${Private-agent}/share/Private-agent/locales \
            ${PrivateVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with Private_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${PrivateVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${PrivateVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without Private_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "Private-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${Private-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${Private-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "Private_TUI_DIR" ${Private-agent}/bin/Private || \
            (echo "FAIL: Private_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: Private_TUI_DIR set in wrapper"
        ...
        # Verify Private_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        Private-node = pkgs.runCommand "Private-node-version" { } ''
          set -e
          echo "=== Checking Private_NODE in wrapper ==="
          grep -q "Private_NODE" ${Private-agent}/bin/Private || \
            (echo "FAIL: Private_NODE not set in wrapper"; exit 1)
          echo "PASS: Private_NODE present in wrapper"

          Private_NODE=$(sed -n "s/^export Private_NODE='\(.*\)'/\1/p" ${Private-agent}/bin/Private)
          test -x "$Private_NODE" || (echo "FAIL: Private_NODE=$Private_NODE not executable"; exit 1)
          echo "PASS: Private_NODE executable at $Private_NODE"

          NODE_MAJOR=$("$Private_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All Private_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify Private_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "Private-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(Private_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking Private_MANAGED guards ==="
          check_blocked "config set" ${Private-agent}/bin/Private config set model foo
          check_blocked "config edit" ${Private-agent}/bin/Private config edit
        ...
        extra-python-packages = let
          ...
          PrivateWithExtra = Private-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "Private-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${PrivateWithExtra}/bin/Private || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${PrivateWithExtra}/bin/Private || \
          ...
          if grep -q "PYTHONPATH" ${Private-agent}/bin/Private; then
        ...
        extra-dependency-groups = let
          PrivateWithGroups = Private-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "Private-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${PrivateWithGroups}"
          echo "venv: ${PrivateWithGroups.PrivateVenv}"
        ...
        messaging-variant = pkgs.runCommand "Private-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.PrivateVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "Private-config-roundtrip" {
          ...
          merge_and_load() {
            ...
            export Private_HOME="$Private_home"
            ${configMergeScript} ${nixSettings} "$Private_home/config.yaml"
            ${PrivateVenv}/bin/python3 -c '
import json, sys
