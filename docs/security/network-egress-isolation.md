...
When running Private inside Docker, the default `network_mode: host` gives the
...
The Private [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ Private-agent │   │ Private-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   Private_UID=$(id -u) Private_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=Private,Private-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://Private-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Private trust model and vulnerability reporting
