...
Private supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.Private/config.yaml`:
...
Or set the env var: `Private_KANBAN_DISPATCH_IN_GATEWAY=false`
