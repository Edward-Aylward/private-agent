# Private Observer Hooks

Private observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Private' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Private catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Private only builds
