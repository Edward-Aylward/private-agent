...
Private middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Private'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Private calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Private retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Private guardrails,
...
- `telemetry_schema_version`: currently `Private.observer.v1`
- `middleware_schema_version`: currently `Private.middleware.v1`
...
Private stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Private runs
them as a nested chain in registration order. Middleware failures are fail-open:
Private logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Private applies middleware in this order:
...
For each tool call, Private applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Private pre-execution path against the effective arguments:
...
```bash
Private plugins enable <plugin-name>
```

For isolated local testing, use one `Private_HOME` for plugin enablement and the
agent run:

```bash
export Private_HOME=/tmp/Private-middleware-test
mkdir -p "$Private_HOME"
Private plugins enable <plugin-name>
Private chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run Private plugins enable <plugin-name>
uv run Private chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["Private_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/Private-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Private expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Private treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Private preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Private does not convert downstream
