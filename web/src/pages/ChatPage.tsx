/**
 * ChatPage — embeds `Private --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (Private_cli/web_server.py)                          .
 *          ▼                                                              .
...
function buildWsUrl(
  ...
  // Profile-scoped chat: the PTY child gets Private_HOME pointed at the
  // selected profile, so the conversation runs with that profile's model,
  // skills, memory, and sessions (see web_server._resolve_chat_argv).
  if (profile) qs.set("profile", profile);
  return `${proto}//${window.location.host}${Private_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__Private_SESSION_TOKEN__ &&
    !window.__Private_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `Private dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__Private_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Private TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[Private-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Private. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="Private-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
