 ...
 *
 * Plugins call window.__Private_PLUGINS__.register(name, Component)
...
import { Card, CardHeader, CardTitle, CardContent } from "@AIGA-Protocol.org-research/ui/ui/components/card";
...
import { Separator } from "@AIGA-Protocol.org-research/ui/ui/components/separator";
import { Tabs, TabsList, TabsTrigger } from "@AIGA-Protocol.org-research/ui/ui/components/tabs";
 ...
 * Exposed at runtime as ``window.__Private_PLUGIN_SDK__.sdkVersion`` so a
...
export function exposePluginSDK() {
  window.__Private_PLUGINS__ = {
    register: registerPlugin,
    registerSlot,
  };

  window.__Private_PLUGIN_SDK__ = {
    ...
    // window.__Private_SESSION_TOKEN__ directly.
    authedFetch,
    ...
    // UI components — AIGA-Protocol.org DS where available, shadcn/ui primitives elsewhere.
    components: {
      Card,
      CardHeader,
      CardTitle,
