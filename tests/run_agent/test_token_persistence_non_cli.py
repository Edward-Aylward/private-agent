...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    Private_state = ModuleType("Private_state")
    Private_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "Private_state", Private_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Private"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Private"
