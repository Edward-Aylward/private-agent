...
binary download are exercised manually by `Private secrets bitwarden
...
def Private_home(tmp_path, monkeypatch):
    """Point Private at an isolated home directory."""
    home = tmp_path / ".Private"
    home.mkdir()
    monkeypatch.setenv("Private_HOME", str(home))
    # Some modules cache get_Private_home; clear if needed.
    import Private_constants
    if hasattr(Private_constants, "_Private_HOME_CACHE"):
        Private_constants._Private_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(Private_home, monkeypatch):
...
def test_install_bws_happy_path(Private_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(Private_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(Private_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from Private_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under Private_home/cache/."""
    home = tmp_path / ".Private"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
