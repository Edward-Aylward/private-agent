...
class TestPrivateConstantsFallback:
    """Tests for _Private_home.py fallback when Private_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_Private_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _Private_home.py with Private_constants blocked."""
        monkeypatch.setitem(sys.modules, "Private_constants", None)
        spec = importlib.util.spec_from_file_location("_Private_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_Private_home_env_var(self, monkeypatch, tmp_path):
        """When Private_constants is missing, Private_HOME comes from env var."""
        monkeypatch.setenv("Private_HOME", str(tmp_path / "custom-Private"))
        module = self._load_helper(monkeypatch)
        assert module.get_Private_home() == tmp_path / "custom-Private"

    def test_fallback_defaults_to_dot_Private(self, monkeypatch):
        """When Private_constants is missing and Private_HOME unset, default to ~/.Private."""
        monkeypatch.delenv("Private_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_Private_home() == Path.home() / ".Private"

    def test_fallback_ignores_empty_Private_home(self, monkeypatch):
        """Empty/whitespace Private_HOME is treated as unset."""
        monkeypatch.setenv("Private_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_Private_home() == Path.home() / ".Private"

    def test_fallback_display_Private_home_shortens_path(self, monkeypatch):
        """Fallback display_Private_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("Private_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_Private_home() == "~/.Private"

    def test_fallback_display_Private_home_profile_path(self, monkeypatch):
        """Fallback display_Private_home() handles profile paths under ~/."""
        monkeypatch.setenv("Private_HOME", str(Path.home() / ".Private/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_Private_home() == "~/.Private/profiles/coder"

    def test_fallback_display_Private_home_custom_path(self, monkeypatch):
        """Fallback display_Private_home() returns full path for non-home locations."""
        monkeypatch.setenv("Private_HOME", "/opt/Private-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_Private_home() == "/opt/Private-custom"

    def test_delegates_to_Private_constants_when_available(self):
        """When Private_constants IS importable, _Private_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_Private_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import Private_constants
        assert module.get_Private_home is Private_constants.get_Private_home
        assert module.display_Private_home is Private_constants.display_Private_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Private Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "Private-agent[google]" in out
