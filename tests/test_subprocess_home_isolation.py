...
See: https://github.com/AIGA-Protocol.orgResearch/Private-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for Private_constants.get_subprocess_home()."""

    def test_returns_none_when_Private_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        profile_home = Private_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("Private_HOME", str(Private_home))
        from Private_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".Private" / "profiles"
        ...
        monkeypatch.setenv("Private_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("Private_HOME", str(base / "beta"))
    ...
    def test_context_override_is_thread_local(self, tmp_path, monkeypatch):
        ...
        def read_from_other_thread():
            ...
            seen.append(str(get_Private_home()))

        thread = threading.Thread(target=read_from_other_thread)
        thread.start()
        assert ready.wait(timeout=5)

        token = set_Private_home_override(profile)
        try:
            assert get_Private_home() == profile
            release.set()
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(Private_home / "home")
    ...
    def test_no_injection_when_Private_home_unset(self, monkeypatch):
        monkeypatch.delenv("Private_HOME", raising=False)
    ...
    def test_context_override_bridges_to_subprocess_env(self, tmp_path, monkeypatch):
        ...
        assert result["Private_HOME"] == str(profile)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from Private_cli.profiles import _PROFILE_DIRS
