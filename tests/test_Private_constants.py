"""Tests for Private_constants module."""
...
class TestGetDefaultPrivateRoot:
    """Tests for get_default_Private_root() — Docker/custom deployment awareness."""

    def test_no_Private_home_returns_native(self, tmp_path, monkeypatch):
        """When Private_HOME is not set, returns ~/.Private."""
        monkeypatch.delenv("Private_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_Private_root() == tmp_path / ".Private"

    def test_Private_home_is_native(self, tmp_path, monkeypatch):
        """When Private_HOME = ~/.Private, returns ~/.Private."""
        native = tmp_path / ".Private"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Private_HOME", str(native))
        assert get_default_Private_root() == native

    def test_Private_home_is_profile(self, tmp_path, monkeypatch):
        """When Private_HOME is a profile under ~/.Private, returns ~/.Private."""
        native = tmp_path / ".Private"
    ...
    def test_Private_home_is_docker(self, tmp_path, monkeypatch):
        """When Private_HOME points outside ~/.Private (Docker), returns Private_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Private_HOME", str(docker_home))
        assert get_default_Private_root() == docker_home

    def test_Private_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any Private_HOME outside ~/.Private is treated as the root."""
        custom = tmp_path / "my-Private-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Private_HOME", str(custom))
        assert get_default_Private_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (Private_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("Private_HOME", str(profile))
        assert get_default_Private_root() == docker_root

    def test_no_Private_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\Private, not ~/.Private."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("Private_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(Private_constants.sys, "platform", "win32")

        assert get_default_Private_root() == local_appdata / "Private"

    def test_no_Private_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/Private without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("Private_HOME", raising=False)
        ...
        assert get_default_Private_root() == home / "AppData" / "Local" / "Private"


class TestGetPrivateHome:
    """Tests for get_Private_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When Private_HOME is unset on Windows, use %LOCALAPPDATA%\\Private."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("Private_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(Private_constants.sys, "platform", "win32")
        monkeypatch.setattr(Private_constants, "_profile_fallback_warned", False)

        assert get_Private_home() == local_appdata / "Private"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(Private_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(Private_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".Private"
