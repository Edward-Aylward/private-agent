"""Tests for the top-level `./Private` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./Private` should use `Private_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "Private"
    called = []

    fake_main_module = types.ModuleType("Private_cli.main")

    def fake_main():
        called.append("Private_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "Private_cli.main", fake_main_module)
    ...
    assert called == ["Private_cli.main"]
