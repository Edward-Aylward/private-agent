"""Tests for `Private memory setup [provider]` routing.
...
``Private memory setup honcho``) without the interactive picker — which matters
because the per-provider ``Private <provider>`` subcommand is only registered
...
from Private_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "Private memory setup" in out
