...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from Private_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("Private_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_Private_ca_bundle_env_falls_back(self, monkeypatch):
        from Private_cli.auth import _resolve_verify

        monkeypatch.setenv("Private_CA_BUNDLE", "/nonexistent/Private-ca.pem")
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    re-authenticate" string led users to report this as a Private persistence
...
def shared_store_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("Private_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without Private_SHARED_AUTH_DIR override, the seat belt must trip.
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "Private-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different Private_HOME) sees the same shared state and can rehydrate
