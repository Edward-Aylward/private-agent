"""Tests for the Private plugin system (Private_cli.plugins)."""
...
from Private_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<Private_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<Private_home>/plugins/``; we derive
    ``<Private_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <Private_HOME>/config.yaml.
        # Config is always read from Private_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        Private_home_str = os.environ.get("Private_HOME")
        if Private_home_str:
            Private_home = Path(Private_home_str)
        else:
            Private_home = base.parent
        Private_home.mkdir(parents=True, exist_ok=True)
        cfg_path = Private_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.Private/plugins/ are discovered."""
        plugins_dir = tmp_path / "Private_test" / "plugins"
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".Private" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        Private_home = tmp_path / "Private_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under Private_plugins.<name>."""
        plugins_dir = tmp_path / "Private_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("Private_HOME", str(tmp_path / "Private_test"))

        # Clean up any prior namespace module
        sys.modules.pop("Private_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "Private_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="Private_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import Private_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("Private_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / "Private_test"
        plugins_dir = Private_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "Private_test" / "plugins"
        # Set Private_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("Private_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("Private_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("Private_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("Private_cli.plugins.PluginContext.dispatch_tool.__module__", "Private_cli.plugins"):
...
class TestPluginDebugLogging:
    """Private_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("Private_PLUGINS_DEBUG", raising=False)
        from Private_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With Private_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("Private_PLUGINS_DEBUG", "1")
        from Private_cli import plugins as plugins_mod
