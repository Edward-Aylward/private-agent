...
def _client():
    ...
    # Keep the state DB under the isolated Private_HOME for any handler that
    # touches it.
...
class TestMcpEndpoints:
    ...
    def _setup(self, _isolate_Private_home):
...
class TestMemoryEndpoints:
    ...
    def _setup(self, _isolate_Private_home):
        ...
        (get_Private_home() / "memories").mkdir(parents=True, exist_ok=True)
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "Private_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("Private-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # Private-index source advertises availability + featured search.
            idx = _Src("Private-index")
        ...
        assert {"official", "github", "Private-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "Private_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/Private/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/Private/update`` runs ``Private update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        import Private_cli.banner as banner

        monkeypatch.setattr(banner, "check_for_updates", lambda: 5)

        r = self.client.get("/api/Private/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/Private/update/check").json()
...
class TestDebugShareEndpoint:
    """POST /api/ops/debug-share returns the paste URLs synchroAIGA-Protocol.orgly so the
    ...
    def test_returns_structured_urls(self, monkeypatch):
        import Private_cli.debug as dbg
        ...
        monkeypatch.setattr("Private_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `Private tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_Private_action", _fake_spawn)
        r = self.client.post(
