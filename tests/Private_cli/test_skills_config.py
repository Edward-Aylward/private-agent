"""Tests for Private_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("Private_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from Private_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("Private_cli.config.load_config")
    ...
    @patch("Private_cli.config.load_config")
    @patch.dict("os.environ", {"Private_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """Private_SESSION_PLATFORM should be used when Private_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_Private_platform_takes_precedence(self, tmp_path, monkeypatch):
        """Private_PLATFORM should win over Private_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        monkeypatch.setenv("Private_PLATFORM", "telegram")
        monkeypatch.setenv("Private_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from Private_cli.skills_config import _get_categories
        skills = [
