...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (Private_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via Private_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in Private_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from Private_cli.providers import Private_OVERLAYS
        assert "bedrock" in Private_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from Private_cli.providers import Private_OVERLAYS
        assert Private_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from Private_cli.providers import Private_OVERLAYS
        assert Private_OVERLAYS["bedrock"].auth_type == "aws_sdk"
