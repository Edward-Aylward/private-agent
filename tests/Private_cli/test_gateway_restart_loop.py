...
Covers:
- Defense 1: gateway stop/restart refuse when _Private_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Private  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "Private GATEWAY RESTART",           # uppercase
    ])
    def test_Private_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.Private.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.Private.gateway.plist",
        "launchctl stop ai.Private.gateway",
        "systemctl restart Private-gateway",
        "systemctl stop Private-gateway.service",
        "systemctl start Private-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill Private gateway process",
        "pkill -f Private.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "Private cron list",
        "Private update",
        "Private config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_Private_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade Private then run Private gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.Private.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify Private gateway stop/restart refuse when _Private_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_Private_GATEWAY", raising=False)
