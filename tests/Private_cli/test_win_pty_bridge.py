"""Unit tests for Private_cli.win_pty_bridge — ConPTY spawning + byte forwarding.

Windows-only counterpart to tests/Private_cli/test_pty_bridge.py.  Drives
...
``Private_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
from Private_cli.win_pty_bridge import PtyUnavailableError, WinPtyBridge
...
class TestWinPtyBridgeUnavailable:
    ...
    def test_bridge_class_is_importable(self):
        ...
        #     from Private_cli.win_pty_bridge import WinPtyBridge, PtyUnavailableError
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo Private-ok"])
        try:
            output = _read_until(bridge, b"Private-ok")
            assert b"Private-ok" in output
        finally:
            bridge.close()
...
class TestClampDimension:
    ...
    def test_clamps_above_max(self):
        from Private_cli.win_pty_bridge import _MAX_COLS, _MAX_ROWS, _clamp

        assert _clamp(131072, _MAX_COLS) == _MAX_COLS
        assert _clamp(131072, _MAX_ROWS) == _MAX_ROWS

    def test_floors_at_one(self):
        from Private_cli.win_pty_bridge import _MAX_COLS, _clamp
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('Private_PTY_TEST=' + os.environ.get('Private_PTY_TEST',''))",
            ],
            env={**os.environ, "Private_PTY_TEST": "pty-env-works"},
        )
        try:
