"""Tests for Private_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import Private_cli.gui_uninstall as gu


def _make_agent(Private_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = Private_home / "Private-agent"
    (agent_root / "Private_cli").mkdir(parents=True)
    (agent_root / "Private_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(Private_home: Path) -> None:
    """Create the source-built GUI artifacts a `Private desktop` run produces."""
    desktop = Private_home / "Private-agent" / "apps" / "desktop"
    ...
    (Private_home / "Private-agent" / "node_modules").mkdir(parents=True)
    (Private_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(Private_home: Path) -> None:
    (Private_home / "config.yaml").write_text("x: 1\n")
    (Private_home / ".env").write_text("KEY=secret\n")
...
def test_agent_is_installed_detects_source_and_venv(tmp_path):
    ...
    Private_home.mkdir()
    assert gu.agent_is_installed(Private_home) is False
    _make_agent(Private_home)
    assert gu.agent_is_installed(Private_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    Private_home = tmp_path / ".Private"
    (Private_home / "Private-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(Private_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    Private_home = tmp_path / ".Private"
    _make_gui_build(Private_home)
    artifacts = gu.source_built_gui_artifacts(Private_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(Private_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(Private_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    Private_home = tmp_path / ".Private"
    agent_root = _make_agent(Private_home)
    _make_gui_build(Private_home)
    _make_user_data(Private_home)
    ...
    removed = gu.uninstall_gui(Private_home)
    ...
    assert not (Private_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "Private_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (Private_home / "config.yaml").exists()
    assert (Private_home / ".env").exists()
    assert (Private_home / "sessions").exists()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(Private_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(Private_home)
    ...
    assert summary["Private_home"] == str(Private_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Private"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Private"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Private"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Private"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    Private_HOME with all the destructive externals stubbed out.
    """
    import Private_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_Private_home", lambda: Private_home)
    ...
    from Private_cli import gui_uninstall as gu_mod
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole Private_HOME non-interactively."""
    import Private_cli.uninstall as uninstall

    Private_home = tmp_path / ".Private"
    (Private_home / "Private-agent" / "Private_cli").mkdir(parents=True)
    (Private_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m Private_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_Private_home", lambda: Private_home)
    ...
    assert not (desktop / "release").exists()
    assert not (Private_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "Private_cli").exists()
    assert (Private_home / "config.yaml").exists()
