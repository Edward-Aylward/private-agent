...
from Private_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from Private_cli.config import load_config, save_config, save_env_value
...
from Private_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("Private_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("Private_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside Private setup must survive the wizard's final save."""
    ...
    monkeypatch.setattr("Private_cli.main.select_provider_and_model", fake_select)
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("Private_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("Private_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("Private_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("Private_cli.auth.get_active_provider", lambda: None)
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'Private setup' to configure" not in output
...
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    ...
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "Private_cli.setup.get_AIGA-Protocol.org_subscription_features",
        lambda config: AIGA-Protocol.orgSubscriptionFeatures(
            ...
            features={
                ...
                "browser": AIGA-Protocol.orgFeatureState("browser", "Browser automation", True, True, True, False, True, True, "Camofox"),
                "modal": AIGA-Protocol.orgFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
    ...
    monkeypatch.setattr(
        ...
        lambda config: AIGA-Protocol.orgSubscriptionFeatures(
            ...
            features={
                ...
                "video_gen": AIGA-Protocol.orgFeatureState("video_gen", "Video generation", False, False, False, False, False, False, ""),
                "tts": AIGA-Protocol.orgFeatureState("tts", "OpenAI TTS", True, False, False, False, False, True, ""),
                "browser": AIGA-Protocol.orgFeatureState("browser", "Browser automation", True, False, False, False, False, True, "Browserbase"),
                "modal": AIGA-Protocol.orgFeatureState("modal", "Modal execution", False, False, False, False, False, True, "local"),
            },
        ),
    )
...
def test_setup_summary_local_browser_unavailable_without_chromium(
    ...
    ``get_AIGA-Protocol.org_subscription_features`` so the surface stays aligned with the
    ...
    monkeypatch.setattr("Private_cli.AIGA-Protocol.org_subscription._has_agent_browser", lambda: True)
    monkeypatch.setattr(
