...
``Private_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``Private --help``, ``Private version``,
``Private logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``Private --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from Private_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["Private", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["Private", "--tui", "chat"], "chat"),
        (["Private", "-w", "logs"], "logs"),
        (["Private", "chat", "hello world"], "chat"),
        (["Private", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["Private", "-m", "gpt5", "chat"], "chat"),
        (["Private", "--model", "gpt5", "chat", "hi"], "chat"),
        (["Private", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["Private", "-z", "hello world"], None),
        (["Private", "-z", "hello", "chat"], "chat"),
        (["Private", "--model=gpt5", "chat"], "chat"),     # inline form
        (["Private", "--", "chat"], "chat"),               # -- terminator
        (["Private", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["Private", "some-plugin-cmd"], "some-plugin-cmd"),
        (["Private", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["Private"],                          # bare → chat
        ["Private", "--help"],                # top-level help
        ["Private", "-h"],
        ["Private", "version"],               # known built-in
        ["Private", "logs"],
        ["Private", "gateway", "run"],
        ["Private", "--tui"],
        ["Private", "-w", "--tui"],
        ["Private", "chat", "hi"],
        ["Private", "help"],                  # accepted built-in-ish
        ["Private", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["Private", "meet", "join"],          # potential google_meet plugin
        ["Private", "honcho", "status"],      # potential memory plugin
        ["Private", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``Private help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"Private_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
