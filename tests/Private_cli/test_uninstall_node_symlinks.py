"""Tests for Private_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $Private_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Private-managed node dir.
"""
...
def _make_Private_node(Private_home: Path) -> Path:
    """Create a fake $Private_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = Private_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_Private_node(fake_home):
    Private_home = fake_home / ".Private"
    node_bin = _make_Private_node(Private_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(Private_home)
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    Private_home = fake_home / ".Private"
    _make_Private_node(Private_home)
    local_bin = fake_home / ".local" / "bin"
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(Private_home) == []


def test_removes_dangling_symlink_into_Private_node(fake_home):
    """A link into the Private node dir is removed even if the target file is
...
def test_only_some_links_present(fake_home):
    """Removes the Private links that exist; ignores the ones that don't."""
    Private_home = fake_home / ".Private"
    node_bin = _make_Private_node(Private_home)
    local_bin = fake_home / ".local" / "bin"

    # Only npm and npx are Private-managed; node is a real user binary.
