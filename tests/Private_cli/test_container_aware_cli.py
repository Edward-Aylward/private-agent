...
def container_env(tmp_path, monkeypatch):
    """Set up a fake Private_HOME with .container-mode file."""
    Private_home = tmp_path / ".Private"
    Private_home.mkdir()
    monkeypatch.setenv("Private_HOME", str(Private_home))
    monkeypatch.delenv("Private_DEV", raising=False)

    container_mode = Private_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=Private-agent\n"
        "exec_user=Private\n"
        "Private_bin=/data/current-package/bin/Private\n"
    )
    return Private_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("Private_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "Private-agent"
    assert info["exec_user"] == "Private"
    assert info["Private_bin"] == "/data/current-package/bin/Private"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("Private_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_Private_dev(container_env, monkeypatch):
    """Returns None when Private_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("Private_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_Private_dev_zero(container_env, monkeypatch):
    """Private_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("Private_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        Private_home = Path(tmpdir) / ".Private"
        Private_home.mkdir()
        (Private_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("Private_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_Private_home": lambda: Private_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("Private_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=Private-custom\n"
        "exec_user=myuser\n"
        "Private_bin=/opt/Private/bin/Private\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "Private-custom"
    assert info["exec_user"] == "myuser"
    assert info["Private_bin"] == "/opt/Private/bin/Private"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from Private_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "Private"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "Private-agent" in cmd
    assert "/data/current-package/bin/Private" in cmd
    assert "chat" in cmd
