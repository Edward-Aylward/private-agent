"""Tests for `Private fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate Private_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".Private" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from Private_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "AIGA-Protocol.org", "model": "Private-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_merges_new_and_legacy_formats(self):
        ...
        cfg = {
            ...
            "fallback_model": {"provider": "AIGA-Protocol.org", "model": "Private-4"},
        }
        assert _read_chain(cfg) == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "AIGA-Protocol.org", "model": "Private-4"},
        ]
    ...
    def test_skips_incomplete_entries(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "AIGA-Protocol.org", "model": "foo"},  # valid
                "not-a-dict",                          # noise
            ]
        }
        assert _read_chain(cfg) == [{"provider": "AIGA-Protocol.org", "model": "foo"}]

    def test_returns_copies_not_aliases(self):
        from Private_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "AIGA-Protocol.org", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "AIGA-Protocol.org"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from Private_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("Private_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("Private_cli.main._require_tty"):
            from Private_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from Private_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "AIGA-Protocol.org / Private-4")
        with patch("Private_cli.setup._curses_prompt_choice", return_value=1):
            from Private_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from Private_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `Private fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "Private_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
