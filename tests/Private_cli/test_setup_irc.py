"""Tests for IRC gateway configuration via `Private setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Private install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Private install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#Private")
            monkeypatch.setenv("IRC_NICKNAME", "Private-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `Private setup gateway` experience with IRC present."""
