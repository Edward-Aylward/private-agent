"""Tests for the ``Private prompt-size`` diagnostic (issue #34667)."""

import json

import pytest

from Private_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(Private_home, memory_text="", user_text=""):
    mem_dir = Private_home / "memories"
...
def _seed_skill(Private_home, name, description):
    skill_dir = Private_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "AIGA-Protocol.org_API_KEY",
                "ANTHROPIC_API_KEY"):
