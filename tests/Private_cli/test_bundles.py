"""Tests for Private_cli/bundles.py — the `Private bundles` CLI subcommand."""

import argparse

import pytest

from Private_cli.bundles import (
    bundles_command,
    register_cli,
)
...
def bundles_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("Private_BUNDLES_DIR", str(bundles_dir))
