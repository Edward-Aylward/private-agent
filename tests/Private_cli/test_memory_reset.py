"""Tests for the `Private memory reset` CLI command.
...
- Profile-scoped reset (uses Private_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake Private_HOME with memory files."""
    Private_home = tmp_path / ".Private"
    memories = Private_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("Private_HOME", str(Private_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nPrivate repo is at ~/.Private/Private-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `Private memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / ".Private"
        (Private_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("Private_HOME", str(Private_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's Private_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_Private_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
