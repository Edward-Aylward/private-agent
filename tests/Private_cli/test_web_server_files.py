...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("Private_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("Private_DASHBOARD_FILES_ROOT", raising=False)
...
def test_hosted_policy_locks_to_opt_data(monkeypatch):
    monkeypatch.delenv("Private_DASHBOARD_FILES_ROOT", raising=False)
    monkeypatch.setenv("Private_HOME", "/opt/data")
    client, prev_auth_required, prev_bound_host = _client_with_app_state()
    try:
