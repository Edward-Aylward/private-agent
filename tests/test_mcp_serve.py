"""
Tests for mcp_serve — Private MCP server.
...
def _isolate_Private_home(tmp_path, monkeypatch):
    """Redirect Private_HOME to a temp directory."""
    monkeypatch.setenv("Private_HOME", str(tmp_path))
    try:
        import Private_constants
        monkeypatch.setattr(Private_constants, "get_Private_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking Private_state schema."""
