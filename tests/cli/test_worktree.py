...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("Private/Private-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned Private/* and pr-* branches."""

    def test_prunes_orphaned_Private_branch(self, git_repo):
        """Private/Private-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "Private/Private-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "Private/Private-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "Private/Private-deadbeef" in result.stdout
        ...
        assert "Private/Private-deadbeef" in orphaned
        ...
        assert "Private/Private-deadbeef" not in result.stdout
