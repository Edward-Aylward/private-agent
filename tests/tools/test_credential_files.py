...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        Private_home.mkdir()
        (Private_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(Private_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.Private/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        (Private_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        (Private_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        (Private_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        Private_home = tmp_path / ".Private"
        skills_dir = Private_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.Private/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        Private_home = tmp_path / ".Private"
        (Private_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"Private_HOME": str(Private_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.Private")

        assert mounts[0]["container_path"] == "/home/user/.Private/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.Private/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.Private/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape Private_HOME."""
        monkeypatch.setenv("Private_HOME", str(tmp_path / ".Private"))
        (tmp_path / ".Private").mkdir()

        # Create a sensitive file one level above Private_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside Private_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside Private_HOME must still be registered."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        monkeypatch.setenv("Private_HOME", str(Private_home))
        (Private_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_Private_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of Private_HOME must be allowed."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        subdir = Private_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside Private_HOME pointing outside must be rejected."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        monkeypatch.setenv("Private_HOME", str(Private_home))

        # Create a sensitive file outside Private_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside Private_home pointing outside
        symlink = Private_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes Private_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, Private_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape Private_HOME."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        monkeypatch.setenv("Private_HOME", str(Private_home))
        ...
        sensitive.write_text("{}")
        self._write_config(Private_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(Private_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside Private_HOME via config must still mount."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        monkeypatch.setenv("Private_HOME", str(Private_home))

        (Private_home / "oauth.json").write_text("{}")
        self._write_config(Private_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        (Private_home / "cache" / "documents").mkdir(parents=True)
        (Private_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("Private_HOME", str(Private_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.Private/cache/documents" in paths
        assert "/root/.Private/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.Private/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        # Use legacy dir name — get_Private_dir prefers old if it exists
        (Private_home / "document_cache").mkdir()
        (Private_home / "image_cache").mkdir()
        monkeypatch.setenv("Private_HOME", str(Private_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(Private_home / "document_cache") in host_paths
        assert str(Private_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.Private/cache/documents" in container_paths
        assert "/root/.Private/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        Private_home = tmp_path / ".Private"
        img_dir = Private_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.Private/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.Private")
            == "/home/agent/.Private/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        Private_home = tmp_path / ".Private"
        (Private_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("Private_HOME", str(Private_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(Private_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / ".Private"
        doc_dir = Private_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / ".Private"
        ss_dir = Private_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.Private/cache/screenshots/session_abc/screen1.png"
