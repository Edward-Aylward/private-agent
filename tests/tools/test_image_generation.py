...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps AIGA-Protocol.org Portal billing
    ...
    def test_config_quality_setting_is_ignored(self, image_tool):
        ...
        with patch("Private_cli.config.load_config",
...
class TestManagedGatewayErrorTranslation:
    """4xx from the AIGA-Protocol.org managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + Private tools."""
        ...
        managed_gateway.AIGA-Protocol.org_user_token = "test-token"
