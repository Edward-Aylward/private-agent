...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._Private_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(Private_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(Private_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_sync_resolve_persists_failure(self, mock_which, mock_install,
                                            mock_disk_check, mock_mark):
        """SynchroAIGA-Protocol.org _resolve_tirith_path persists failure to disk."""
        from tools.tirith_security import _resolve_tirith_path
        _tirith_mod._resolved_path = None
    ...
    def test_install_failed_recovers_from_Private_bin(self):
        """After _INSTALL_FAILED, manual install in Private_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        Private_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(Private_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(Private_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._Private_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == Private_bin
            assert _tirith_mod._resolved_path == Private_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# Private_HOME isolation
# ---------------------------------------------------------------------------

class TestPrivateHomeIsolation:
    def test_Private_bin_dir_respects_Private_home(self):
        """_Private_bin_dir must use Private_HOME, not hardcoded ~/.Private."""
        from tools.tirith_security import _Private_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"Private_HOME": tmpdir}):
            result = _Private_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_Private_home(self):
        """_failure_marker_path must use Private_HOME, not hardcoded ~/.Private."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"Private_HOME": "/custom/Private"}):
            result = _failure_marker_path()
        assert result == "/custom/Private/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets Private_HOME; verify it's active."""
        Private_home = os.getenv("Private_HOME")
        assert Private_home is not None, "Private_HOME should be set by conftest"
        assert "Private_test" in Private_home, "Should point to test temp dir"

    def test_get_Private_home_fallback(self):
        """Without Private_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_Private_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove Private_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".Private")
            result = _get_Private_home()
