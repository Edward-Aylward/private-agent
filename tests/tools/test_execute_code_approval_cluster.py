...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad Private_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: Private_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("Private_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("Private_CRON_SESSION", "1")
    monkeypatch.delenv("Private_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_Private_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "Private_HOME": "/h", "Private_PROFILE": "p",
        "Private_CONFIG": "/c.yaml", "Private_ENV": "/e",
        # other Private_* → dropped (broad prefix removed)
        "Private_BASE_URL": "https://x", "Private_INTERACTIVE": "1",
        "Private_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("Private_HOME", "Private_PROFILE", "Private_CONFIG", "Private_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "Private_BASE_URL", "Private_INTERACTIVE", "Private_KANBAN_DB",
...
def test_env_scrub_logs_dropped_Private_vars(caplog):
    """Dropping a non-allowlisted, non-secret Private_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset Private_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "Private_HOME": "/h",          # allowlisted → kept, not logged
        "Private_BASE_URL": "https://x",   # dropped → logged
        "Private_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "Private_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "Private_HOME" in out and "PATH" in out
    assert "Private_BASE_URL" not in out and "Private_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "Private_BASE_URL" in msgs and "Private_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "Private_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped Private_* vars."""
    import logging
