...
and the shared slash-command handlers (Private_cli/write_approval_commands.py).
...
@pytest.fixture
def Private_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="Private_wa_test_")
    home = os.path.join(d, ".Private")
    os.makedirs(home)
    monkeypatch.setenv("Private_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import Private_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(Private_home):
...
def test_invalid_subsystem_is_off(Private_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(Private_home):
...
def test_memory_gate_on_no_interactive_stages(Private_home):
...
def test_memory_gate_on_then_apply(Private_home):
...
def test_skill_gate_off_allows_create(Private_home):
...
def test_skill_gate_on_always_stages(Private_home):
...
def test_skill_gate_on_then_apply_writes_file(Private_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's Private_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(Private_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(Private_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(Private_home):
...
def test_handle_approve_all(Private_home):
    from Private_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(Private_home):
    from Private_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(Private_home):
    from Private_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(Private_home):
    from Private_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(Private_home):
...
def test_handle_approval_invalid(Private_home):
    from Private_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(Private_home):
    from Private_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(Private_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(Private_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(Private_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(Private_home, monkeypatch):
    ...
    from tools import write_approval as wa
    _set_approval("memory", True)
    monkeypatch.setenv("Private_GATEWAY_SESSION", "1")
...
def test_skills_never_prompt_inline_even_with_callback(Private_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(Private_home):
