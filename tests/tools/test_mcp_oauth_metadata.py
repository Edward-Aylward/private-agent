...
Covers:
- :class:`PrivateTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.PrivateMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _Private_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# PrivateTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = PrivateTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        storage = PrivateTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (PrivateMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: PrivateTokenStorage, **context_attrs):
    ...
    provider = _Private_PROVIDER_CLS.__new__(_Private_PROVIDER_CLS)
    provider._Private_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        storage = PrivateTokenStorage("mgr-srv")
        ...
        with patch.object(
            _Private_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        storage = PrivateTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        storage = PrivateTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        storage = PrivateTokenStorage("noop-srv")
        ...
        with patch.object(
            PrivateTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("Private_HOME", str(tmp_path))
        storage = PrivateTokenStorage("flow-srv")
        ...
        with patch.object(
            _Private_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
