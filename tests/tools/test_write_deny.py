...
class TestWriteDenyExactPaths:
    ...
    def test_Private_env(self):
        # ``.env`` under the active Private_HOME (profile-aware, not just
        # ``~/.Private``) must be write-denied. The hermetic test conftest
        # points Private_HOME at a tempdir — resolve via get_Private_home()
        # to match the denylist.
        from Private_constants import get_Private_home
        path = str(get_Private_home() / ".env")
        assert _is_write_denied(path) is True

    def test_Private_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.Private/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: Private_HOME does point to the profile dir, not the root.
        from Private_constants import get_Private_home, get_default_Private_root
        assert get_Private_home() == profile_home
        assert get_default_Private_root() == root
...
class TestWriteAllowed:
    ...
    def test_Private_config_not_env(self):
        path = os.path.join(str(Path.home()), ".Private", "config.yaml")
