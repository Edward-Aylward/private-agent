...
When Private_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the Private user's home directory ($Private_HOME == /opt/data).
Runtime-writable trees under ``/opt/Private`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/Private/gateway`` is one such
