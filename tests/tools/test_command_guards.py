...
def _clean_state():
    ...
    saved = {}
    for k in ("Private_INTERACTIVE", "Private_GATEWAY_SESSION", "Private_EXEC_ASK", "Private_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["Private_INTERACTIVE"] = "1"
        session_key = os.getenv("Private_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No Private_INTERACTIVE or Private_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
