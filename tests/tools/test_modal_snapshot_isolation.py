...
def _restore_tool_modules():
    original_Private_home = os.environ.get("Private_HOME")
    original_modules = {
        name: module
        ...
        or name == "Private_cli"
        or name.startswith("Private_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_Private_home is None:
            os.environ.pop("Private_HOME", None)
        else:
            os.environ["Private_HOME"] = original_Private_home
        _reset_modules(("tools", "Private_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    Private_cli = types.ModuleType("Private_cli")
    Private_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["Private_cli"] = Private_cli
    Private_home = tmp_path / "Private-home"
    os.environ["Private_HOME"] = str(Private_home)
    sys.modules["Private_cli.config"] = types.SimpleNamespace(
        get_Private_home=lambda: Private_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="Private-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": Private_home / "modal_snapshots.json",
