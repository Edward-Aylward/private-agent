...
def test_Private_provider_subclass_exists():
    """PrivateMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _Private_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _Private_PROVIDER_CLS is not None
    assert issubclass(_Private_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_Private_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns PrivateMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _Private_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("Private_HOME", str(tmp_path))
    ...
    assert _Private_PROVIDER_CLS is not None
    assert isinstance(provider, _Private_PROVIDER_CLS)
    assert provider._Private_server_name == "srv"
