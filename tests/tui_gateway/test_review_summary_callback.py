...
def server():
    with patch.dict(
        "sys.modules",
        {
            "Private_constants": MagicMock(
                get_Private_home=MagicMock(return_value="/tmp/Private_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'Private-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'Private-release' patched"
    }
