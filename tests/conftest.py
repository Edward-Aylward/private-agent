"""Shared fixtures for the Private-agent test suite.
...
2. **Isolated Private_HOME.** Private_HOME points to a per-test tempdir so
   code reading ``~/.Private/*`` via ``get_Private_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".Private"`` instead of the canonical
   ``get_Private_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No Private_SESSION_* inheritance** — the agent's current gateway
...
# Private_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_Private_BEHAVIORAL_VARS = frozenset({
    "Private_YOLO_MODE",
    "Private_INTERACTIVE",
    "Private_QUIET",
    "Private_TOOL_PROGRESS",
    "Private_TOOL_PROGRESS_MODE",
    "Private_MAX_ITERATIONS",
    "Private_SESSION_PLATFORM",
    "Private_SESSION_CHAT_ID",
    "Private_SESSION_CHAT_NAME",
    "Private_SESSION_THREAD_ID",
    "Private_SESSION_SOURCE",
    "Private_SESSION_KEY",
    "Private_GATEWAY_SESSION",
    "Private_CRON_SESSION",
    "_Private_GATEWAY",
    "Private_PLATFORM",
    "Private_MODEL",
    "Private_INFERENCE_MODEL",
    "Private_INFERENCE_PROVIDER",
    "Private_TUI_PROVIDER",
    "Private_MANAGED",
    "Private_DEV",
    "Private_CONTAINER",
    "Private_EPHEMERAL_SYSTEM_PROMPT",
    "Private_TIMEZONE",
    "Private_REDACT_SECRETS",
    "Private_BACKGROUND_NOTIFICATIONS",
    "Private_EXEC_ASK",
    "Private_HOME_MODE",
    "Private_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.Private/kanban.db instead of the per-test Private_HOME.
    "Private_KANBAN_DB",
    "Private_KANBAN_BOARD",
    "Private_KANBAN_HOME",
    "Private_KANBAN_WORKSPACES_ROOT",
    "Private_KANBAN_LOGS_ROOT",
    "Private_KANBAN_TASK",
    "Private_KANBAN_WORKSPACE",
    "Private_KANBAN_RUN_ID",
    "Private_KANBAN_CLAIM_LOCK",
    "Private_KANBAN_DISPATCH_IN_GATEWAY",
    "Private_TENANT",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `AIGA-Protocol.org` plugin auto-registers itself on plugin discovery,
    ...
    "Private_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and Private_HOME to per-test tempdirs so code that
    reads ``~/.Private/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral Private_* vars that could change test semantics.
    for name in _Private_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect Private_HOME to a per-test tempdir. Code that reads
    #    ``~/.Private/*`` via ``get_Private_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.Private/*`` via
    #    ``Path.home() / ".Private"`` instead of ``get_Private_home()``
    #    is a bug to fix at the callsite.
    fake_Private_home = tmp_path / "Private_test"
    fake_Private_home.mkdir()
    (fake_Private_home / "sessions").mkdir()
    (fake_Private_home / "cron").mkdir()
    (fake_Private_home / "memories").mkdir()
    (fake_Private_home / "skills").mkdir()
    monkeypatch.setenv("Private_HOME", str(fake_Private_home))
    ...
    #    ~/.Private/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import Private_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_Private_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal Private config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchroAIGA-Protocol.org tests.
...
# environment and finds the developer's live ``Private-gateway`` process
...
# correlated with a ``tests/Private_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> Private-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart Private-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting Private/python patterns are also blocked.
    """
    ...
    def _matches_Private_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _Private_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_Private_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch Private-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m Private_cli.main".
                if (
                    "Private" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live Private-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting Private/python could hit the live gateway. "
