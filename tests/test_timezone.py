"""
Tests for timezone support (Private_time module + integration points).
...
def _reset_Private_time_cache():
    """Reset the Private_time module cache (replacement for removed reset_cache)."""
    Private_time._cached_tz = None
    Private_time._cached_tz_name = None
    Private_time._cache_resolved = False


# =========================================================================
# Private_time.now() — core helper
# =========================================================================

class TestPrivateTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_Private_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["Private_TIMEZONE"] = "Asia/Kolkata"
        result = Private_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["Private_TIMEZONE"] = "UTC"
        result = Private_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["Private_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="Private_time"):
            result = Private_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["Private_TIMEZONE"] = "Europe/London"
        tz = Private_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["Private_TIMEZONE"] = "Not/A/Timezone"
        tz = Private_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When Private_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) Private_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("Private_TIMEZONE=" + os.environ.get("Private_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "Private_TIMEZONE=NOT_SET" in result["output"], (
            "Private_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When Private_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("Private_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=Private_tz) which shifted
        absolute time when system-local tz != Private tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_Private_tz(self):
        """Already-aware datetimes should be normalized to Private tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Private tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Private tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Private is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Private tz
        ...
        # Use a Private timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _Private_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["Private_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_Private_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Private timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["Private_TIMEZONE"] = "US/Eastern"
        _reset_Private_time_cache()
