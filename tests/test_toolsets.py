...
class TestToolsetConsistency:
    ...
    def test_Private_platforms_share_core_tools(self):
        """All Private-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on Private-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["Private-cli", "Private-telegram", "Private-discord", "Private-whatsapp", "Private-slack", "Private-signal", "Private-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_Private_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("Private-whatsapp")

    def test_Private_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("Private-api-server")
