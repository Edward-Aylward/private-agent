...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``Private_cli``) for the package
    itself and ``Private_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``Private_cli`` without ``Private_cli.*``,
    so the wheel shipped ``Private_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'Private_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    Private_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (Private_cli/plugins.py) registers each bundled plugin by
...
def test_optional_mcps_manifests_ship_in_both_wheel_and_sdist():
    ...
    Private_cli/mcp_catalog.py resolves the catalog via get_optional_mcps_dir()
    ...
    `Private mcp catalog` and the dashboard catalog screen come up empty on
