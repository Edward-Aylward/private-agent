...
Run from the PR worktree:
    ...
    cd ~/.Private/Private-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.Private/Private-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.Private/Private-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a Private-agent checkout"
)
...
# Isolated Private_HOME for the config write.
