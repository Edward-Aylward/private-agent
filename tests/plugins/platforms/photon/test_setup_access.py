"""Tests for `Private photon setup`'s access auto-configuration.

`_autoconfigure_access` allowlists the operator and points the cron home
channel at their DM, writing to the per-test ~/.Private/.env (the hermetic
Private_HOME fixture isolates this). It must fill only unset keys so a re-run
