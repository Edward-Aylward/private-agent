"""Harness: dashboard opt-in via Private_DASHBOARD.

Today (tini): dashboard starts once when Private_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``Private`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without Private_DASHBOARD, no dashboard process should be running."""
    subprocess.run(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "Private dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without Private_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without Private_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `Private doctor` and any other health check).
    ...
    Private_DASHBOARD is unset, so the slot reflects reality.
    """
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name, built_image,
         "sleep", "60"],
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without Private_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with Private_DASHBOARD=1, s6-svstat reports the slot as up."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         ...
         # Private_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
         ...
         "-e", "Private_DASHBOARD_INSECURE=1",
...
def test_dashboard_opt_in_starts(
    ...
    """With Private_DASHBOARD=1, a dashboard process should be visible."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "Private_DASHBOARD=1",
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'Private dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with Private_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """Private_DASHBOARD_PORT changes the dashboard's listen port."""
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "Private_DASHBOARD=1", "-e", "Private_DASHBOARD_PORT=9120",
         # Default bind is 0.0.0.0; pin insecure opt-in so the auth gate
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "Private dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as Private, so the
    # Private user can kill it (same UID).
...
# whenever `Private_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/Private/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
    end = time.monotonic() + deadline_s
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    subprocess.run(
        ["docker", "run", "-d", "--name", container_name,
         "-e", "Private_DASHBOARD=1",
         "-e", "Private_DASHBOARD_HOST=0.0.0.0",
         "-e", "Private_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
         built_image, "sleep", "120"],
        check=True, capture_output=True, timeout=30,
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and Private_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_opts_out_of_gate(
    ...
    """``Private_DASHBOARD_INSECURE=1`` re-enables the legacy no-gate mode
    ...
    assert status.get("auth_required") is False, (
        "Private_DASHBOARD_INSECURE=1 must disable the auth gate (explicit "
