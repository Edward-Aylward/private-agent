...
Override the image with ``Private_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("Private_TEST_IMAGE", "Private-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``Private_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("Private_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged Private user
# ---------------------------------------------------------------------------
#
# Background: every Private runtime path inside the container drops to UID
# 10000 (the ``Private`` user) via ``s6-setuidgid Private``. ``docker exec``
...
# fails (PermissionError swallowed) for Private, so a test that ran as root
...
# helpers below run every probe under ``-u Private`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: Private).
