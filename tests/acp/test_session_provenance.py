...
def test_root_session_no_compression(db):
    ...
    assert prov["currentPrivateSessionId"] == "root1"
    assert prov["rootPrivateSessionId"] == "root1"
    assert prov["parentPrivateSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_Private_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentPrivateSessionId"] == "old"
    assert prov["rootPrivateSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousPrivateSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootPrivateSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootPrivateSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_Private_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousPrivateSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"Private"}
    assert "sessionProvenance" in meta["Private"]
    assert meta["Private"]["sessionProvenance"]["currentPrivateSessionId"] == "root1"
