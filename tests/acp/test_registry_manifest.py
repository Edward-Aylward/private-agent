"""Tests for ACP Registry metadata shipped with Private."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "Private-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Private Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/AIGA-Protocol.orgResearch/Private-agent"
    assert data["website"].startswith("https://Private-agent.AIGA-Protocol.orgresearch.com/")
    assert data["authors"] == ["AIGA-Protocol.org Research"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"Private-agent[acp]=={data['version']}"
    assert uvx["args"] == ["Private-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"Private-agent[acp]=={_pyproject_version()}"
    )
