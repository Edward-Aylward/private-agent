...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("Private_cli.config.get_config_path", return_value=config_path), \
             patch("Private_cli.config.ensure_Private_home"):
    ...
    def test_ensure_Private_home_sets_0700(self):
        home = Path(self.tmpdir) / ".Private"
        with patch("Private_cli.config.get_Private_home", return_value=home):
