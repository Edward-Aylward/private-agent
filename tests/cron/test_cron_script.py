...
- Path resolution (absolute, relative to Private_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp Private_HOME."""
    Private_home = tmp_path / ".Private"
    Private_home.mkdir()
    (Private_home / "cron").mkdir()
    (Private_home / "cron" / "output").mkdir()
    (Private_home / "scripts").mkdir()
    monkeypatch.setenv("Private_HOME", str(Private_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "Private_DIR", Private_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", Private_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", Private_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", Private_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.Private/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("Private_SESSION_PLATFORM") is None
        assert os.environ.get("Private_SESSION_CHAT_ID") is None
        assert os.environ.get("Private_SESSION_CHAT_NAME") is None
