...
from Private_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(Private_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <Private_home>/plugins/<name> and opt it in."""
    plugin_dir = Private_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("Private_HOME", str(tmp_path / "Private_empty"))
