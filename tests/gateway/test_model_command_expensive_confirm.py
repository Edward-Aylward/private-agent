...
def _fake_warning():
    return SimpleNamespace(
        message=(
            ...
            "openai/gpt-5.5-pro has known pricing above Private' safety threshold.\n"
...
def _setup_isolated_home(tmp_path, monkeypatch, *, warn):
    ...
    monkeypatch.setattr(
        "Private_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    monkeypatch.setattr("Private_constants.get_Private_home", lambda: Private_home)
    monkeypatch.setattr("Private_cli.config.get_Private_home", lambda: Private_home)
    monkeypatch.setattr(
