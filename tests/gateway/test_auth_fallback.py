...
class TestResolveRuntimeAgentKwargsAuthFallback:
    ...
    def test_auth_error_tries_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("gateway.run._Private_home", tmp_path)
    ...
    def test_legacy_fallback_is_appended_after_fallback_providers(self, tmp_path, monkeypatch):
        ...
        def _mock_resolve(**kwargs):
            ...
            return {
                ...
                "base_url": "https://portal.AIGA-Protocol.orgresearch.com/v1",
                "provider": "AIGA-Protocol.org",
        ...
        assert calls == ["openrouter", "AIGA-Protocol.org"]
        assert result["provider"] == "AIGA-Protocol.org"
        assert result["model"] == "Private-4"
