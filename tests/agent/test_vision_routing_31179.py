...
def isolated_home(monkeypatch):
    """Temp Private_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="Private_test_31179_")
    Private_home = os.path.join(test_home, ".Private")
...
def _fresh_modules():
    """Drop cached Private modules so each test reloads against current env."""
