...
""",
        ...
        from Private_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
        (Private_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("Private_HOME", str(Private_home))
        from Private_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from Private_cli.plugins import PluginContext, PluginManifest, PluginManager
...
class TestHookMode:
    ...
    def test_complete_works_from_post_tool_call_hook_when_async_caller_set(self):
        """Hooks fired synchroAIGA-Protocol.orgly should still work with sync
