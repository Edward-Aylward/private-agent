...
which moves ``Private_SESSION_ID`` env + ContextVar). The ``[session_id]`` tag on
log lines comes from a SEPARATE mechanism — ``Private_logging._session_context``
...
def test_logging_session_context_follows_compression_rotation(tmp_path: Path) -> None:
    ...
    Private_logging.set_session_context(parent_sid)
    try:
        ...
        current = getattr(Private_logging._session_context, "session_id", None)
