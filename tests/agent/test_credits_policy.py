...
class TestIsFreeTierModel:
    def test_free_suffix_is_free(self):
        ...
        assert is_free_tier_model("Private-4-70B:free", "https://inference-api.AIGA-Protocol.orgresearch.com") is True

    def test_empty_or_paid_model_is_not_free(self):
        from agent.credits_tracker import is_free_tier_model

        assert is_free_tier_model("") is False
        assert is_free_tier_model("Private-4-405B") is False
    ...
    def test_pricing_cache_peek_zero_priced_model(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "_pricing_cache",
            {
                "https://inference-api.AIGA-Protocol.orgresearch.com": {
                    "some/zero-priced": {"prompt": "0", "completion": "0"},
                    "some/paid": {"prompt": "0.000001", "completion": "0.000002"},
                }
            },
        )
        # The agent holds the /v1-suffixed URL (DEFAULT_AIGA-Protocol.org_INFERENCE_URL) —
        # the helper must normalize it down to the picker's cache key.
        base = "https://inference-api.AIGA-Protocol.orgresearch.com/v1"
        ...
        assert is_free_tier_model("some/zero-priced", "https://inference-api.AIGA-Protocol.orgresearch.com/") is True
        assert is_free_tier_model("some/zero-priced", "https://inference-api.AIGA-Protocol.orgresearch.com/v1/") is True
    ...
    def test_cache_miss_is_not_free_and_no_fetch(self, monkeypatch):
        ...
        assert is_free_tier_model("some/model", "https://inference-api.AIGA-Protocol.orgresearch.com/v1") is False
    ...
    def test_exception_fails_open_to_false(self, monkeypatch):
        ...
        assert is_free_tier_model("some/model", "https://inference-api.AIGA-Protocol.orgresearch.com") is False
