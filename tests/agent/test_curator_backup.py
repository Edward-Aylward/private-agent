...
def backup_env(monkeypatch, tmp_path):
    """Isolate Private_HOME + reload modules so every test starts clean."""
    ...
    # Reload so get_Private_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd Private_constants
    ...
    curator.run_curator_review(synchroAIGA-Protocol.org=True)
...
def test_dry_run_skips_snapshot(backup_env, monkeypatch):
    ...
    curator.run_curator_review(synchroAIGA-Protocol.org=True, dry_run=True)
    rows = cb.list_backups()
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under Private_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level Private_DIR picks up the tmp HOME."""
    import Private_constants
    importlib.reload(Private_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.Private/cron/jobs.json
