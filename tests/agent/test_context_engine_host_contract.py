...
engine plugins (e.g. Private-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``Private plugins`` picker."""
    from Private_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "Private_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "Private_cli.plugins.get_plugin_context_engine",
