...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Private passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("Private_HOME", "/users/alice/.Private/profiles/backend-worker")
        monkeypatch.setenv("Private_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "Private_KANBAN_DB",
            "/users/alice/.Private/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.Private/kanban/boards/smoke"]'
            in cmd
        )
