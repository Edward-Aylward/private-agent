...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_Private_env(self):
        """Private' own .env inside Private_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: Private internal .env is in ~/.Private/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.Private/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.Private/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Private cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        Private_home = tmp_path / ".Private"
        cache = Private_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._Private_home_path", return_value=Private_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Private cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        Private_home = tmp_path / ".Private"
        hub = Private_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_Private_home(self, tmp_path):
        """The env basename guard does not depend on Private_HOME resolution."""
        Private_home = tmp_path / ".Private"
        Private_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        Private_home = tmp_path / ".Private"
        cache = Private_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
