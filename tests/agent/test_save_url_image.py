...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate Private_HOME under tmp_path."""
    monkeypatch.setenv("Private_HOME", str(tmp_path / ".Private"))
    (tmp_path / ".Private").mkdir()

    # Force the constants/image cache helpers to re-read Private_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("Private_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_Private_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under Private_HOME — gateway cleanup
