"""Tests for Private_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Private entry point
(Private, Private-agent, Private-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Private entry point imports Private_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported Private_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("Private_bootstrap", None)
    import Private_bootstrap  # noqa: WPS433
    return Private_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Private CLI
...
class TestEntryPointsImportBootstrap:
    """Every Private entry point must import Private_bootstrap as its
    ...
    # Entry points that invoke Private as a process.  Each one must
    # import Private_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "Private_cli/main.py",   # Private CLI (console_script)
        "run_agent.py",          # Private-agent (console_script)
        "acp_adapter/entry.py",  # Private-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import Private_bootstrap' and that
        ...
        half-finished ``Private update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``Private_bootstrap`` as a top-level module) leaves Private
        ...
        """
        # Resolve relative to the Private-agent repo root.  Tests live
        # at tests/test_Private_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # Private start even when Private_bootstrap hasn't been
        ...
        assert first_import_name == "Private_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'Private_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import Private_bootstrap' line to be the first import."
        )
