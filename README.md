...
```bash
Private              # Interactive CLI — start a conversation
...
Private gateway      # Start the messaging gateway (Telegram, Discord, etc.)
Private setup        # Run the full setup wizard (configures everything at once)
Private claw migrate # Migrate from OpenClaw (if coming from OpenClaw)
Private update       # Update to the latest version
...
Private has two entry points: start the terminal UI with `Private`, or run the gateway and talk to it from Telegram, Discord, Slack, WhatsApp, Signal, or Email. Once you're in a conversation, many slash commands are shared across both interfaces.

| Action                         | CLI                                           | Messaging platforms                                                              |
| ------------------------------ | --------------------------------------------- | -------------------------------------------------------------------------------- |
| Start chatting                 | `Private`                                      | Run `Private gateway setup` + `Private gateway start`, then send the bot a message |
...
If you're coming from OpenClaw, Private can automatically import your settings, memories, skills, and API keys.

**During first-time setup:** The setup wizard (`Private setup`) automatically detects `~/.openclaw` and offers to migrate before configuration begins.

**Anytime after install:**

```bash
Private claw migrate              # Interactive migration (full preset)
...
- **Skills** — user-created skills → `~/.Private/skills/openclaw-imports/`
...
See `Private claw migrate --help` for all options, or use the `openclaw-migration` skill for an interactive agent-guided migration with dry-run previews.
