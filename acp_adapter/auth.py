"""ACP auth helpers — detect and advertise Private authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "Private-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Private runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Private can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Private.
    ...
    not have Private provider credentials configured yet, so Private always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Private using the currently configured "
