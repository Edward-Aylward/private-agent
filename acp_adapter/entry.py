"""CLI entry point for the Private-agent ACP adapter.

Loads environment variables from ``~/.Private/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from Private_HOME (default ``~/.Private``)."""
    from Private_cli.env_loader import load_Private_dotenv

    Private_home = get_Private_home()
    loaded = load_Private_dotenv(Private_home=Private_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", Private_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="Private-acp",
        description="Run Private Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Private version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Private provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.Private/node/ "
...
def _print_version() -> None:
    ...
    print(Private_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import PrivateACPAgent  # noqa: F401

    print("Private ACP check OK")


def _run_setup() -> None:
    from Private_cli.main import main as Private_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "Private", "model"]
        Private_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``Private postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting Private-agent ACP adapter")
    ...
    import acp
    from .server import PrivateACPAgent
