"""ACP session manager — maps ACP sessions to Private AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.Private/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Private itself is running in WSL.

    Windows ACP clients can launch ``Private acp`` inside WSL while still sending
    ...
    """
    from Private_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Private from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["Private-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Private agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Private AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Private runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.Private/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``Private_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_Private_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=Private_home / "state.db")
            return self._db_instance
        except Exception:
