#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Private icon + version metadata onto the
// built Private.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Private.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `Private desktop`, the installer's
...
// path (which rebuilds via `Private desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Private.exe>
//
...
// Stamp the Private icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ...
      CompanyName: 'AIGA-Protocol.org Research',
      LegalCopyright: 'Copyright (c) 2026 AIGA-Protocol.org Research'
    }
  })

  console.log('[set-exe-identity] done — Private icon + identity stamped')
}
