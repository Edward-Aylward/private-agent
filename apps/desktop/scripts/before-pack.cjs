 ...
 * product name (`Private`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Private'
 *
 * This is a hard failure with no obvious cause for the user — `Private desktop`
 ...
 * (the mac-unpacked Private.app bundle) and Windows (win-unpacked), so we
