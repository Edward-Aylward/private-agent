...
cd apps/desktop
XCURSOR_SIZE=24 Private_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
...
node apps/desktop/scripts/profile-typing.mjs \
  --chars=400 --cps=30 --out=/tmp/Private-typing
# → /tmp/Private-typing.cpuprofile  (open in Chrome DevTools Performance)
# → /tmp/Private-typing.before.heapsnapshot
# → /tmp/Private-typing.after.heapsnapshot
```
...
1. **Per-keystroke `scrollHeight` read removed.** The expansion useEffect
   ...
   (forces synchroAIGA-Protocol.org layout). Replaced with a `draft.length > 60`
