...
// no longer bundles the Private Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Private.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Private'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Private.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Private'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default Private_HOME for non-sandboxed runs -- matches main.cjs's
// resolvePrivateHome(). On Windows it's %LOCALAPPDATA%\Private; elsewhere
// it's ~/.Private. The fresh-install sandbox launchFresh() sets its own
// Private_HOME and never touches this.
const DEFAULT_Private_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'Private')
  }
  return path.join(os.homedir(), '.Private')
})()
const VENV_ROOT = path.join(DEFAULT_Private_HOME, 'Private-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'Private-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.Private_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Private-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Private-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Private-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Private-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.Private_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.Private_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const PrivateHome = path.join(sandbox, 'Private-home')
  ...
  env.Private_DESKTOP_CWD = cwd
  env.Private_DESKTOP_IGNORE_EXISTING = '1'
  env.Private_DESKTOP_TEST_MODE = 'fresh-install'
  env.Private_DESKTOP_USER_DATA_DIR = userDataDir
  env.Private_HOME = PrivateHome
  delete env.Private_DESKTOP_Private
  delete env.Private_DESKTOP_Private_ROOT
  ...
  console.log(`  Private_HOME: ${PrivateHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(PrivateHome, 'Private-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Private Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of Private_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'Private-agent', 'Private_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Private
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + Private_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  Private_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
