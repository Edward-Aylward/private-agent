...
const USER_DATA_OVERRIDE = process.env.Private_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.Private_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with Private_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[Private] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[Private] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[Private] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[Private] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// Private_HOME — the user-facing root for everything Private-related. Mirrors
// scripts/install.ps1's $PrivateHome and scripts/install.sh's $Private_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\Private (matches install.ps1)
//   macOS / Linux: ~/.Private (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.Private directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\Private yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// Private_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// Private_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.Private / %LOCALAPPDATA%\Private.
function resolvePrivateHome() {
  if (process.env.Private_HOME) return path.resolve(process.env.Private_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'Private-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'Private')
    const legacy = path.join(app.getPath('home'), '.Private')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.Private setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.Private')
}

const Private_HOME = resolvePrivateHome()
// ACTIVE_Private_ROOT — the canonical mutable Private install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_Private_ROOT = path.join(Private_HOME, 'Private-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_Private_ROOT, 'venv')
...
// boots, going straight to `resolvePrivateBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_Private_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_Private_ROOT, '.Private-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Private profile the desktop launches its
// local backend as. When set, startPrivate() passes `Private --profile <name>
// dashboard …`, which deterministically pins Private_HOME (see
// _apply_profile_override in Private_cli/main.py) and bypasses the sticky
// ~/.Private/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors Private_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// PrivateDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under Private_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by Private_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(Private_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (Private_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.Private_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.Private_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Private'
...
// Seed the native About panel with the live Private version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `Private update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'Private-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(Private-media://…)`.
...
let mainWindow = null
let PrivateProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by PrivateProcess +
// connectionPromise + startPrivate(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.Private_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.Private_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startPrivate() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Private backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[Private] ${line}`)
  PrivateLog.push(...lines)
  if (PrivateLog.length > 300) {
    PrivateLog.splice(0, PrivateLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-Private-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('Private:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the Private:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Private:bootstrap:event', ev)
}
...
function isPrivateSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'Private_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.Private_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Private deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Private' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\Private\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'Private', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'Private', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\Private\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'Private', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'Private', 'git', 'bin', 'git.exe'))
  }
...
function recentPrivateLog() {
  return PrivateLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's Private root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_Private_ROOT.
// Private_DESKTOP_Private_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.Private_DESKTOP_Private_ROOT && path.resolve(process.env.Private_DESKTOP_Private_ROOT),
    !IS_PACKAGED && isPrivateSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isPrivateSourceRoot(ACTIVE_Private_ROOT) ? ACTIVE_Private_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_Private_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('Private:updates:progress', merged)
  }
}
...
// Private_HOME/Private-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_Private_home). That binary owns
// ALL repo mutation — running `Private update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'Private-setup.exe' : 'Private-setup'
  const candidate = path.join(Private_HOME, name)
...
// Path to the venv shim whose lock decides whether `Private update` can write
// fresh entry points. On Windows this is the file the running backend
// `Private.exe` holds open; on POSIX it's never mandatory-locked.
function venvPrivateShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'Private.exe')
    : path.join(updateRoot, 'venv', 'bin', 'Private')
}
...
// `PrivateProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `Private.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a Private
// REPL, a pty terminal, the gateway) keep `Private.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (PrivateProcess && Number.isInteger(PrivateProcess.pid)) pids.push(PrivateProcess.pid)
  ...
  if (PrivateProcess && !PrivateProcess.killed) {
    try {
      PrivateProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvPrivateShimPath(updateRoot)
...
// `Private update`). Instead we spawn the staged Private-Setup binary with
// --update and quit, so it can run `Private update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri Private-setup. Unlike Windows
      ...
      // whole update itself: `Private update` (backend) + `Private desktop
    ...
    if (!updater) {
      ...
      // `Private desktop`, never the Tauri installer that self-copies
      // Private-setup.exe into Private_HOME). They DO have a working `Private`
      ...
      // checkout they're on — bare `Private update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'Private update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `Private update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `Private update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, PrivateRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Private updater…', percent: 100 })
    ...
    // Private.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `Private update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: Private_HOME,
      env: {
        ...process.env,
        Private_HOME,
        PATH: [path.join(Private_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the Private CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `Private` on PATH.
function resolvePrivateCliBinary(updateRoot) {
  const venvPrivate = path.join(updateRoot, 'venv', 'bin', 'Private')
  if (fileExists(venvPrivate)) return venvPrivate
  return findOnPath('Private') || null
}
...
// macOS/Linux in-app update: backend (`Private update`) + OS-aware GUI rebuild
// (`Private desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const Private = resolvePrivateCliBinary(updateRoot)
  if (!Private) {
    emitUpdateProgress({ stage: 'manual', message: 'Private update', percent: null })
    return { ok: true, manual: true, command: 'Private update', PrivateRoot: updateRoot }
  }

  // Put the Private-managed Node and the venv on PATH so `Private desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(Private_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    Private_HOME,
  ...
  // `Private update` reaps stale `Private dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads Private_DESKTOP_CHILD_PID and excludes
  ...
  if (PrivateProcess && Number.isInteger(PrivateProcess.pid)) {
    desktopChildPids.push(PrivateProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.Private_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Private (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(Private, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'Private update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'Private update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(Private, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Private to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Private.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Private.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Private to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.Private-update-new"; then
    rm -rf "$DST.Private-update-old" 2>/dev/null || true
    mv "$DST" "$DST.Private-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.Private-update-new" "$DST"
    rm -rf "$DST.Private-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `Private-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Private to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `Private
  ...
  return isPrivateSourceRoot(ACTIVE_Private_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.Private_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set Private_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: Private desktop --force-build`
  )
  return candidates[0]
}
...
function resolvePrivateCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Private.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.Private_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolvePrivateCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'Private_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_Private_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Private at ${ACTIVE_Private_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'Private_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_Private_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_Private_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolvePrivateBackend(dashboardArgs) {
  // 1. Explicit override -- Private_DESKTOP_Private_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.Private_DESKTOP_Private_ROOT && path.resolve(process.env.Private_DESKTOP_Private_ROOT)
  if (overrideRoot && isPrivateSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Private source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `Private` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isPrivateSourceRoot.)
  if (!IS_PACKAGED && isPrivateSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Private source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_Private_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\Private\Private-agent (Windows) or ~/.Private/Private-agent.
  ...
  //    to spawning Private. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `Private update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `Private` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    Private_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.Private_DESKTOP_IGNORE_EXISTING !== '1') {
    let PrivateCommand = null
    const PrivateOverride = process.env.Private_DESKTOP_Private

    if (PrivateOverride) {
      const resolvedOverride = findOnPath(PrivateOverride)
      if (resolvedOverride) {
        PrivateCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(PrivateOverride, { isWsl: IS_WSL })) {
        PrivateCommand = PrivateOverride
      } else {
        rememberLog(`Ignoring Windows Private override under WSL: ${PrivateOverride}`)
      }
    } else {
      PrivateCommand = findOnPath('Private')
    }

    if (PrivateCommand) {
      if (looksLikeDesktopAppBinary(PrivateCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Private CLI: ${PrivateCommand}`)
        PrivateCommand = null
      }
    }

    if (PrivateCommand) {
      // Smoke-test the candidate before trusting it. A `Private` shim
      ...
      const shellForProbe = isCommandScript(PrivateCommand)
      if (verifyPrivateCli(PrivateCommand, { shell: shellForProbe })) {
        return {
          label: `existing Private CLI at ${PrivateCommand}`,
      ...
      rememberLog(
        `Ignoring existing Private CLI at ${PrivateCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed Private_cli module via system Python.
  ...
  if (python) {
    ...
    // having Private_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\Private\Private-agent\venv.
    if (canImportPrivateCli(python)) {
      return {
        kind: 'python',
        label: `installed Private_cli module via ${python}`,
        command: python,
        args: ['-m', 'Private_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: Private_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolvePrivateBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Private Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_Private_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolvePrivateBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Private install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      logRoot: path.join(Private_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Private install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Private bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(Private_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startPrivate() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // Private:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolvePrivateBackend(backend.args))
  }
  ...
  if (!isPrivateSourceRoot(ACTIVE_Private_ROOT)) {
    throw new Error(
      `Private install at ${ACTIVE_Private_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Private' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\Private\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `Private` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Private on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Private.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isPrivateSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Private venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Private at ${ACTIVE_Private_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Private runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Private backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Private backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Private backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Private-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('Private:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolvePrivateCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Private:preview-file-changed', payload)
}
...
async function waitForPrivate(baseUrl, token) {
  ...
  throw new Error(`Private backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Private:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Private:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Private...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Private:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Private:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('Private:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'Private:desktop:zoomLevel'
...
const OAUTH_SESSION_PARTITION = 'persist:Private-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Private gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Private backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root Private_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.Private_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.Private_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by Private_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (Private #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Private gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Private gateway is selected, but no session token is saved. ' +
...
//   2. env override (Private_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.Private_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.Private_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'Private_DESKTOP_REMOTE_URL is set but Private_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Private backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.Private_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Private
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startPrivate())
  ...
  // connect to Private gateway". Mirror the renderer's connect here so the test
...
function resetPrivateConnection() {
...
// startPrivate() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetPrivateConnection() nulls PrivateProcess.
  const dying = PrivateProcess && !PrivateProcess.killed ? PrivateProcess : null
  resetPrivateConnection()
...
// profile to startPrivate() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startPrivate() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForPrivate(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: PrivateLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited Private_HOME env (see _apply_profile_override
  // step 3 in Private_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  const backend = await ensureRuntime(resolvePrivateBackend(dashboardArgs))
  const PrivateCwd = resolvePrivateCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Private backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(backend.command, backend.args, hiddenWindowsChildOptions({
    ...
    env: {
      ...
      TERMINAL_CWD: PrivateCwd,
      Private_DASHBOARD_SESSION_TOKEN: token,
      // Marks this dashboard backend as desktop-spawned so it runs the cron
      // scheduler tick loop (the gateway isn't running under the app).
      Private_DESKTOP: '1',
      Private_WEB_DIST: webDist
    },
    shell: backend.shell,
    stdio: ['ignore', 'pipe', 'pipe']
  }))
  ...
  child.once('error', error => {
    rememberLog(`Private backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Private backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Private backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForPrivate(baseUrl, token), startFailed])
  ready = true
...
async function startPrivate() {
  ...
  // process, every subsequent startPrivate() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Private backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Private backend at ${remote.baseUrl}`, 24)
      await waitForPrivate(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Private backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.Private/active_profile file) and
    // resolves Private_HOME the same way `Private -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Private runtime', 28)
    const backend = await ensureRuntime(resolvePrivateBackend(dashboardArgs))
    const PrivateCwd = resolvePrivateCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Private backend via ${backend.label}`, 84)
    rememberLog(`Starting Private backend via ${backend.label}`)

    PrivateProcess = spawn(backend.command, backend.args, hiddenWindowsChildOptions({
      cwd: PrivateCwd,
      env: {
        ...process.env,
        // Explicitly pin Private_HOME for the child so Python's get_Private_home()
        // resolves to the SAME location our resolvePrivateHome() picked. Without
        // this pin, Python falls back to ~/.Private on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\Private, which differs from C:\Users\<u>\.Private.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets Private_HOME via setx; the desktop
    ...
    PrivateProcess.stdout.on('data', rememberLog)
    PrivateProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    PrivateProcess.once('error', error => {
      rememberLog(`Private backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Private backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    PrivateProcess.once('exit', (code, signal) => {
      rememberLog(`Private backend exited (${signal || code})`)
      PrivateProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Private backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Private backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentPrivateLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Private backend to become ready', 90)
    await Promise.race([waitForPrivate(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Private backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startPrivate().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('Private:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Private…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('Private:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetPrivateConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Private backend failed liveness probe; dropping stale connection.')
    resetPrivateConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('Private:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('Private:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('Private:window:openSession', async (_event, sessionId) => {
...
ipcMain.handle('Private:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startPrivate() call restarts the
...
ipcMain.handle('Private:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startPrivate() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('Private:bootstrap:cancel', async () => {
...
ipcMain.handle('Private:boot-progress:get', async () => bootProgressState)
ipcMain.handle('Private:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('Private:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('Private:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('Private:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('Private:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('Private:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('Private:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('Private:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('Private:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('Private:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new Private_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('Private:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('Private:requestMicrophoneAccess', async () => {
...
ipcMain.handle('Private:api', async (_event, request) => {
...
ipcMain.handle('Private:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'Private',
    body: payload?.body || '',
...
ipcMain.handle('Private:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('Private:readFileText', async (_event, filePath) => {
...
ipcMain.handle('Private:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('Private:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('Private:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('Private:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('Private:saveClipboardImage', async () => {
...
ipcMain.handle('Private:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('Private:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('Private:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('Private:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('Private:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolvePrivateCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('Private:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolvePrivateCwd()
}))

ipcMain.handle('Private:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('Private:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('Private:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('Private:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('Private:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('Private:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: PrivateLog.slice(-200) }))
...
function terminalShellCommand() {
  // Private_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.Private_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Private' TUI into
  ...
  env.TERM_PROGRAM = 'Private'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a Private/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from Private_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.Private_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `Private:terminal:${id}:${suffix}`
}
...
ipcMain.handle('Private:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('Private:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('Private:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Private.')
  }
...
ipcMain.handle('Private:terminal:write', (_event, id, data) => {
...
ipcMain.handle('Private:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('Private:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('Private:updates:check', async () =>
...
ipcMain.handle('Private:updates:apply', async (_event, payload) =>
...
ipcMain.handle('Private:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('Private:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Private version (the one `release.py` bumps in
// Private_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Private version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolvePrivateVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'Private_cli', '__init__.py')
...
// Re-resolve the live Private version and push it into the native About panel
// just before showing it, so an in-place `Private update` is reflected without
...
ipcMain.handle('Private:version', async () => ({
  appVersion: resolvePrivateVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  PrivateRoot: resolveUpdateRoot()
}))
...
// the actual removal via `Private uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (Private_cli/uninstall.py + Private_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_Private_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isPrivateSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'Private_cli.main', 'uninstall', '--gui-summary'], hiddenWindowsChildOptions({
        cwd: agentRoot,
        env: { ...process.env, Private_HOME, NO_COLOR: '1' },
        stdio: ['ignore', 'pipe', 'ignore']
      }))
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Private agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import Private_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_Private_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_Private_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `Private-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `Private-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('Private:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('Private:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('Private:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('Private:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// Private:// deep links (e.g. Private://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const Private_PROTOCOL = 'Private'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find((a) => typeof a === 'string' && a.startsWith(`${Private_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // Private://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('Private:deep-link', payload)
...
ipcMain.handle('Private:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${Private_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(Private_PROTOCOL, process.execPath, [
        path.resolve(process.argv[1]),
      ])
    } else {
      app.setAsDefaultProtocolClient(Private_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `Private://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching Private:// URL is in our own argv.
