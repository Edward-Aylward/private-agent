 ...
 *
 * Drives apps/desktop's first-launch install of Private Agent by spawning
...
function bootstrapCacheDir(PrivateHome) {
  return path.join(PrivateHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.Private/Private-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(PrivateHome) {
  if (!PrivateHome) return null
  const candidate = path.join(PrivateHome, 'Private-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(PrivateHome, commit) {
  return path.join(bootstrapCacheDir(PrivateHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/AIGA-Protocol.orgResearch/Private-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, PrivateHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(PrivateHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(PrivateHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, PrivateHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass Private_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        Private_HOME: PrivateHome || process.env.Private_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, PrivateHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, PrivateHome }) {
  const args = ['--dir', activeRoot, '--Private-home', PrivateHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, PrivateHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, PrivateHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, PrivateHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, PrivateHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(PrivateHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, PrivateHome, emit })
