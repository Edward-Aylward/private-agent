 ...
 *     `X-Private-Session-Token` header; WS uses `?token=`.
...
// Private_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - Private_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - Private_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / Private #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-Private_session_at', '__Secure-Private_session_at', 'Private_session_at']
const RT_COOKIE_VARIANTS = ['__Host-Private_session_rt', '__Secure-Private_session_rt', 'Private_session_rt']
...
/**
 * True if any cookie in `cookies` is a Private session ACCESS-token cookie
