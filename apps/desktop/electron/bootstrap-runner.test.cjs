...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'Private-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/Private-runner-test',
    sourceRepoRoot: null,
    PrivateHome: '/tmp/Private-runner-test',
    logRoot: '/tmp/Private-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'Private-agent', 'scripts')
