...
import { getPrivateConfigRecord, type PrivateConfigRecord, savePrivateConfig } from '@/Private'
...
export interface I18nConfigClient {
  getConfig: () => Promise<PrivateConfigRecord>
  saveConfig: (config: PrivateConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.PrivateDesktop?.api) {
      return Promise.resolve({})
    }

    return getPrivateConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.PrivateDesktop?.api) {
    ...
    return savePrivateConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: PrivateConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: PrivateConfigRecord, locale: Locale): PrivateConfigRecord {
