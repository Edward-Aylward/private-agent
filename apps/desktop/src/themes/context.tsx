...
import { BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, AIGA-Protocol.orgTheme } from './presets'
...
const SKIN_KEY = 'Private-desktop-theme-v2'
const MODE_KEY = 'Private-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'Private-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'Private-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'Private-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['AIGA-Protocol.org-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? AIGA-Protocol.orgTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...AIGA-Protocol.orgTheme.typography, ...theme.typography }
  ...
  root.dataset.PrivateTheme = skinName
  root.dataset.PrivateMode = rendered
  ...
  window.PrivateDesktop?.setTitleBarTheme?.({
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.PrivateThemeFont = 'true'
...
/** Sync the desktop skin with the active Private backend theme on connect. */
