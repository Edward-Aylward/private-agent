...
import { getPrivateConfigDefaults, getPrivateConfigRecord, savePrivateConfig } from '@/Private'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getPrivateConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'Private-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await savePrivateConfig(await getPrivateConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
