...
import {
  getElevenLabsVoices,
  getPrivateConfigDefaults,
  getPrivateConfigRecord,
  getPrivateConfigSchema,
  savePrivateConfig
} from '@/Private'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, PrivateConfigRecord } from '@/types/Private'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<PrivateConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<PrivateConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getPrivateConfigRecord(), getPrivateConfigDefaults(), getPrivateConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await savePrivateConfig(config)
  ...
  const updateConfig = (next: PrivateConfigRecord) => {
