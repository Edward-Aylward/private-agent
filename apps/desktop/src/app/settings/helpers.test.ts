...
import type { PrivateConfigRecord } from '@/types/Private'
...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: PrivateConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: PrivateConfigRecord = {}
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('AIGA-Protocol.org_API_KEY')).toBe('AIGA-Protocol.org Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // Private_QWEN_ and Private_GEMINI_ both share the Private_ stem.
      expect(providerGroup('Private_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('Private_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
