...
import type { PrivateReadDirEntry, PrivateReadDirResult } from '@/global'

export type ProjectTreeEntry = PrivateReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: PrivateReadDirEntry) {
...
async function filterIgnored(entries: PrivateReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<PrivateReadDirResult> {
  if (!window.PrivateDesktop) {
