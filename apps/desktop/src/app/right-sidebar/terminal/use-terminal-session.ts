...
const Private_PATHS_MIME = 'application/x-Private-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(Private_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(Private_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.PrivateDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      fontSize: 11,
      lineHeight: 1.12,
      // Full-screen TUIs (Private --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[Private-terminal] WebGL unavailable; falling back to DOM', err)
      }
