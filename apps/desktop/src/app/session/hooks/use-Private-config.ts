...
import { getPrivateConfig, getPrivateConfigDefaults } from '@/Private'
...
interface PrivateConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function usePrivateConfig({ activeSessionIdRef, refreshProjectBranch }: PrivateConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshPrivateConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getPrivateConfig(), getPrivateConfigDefaults().catch(() => ({}))])
  ...
  return { refreshPrivateConfig, sttEnabled, voiceMaxRecordingSeconds }
}
