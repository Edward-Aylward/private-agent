...
import { getProfiles, transcribeAudio } from '@/Private'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.PrivateDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `Private gateway` process performs the actual transfer, so we
