        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.PrivateDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Private to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.PrivateDesktop?.watchPreviewFile ||
      !window.PrivateDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.PrivateDesktop.onPreviewFileChanged(payload => {
    ...
    void window.PrivateDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.PrivateDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.PrivateDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:Private-preview')
