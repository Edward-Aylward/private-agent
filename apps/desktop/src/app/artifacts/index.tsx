...
import { getSessionMessages, listSessions } from '@/Private'
...
import type { SessionInfo, SessionMessage } from '@/types/Private'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.PrivateDesktop?.openExternal) {
        await window.PrivateDesktop.openExternal(href)
      } else {
