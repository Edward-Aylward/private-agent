...
// bug that drives the REAL useGatewayBoot hook + REAL PrivateGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshPrivateConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { PrivateDesktop?: unknown }).PrivateDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { PrivateDesktop?: unknown }).PrivateDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForPrivate) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startPrivate()'s remote branch awaits waitForPrivate() for 45s before it
    ...
    ;(window as { PrivateDesktop?: unknown }).PrivateDesktop = desktop
    ...
    // After ~45s waitForPrivate gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Private backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
