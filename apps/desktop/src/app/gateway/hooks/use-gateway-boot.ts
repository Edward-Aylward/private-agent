...
import { PrivateGateway } from '@/Private'
...
import type { RpcEvent } from '@/types/Private'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.PrivateDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: PrivateGateway | null) => void
  refreshPrivateConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: PrivateConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Private...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Private…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Private gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshPrivateConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshPrivateConfig()
