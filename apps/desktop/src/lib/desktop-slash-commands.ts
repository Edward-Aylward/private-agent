...
/** Why a known Private command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Private profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Private Agent version', surface: exec() },
...
function isKnownPrivateSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Private' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownPrivateSlashCommand(normalized)
}
