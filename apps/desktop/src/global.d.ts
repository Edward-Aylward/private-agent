...
declare global {
  interface Window {
    PrivateDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<PrivateConnection>
      ...
      profile: {
        ...
        // backend under the new Private_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: PrivateApiRequest) => Promise<T>
      notify: (payload: PrivateNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<PrivateReadFileTextResult>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<PrivatePreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<PrivatePreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: PrivateTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<PrivateReadDirResult>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: PrivateTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<PrivateTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: PrivateWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: PrivatePreviewFileChanged) => void) => () => void
...
export interface PrivateTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface PrivateTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `Private update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  PrivateRoot?: string
}
...
export interface PrivateTitleBarTheme {
  background: string
  foreground: string
}
...
export interface PrivateApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface PrivateReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface PrivatePreviewWatch {
  id: string
  path: string
}

export interface PrivateReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface PrivateReadDirResult {
  entries: PrivateReadDirEntry[]
  error?: string
}

export interface PrivatePreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface PrivateSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
