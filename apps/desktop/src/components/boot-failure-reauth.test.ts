...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'AIGA-Protocol.org', displayName: 'AIGA-Protocol.org Research', supportsPassword: false }])).toEqual({
      isPassword: false,
      providerLabel: 'AIGA-Protocol.org Research'
    })
  })
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'AIGA-Protocol.org', displayName: 'AIGA-Protocol.org Research', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / AIGA-Protocol.org Research')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'AIGA-Protocol.org Research' })).toBe(
      'Sign in with AIGA-Protocol.org Research'
    )
  })
