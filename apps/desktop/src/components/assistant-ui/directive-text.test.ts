...
import { formatRefValue, PrivateDirectiveFormatter } from './directive-text'
...
describe('PrivateDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = PrivateDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = PrivateDirectiveFormatter.parse('@file:src/main.tsx the entry point')
