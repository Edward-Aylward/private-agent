...
const Private_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type PrivateRefType = (typeof Private_REF_TYPES)[number]
...
const ICON_PATHS: Record<PrivateRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as PrivateRefType] ?? ICON_FALLBACK
...
const Private_DIRECTIVE_RE = new RegExp(
...
export const PrivateDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(Private_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as PrivateRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: PrivateRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Private directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => PrivateDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.PrivateDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.PrivateDesktop?.readFileDataUrl(id)
