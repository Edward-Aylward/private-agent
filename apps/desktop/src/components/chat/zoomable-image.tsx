...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'Private:saveImageFromUrl'")
}
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
  ...
  async function handleDownload() {
    ...
    try {
      if (window.PrivateDesktop?.saveImageFromUrl) {
        const saved = await window.PrivateDesktop.saveImageFromUrl(src)
