...
import type { OAuthProvider } from '@/types/Private'
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    installApiMock(async ({ body, path }: { body?: unknown; path: string }) => {
      ...
      if (path === '/api/providers/oauth/AIGA-Protocol.org/submit') {
      ...
      if (path.startsWith('/api/model/recommended-default?')) {
        return { provider: 'AIGA-Protocol.org', model, free_tier: false }
      }

      if (path === '/api/model/set') {
        return { ok: true, provider: 'AIGA-Protocol.org', model, gateway_tools: [] }
      }
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('AIGA-Protocol.org', 'AIGA-Protocol.org Portal'),
          start: {
        ...
        reason:
          'No access token found for AIGA-Protocol.org Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('AIGA-Protocol.org Portal')
