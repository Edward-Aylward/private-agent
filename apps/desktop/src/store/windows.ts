...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.PrivateDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string): Promise<void> {
  ...
  try {
    const result = await window.PrivateDesktop.openSessionWindow(sessionId)
