...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/Private'
...
const CONFIGURED_CACHE_KEY = 'Private-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'Private-onboarding-skipped-v1'
const POLL_MS = 2000
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Private still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Private still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.PrivateDesktop?.openExternal) {
    try {
      await window.PrivateDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Private still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Private still cannot reach ${url}.` }
    }
