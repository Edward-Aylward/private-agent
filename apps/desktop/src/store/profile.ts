...
import { getProfiles, setApiRequestProfile } from '@/Private'
...
import type { ProfileInfo } from '@/types/Private'
...
// /api/profiles/active `current`). "default" is the root ~/.Private. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'Private.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'Private.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.PrivateDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new Private_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.PrivateDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'Private.desktop.showAllProfiles'
...
// Switch to the default (root ~/.Private) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.PrivateDesktop?.touchBackend?.(target).catch(() => undefined)
}
