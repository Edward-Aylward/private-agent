import type { ConnectionState, GatewayEvent } from '@Private/shared'
...
const isOpen = (gateway: PrivateGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<PrivateGateway | null>(null)
...
let primaryGateway: PrivateGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: PrivateGateway | null, profile = 'default'): void {
...
export function activeGateway(): PrivateGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<PrivateGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
