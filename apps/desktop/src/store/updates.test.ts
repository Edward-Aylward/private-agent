...
const checkPrivateUpdateSpy = vi.fn()
const updatePrivateSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/Private', () => ({
  checkPrivateUpdate: (...args: unknown[]) => checkPrivateUpdateSpy(...args),
  updatePrivate: (...args: unknown[]) => updatePrivateSpy(...args),
...
describe('checkBackendUpdates', () => {
  beforeEach(() => {
    storage.clear()
    notifySpy.mockClear()
    checkPrivateUpdateSpy.mockReset()
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkPrivateUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'Private update',
      message: null,
    ...
    expect(checkPrivateUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkPrivateUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updatePrivateSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkPrivateUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'Private update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updatePrivateSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkPrivateUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
