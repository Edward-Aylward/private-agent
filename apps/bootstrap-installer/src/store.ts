...
export const $PrivateHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, PrivateHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_Private_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $PrivateHome.set(PrivateHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Private" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Private-Setup.exe --update);
...
export async function launchPrivateDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_Private_desktop', { installRoot })
}
