...
//!
//! Mirrors `Private_constants.get_Private_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\Private
//!   macOS:   ~/.Private
//!   Linux:   ~/.Private  (override via $Private_HOME)
//!
//! NOTE (macOS): Python's get_Private_home(), scripts/install.sh, and the
//! Electron desktop's resolvePrivateHome() ALL use ~/.Private on macOS — there
...
/// Returns the canonical Private home directory, respecting $Private_HOME if set.
pub fn Private_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("Private_HOME") {
    ...
    {
        // %LOCALAPPDATA%\Private — matches scripts/install.ps1's $PrivateHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("Private");
        }
    }

    // macOS + Linux + fallback: ~/.Private (matches Python get_Private_home(),
    // install.sh, and the Electron desktop's resolvePrivateHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".Private");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    Private_home().join("bootstrap-cache")
}
...
/// Private_HOME so it survives repo checkout deletion (unlike anything under
/// Private-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\Private\Private-setup.exe`; on other
...
pub fn copy_self_to_Private_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to Private_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_Private_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".Private-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under Private_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[Private-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("Private_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_Private_home() -> String {
