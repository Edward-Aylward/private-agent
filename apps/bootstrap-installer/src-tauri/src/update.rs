...
//!
//! Driven when the installer is launched as `Private-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Private desktop process to fully exit (so the venv
//!      shim is free; otherwise `Private update` aborts with exit code 2),
//!   2. run `Private update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in Private_cli/main.py),
//!   3. run `Private desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `Private update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_Private) and
...
/// `Private update` exit code meaning "another Private process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// Private_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `Private update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Private"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let Private_home = crate::paths::Private_home();
    let install_root = Private_home.join("Private-agent");
    ...
    let Private = resolve_Private(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the Private CLI under {}. Is Private installed? \
    ...
    // async on Windows. If it still holds the venv shim, `Private update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: Private update -----------------------------------------
    // Pass --branch so `Private update` targets the branch this installer was
    ...
    // without --branch, `Private update` switches the checkout to `main` (a
    ...
    // --force skips `Private update`'s Windows running-exe guard (which would
    ...
    // time `Private update` runs there is no legitimate Private.exe to protect,
    // and the guard would only produce a false "Private is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `Private update` lazily imports
    ...
    // second `Private update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Private" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Private is still running. Close all Private windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "Private update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::Private_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: Private desktop --build-only ----------------------------
    // `Private update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `Private desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Private manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_Private_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_Private(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Private to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend Private.exe (or a grandchild it spawned)
            ...
            // every Private.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] Private still holding the venv shim; force-killing stragglers…",
            );
            force_kill_other_Private();
...
/// Force-kill any `Private.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `Private.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside `wait_for_venv_free`,
/// which completes BEFORE we spawn `venv\Scripts\Private.exe update`. At this
/// point no update-driven Private.exe exists yet, so the only Private.exe images
...
/// Spawn `Private <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv Private shim under an install root, regardless of existence.
fn venv_Private(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("Private.exe")
    } else {
        install_root.join("venv").join("bin").join("Private")
    }
}

/// Resolve the Private CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `Private` on PATH.
fn resolve_Private(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_Private(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "Private.exe" } else { "Private" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let Private_home = crate::paths::Private_home();
    let mut envs = vec![(
        "Private_HOME".to_string(),
        Private_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        Private_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_Private_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Private.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.Private-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.Private-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::Private_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_Private_is_under_install_root() {
        let root = Path::new("/x/Private-agent");
        let shim = venv_Private(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Private.app"]),
            Some(PathBuf::from("/Applications/Private.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Private.app");
        let tmp = base.join("Private.app.Private-update-new");
        let old = base.join("Private.app.Private-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Private.app");
        let tmp = base.join("Private.app.Private-update-new"); // intentionally absent
        let old = base.join("Private.app.Private-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Private.app");
        let tmp = base.join("Private.app.Private-update-new"); // absent
        let old = base.join("Private.app.Private-update-old");
        write_marker(&target, "OLD");
