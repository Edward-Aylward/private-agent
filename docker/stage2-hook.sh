...
#
# Wired into the image as /etc/cont-init.d/01-Private-setup by the
...
Private_HOME="${Private_HOME:-/opt/data}"
INSTALL_DIR="/opt/Private"

# Drop to Private via s6-setuidgid, but skip it when already non-root.
as_Private() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid Private "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/Private/
# .venv, ui-tui, node_modules) stay owned by the Private build UID (10000), so an
...
# into $Private_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass Private_UID/Private_GID — or the PUID/PGID aliases — which the
...
# `id -u Private` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# Private UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u Private)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-Private UID).
...
and the baked image directories are owned by the Private user (UID $(id -u Private)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e Private_UID=\$(id -u) -e Private_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the Private user to that UID/GID at boot and chowns the data
...
# --- Bootstrap Private_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid Private mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom Private_HOME paths whose parents only
# root can create (e.g. `Private_HOME=/home/Private/.Private` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for Private_UID/Private_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  Private_UID/
# Private_GID still win when both are set.  See #15290, salvages #25872.
Private_UID="${Private_UID:-${PUID:-}}"
Private_GID="${Private_GID:-${PGID:-}}"

if [ -n "${Private_UID:-}" ] && validate_uid_gid "$Private_UID" && [ "$Private_UID" != "$(id -u Private)" ]; then
    echo "[stage2] Changing Private UID to $Private_UID"
    usermod -u "$Private_UID" Private
fi
if [ -n "${Private_GID:-}" ] && validate_uid_gid "$Private_GID" && [ "$Private_GID" != "$(id -g Private)" ]; then
    echo "[stage2] Changing Private GID to $Private_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$Private_GID" Private 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised Private user (UID 10000)
...
# Private process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes Private. Idempotent across container
...
#   - Private is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G Private 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] Private already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" Private 2>/dev/null; then
        echo "[stage2] Added Private to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group Private failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When Private_UID is remapped or the top-level $Private_HOME isn't owned by
# the runtime Private UID, restore ownership to Private — but ONLY for the
# directories Private actually writes to. The full $Private_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of Private-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_Private_uid=$(id -u Private)
needs_chown=false
if [ "$(stat -c %u "$Private_HOME" 2>/dev/null)" != "$actual_Private_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $Private_HOME (targeted) to Private ($actual_Private_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $Private_HOME: chown the directory itself (not its contents)
    # so Private can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown Private:Private "$Private_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $Private_HOME failed (rootless container?) — continuing"
    # Private-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by Private (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$Private_HOME/$sub" ]; then
            chown -R Private:Private "$Private_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $Private_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Private-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime Private UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when Private_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     Private UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R Private:Private` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $Private_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $Private_HOME ownership check
# above. `usermod -u <new> Private` re-chowns the Private home dir
# ($Private_HOME == /opt/data) to the new UID as a side effect, so after a
# Private_UID/PUID remap `stat $Private_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/Private
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $Private_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime Private UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_Private_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to Private ($actual_Private_uid)"
    chown -R Private:Private \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $Private_HOME/profiles to Private on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> Private …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as Private and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$Private_HOME/profiles" ]; then
    chown -R Private:Private "$Private_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $Private_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged Private runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$Private_HOME/cron" ]; then
    chown -R Private:Private "$Private_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of Private-owned top-level state files on every boot.
# The targeted data-volume chown above only covers Private-owned
# *subdirectories*; loose state files living directly under $Private_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> Private …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged Private runtime then hits
...
# sweep so host-owned files in a bind-mounted $Private_HOME are never
...
# entries of Private_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$Private_HOME/$f" ]; then
        chown Private:Private "$Private_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the Private runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$Private_HOME/config.yaml" ]; then
    chown Private:Private "$Private_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$Private_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as Private user ---
# Run as Private via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $Private_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_Private mkdir -p \
    "$Private_HOME/cron" \
    "$Private_HOME/sessions" \
    "$Private_HOME/logs" \
    "$Private_HOME/hooks" \
    "$Private_HOME/memories" \
    "$Private_HOME/skills" \
    "$Private_HOME/skins" \
    "$Private_HOME/plans" \
    "$Private_HOME/workspace" \
    "$Private_HOME/home" \
    "$Private_HOME/pairing" \
    "$Private_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in Private status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the Private user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_Private tee "$Private_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$Private_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_Private cp "$INSTALL_DIR/$src" "$Private_HOME/$dest"
    fi
}
...
if [ -f "$Private_HOME/.env" ]; then
    chown Private:Private "$Private_HOME/.env" 2>/dev/null || true
    chmod 600 "$Private_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $Private_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `Private update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set Private_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$Private_HOME/config.yaml" ]; then
    s6-setuidgid Private "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$Private_HOME/auth.json" ] && [ -n "${Private_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$Private_AUTH_JSON_BOOTSTRAP" > "$Private_HOME/auth.json"
    chown Private:Private "$Private_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$Private_HOME/auth.json"
fi
...
# can set Private_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# Private_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$Private_HOME/gateway_state.json" ] && \
        [ "${Private_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$Private_HOME/gateway_state.json"
    chown Private:Private "$Private_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$Private_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_Private "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/Private/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Private spawns for the
...
# main-wrapper.sh propagates it into the supervised ``Private`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-Private, dashboard, etc.).
