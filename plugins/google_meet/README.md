...
Let the Private agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where Private runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `Private meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `Private meet` CLI |
...
| `cli.py` | `Private meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$Private_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `Private meet node {run,list,approve,remove,status,ping}` |
...
Private meet install                                      # pip + Chromium
Private meet setup                                        # preflight
Private meet auth                                         # optional
Private meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
Private meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.Private/.env
Private meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
Private meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
Private meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.Private/.env
Private meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, Private will **not** switch your system audio input automatically — the
...
Private plugins enable google_meet
Private meet node run --display-name my-mac --host 0.0.0.0 --port 18789
