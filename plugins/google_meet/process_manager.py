...
session-scoped state file under ``$Private_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $Private_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_Private_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this Private install, leave it first —
    ...
    env = os.environ.copy()
    env["Private_MEET_URL"] = url
    env["Private_MEET_OUT_DIR"] = str(out)
    env["Private_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["Private_MEET_HEADED"] = "1"
    if auth_state:
        env["Private_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["Private_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if Private_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["Private_MEET_MODE"] = mode
    if realtime_model:
        env["Private_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["Private_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["Private_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["Private_MEET_REALTIME_KEY"] = realtime_api_key
