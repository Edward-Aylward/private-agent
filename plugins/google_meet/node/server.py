...
Launched by ``Private meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$Private_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``Private meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_Private_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "Private-meet-node",
    ) -> None:
        self.host = host
        self.port = port
