"""`Private meet node ...` subcommand tree.

Wired into the existing ``Private meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``Private meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="Private-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``Private meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             Private meet node approve <name> ws://<host>:{args.port} {token}")
        try:
