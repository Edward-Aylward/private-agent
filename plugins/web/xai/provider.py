...
``{title, url, description, position}`` rows every other Private web
provider produces.
...
prefers Private-managed xAI Grok OAuth (via ``Private auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.Private/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``Private tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `Private auth` to sign in with "
