...
Optional Private observability plugin that maps Private observer hooks to
...
not replace Private Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Private emit NeMo Relay lifecycle events for
the work Private already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Private Agent can:

- Preserve Private execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
Private plugins enable observability/nemo_relay
```

The `Private_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `Private_HOME` that the
agent run will use:

```bash
env Private_HOME=/tmp/Private-nemo-relay-test \
  Private plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`Private_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`Private_HOME` is the Private profile/config home used by both
`Private plugins enable ...` and the later `Private chat ...` run. If unset,
Private uses the user's default home, usually `~/.Private`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export Private_HOME=/tmp/Private-nemo-relay-test
Private plugins enable observability/nemo_relay
Private chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `Private` command you run is built from the
...
uv run Private plugins enable observability/nemo_relay
uv run Private chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/Private_agent-*.whl
...
The plugin can configure exporters directly from `Private_NEMO_RELAY_*`
...
```bash
export Private_NEMO_RELAY_ATOF_ENABLED=1
export Private_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export Private_NEMO_RELAY_ATIF_ENABLED=1
export Private_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `Private_NEMO_RELAY_ATOF_FILENAME`
- `Private_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `Private_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `Private_NEMO_RELAY_ATIF_AGENT_NAME`
- `Private_NEMO_RELAY_ATIF_AGENT_VERSION`
- `Private_NEMO_RELAY_ATIF_MODEL_NAME`
- `Private_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Private at it:

```bash
export Private_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `Private_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`Private_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`Private_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Private does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Private keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Private routes LLM and tool
...
For the full generic Private middleware contract, see
...
export Private_HOME=/tmp/Private-nemo-relay-docs/Private-home
mkdir -p "$Private_HOME"

cat > "$Private_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Private session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export Private_NEMO_RELAY_ATOF_ENABLED=1
export Private_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/Private-nemo-relay-docs/subagent/atof
export Private_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export Private_NEMO_RELAY_ATOF_MODE=overwrite
export Private_NEMO_RELAY_ATIF_ENABLED=1
export Private_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/Private-nemo-relay-docs/subagent/atif
export Private_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export Private_NEMO_RELAY_ATIF_AGENT_NAME='Private Agent E2E'
export Private_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export Private_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"Private.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Private Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Private dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/Private-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/Private-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/Private-nemo-relay-docs/workdir/beta.txt
cd /tmp/Private-nemo-relay-docs/workdir

export Private_NEMO_RELAY_ATOF_ENABLED=1
export Private_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/Private-nemo-relay-docs/parallel/atof
export Private_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export Private_NEMO_RELAY_ATOF_MODE=overwrite
export Private_NEMO_RELAY_ATIF_ENABLED=1
export Private_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/Private-nemo-relay-docs/parallel/atif
export Private_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export Private_NEMO_RELAY_ATIF_AGENT_NAME='Private Agent E2E'
export Private_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Private sessions map to `agent` scopes.
- Private API request hooks map to `llm` scope start/end events.
- Private tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Private execution middleware to hand
...
Enable it for Private:

```bash
export Private_NEMO_RELAY_PLUGINS_TOML=/tmp/Private-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Private routes execution
...
Private provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Private provider adapter next_call(...)

Private tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Private tool dispatcher next_call(...)
```
...
middleware for a local Private run. This path requires a NeMo Relay runtime that
...
```bash
export Private_HOME=/tmp/Private-middleware-test/Private-home
mkdir -p "$Private_HOME" /tmp/Private-middleware-test/nemo-relay
...
cat > /tmp/Private-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/Private-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/Private-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Private Middleware E2E"
agent_version = "local"
...
export Private_NEMO_RELAY_PLUGINS_TOML=/tmp/Private-middleware-test/nemo-relay/plugins.toml
