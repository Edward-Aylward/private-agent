...
Auto-tracks and cleans up ephemeral files created during Private Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$Private_HOME` and `/tmp/Private-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `Private_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `Private_HOME` or `/tmp/Private-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$Private_HOME/disk-cleanup/` is itself excluded
- `$Private_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
