"""disk_cleanup — ephemeral file cleanup for Private Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under Private_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly Private_HOME and /tmp/Private-*
Never touches: ~/.Private/logs/ or any system directory.
"""
...
try:
    ...
    def get_Private_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("Private_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".Private").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$Private_HOME/logs/``."""
    return get_Private_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$Private_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under Private_HOME or ``/tmp/Private-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    Private_home = get_Private_home()
    try:
        path.resolve().relative_to(Private_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/Private-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("Private-"):
        return True
    return False
...
# Paths under $Private_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so Private_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        Private_home = get_Private_home()
        for parent in ("cron", "cronjobs"):
            base = Private_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside Private_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under Private_HOME (but leave Private_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "Private-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(Private_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == Private_home:
                continue
            try:
                rel_parts = dirpath.relative_to(Private_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    Private_home = get_Private_home()
    try:
        rel = path.resolve().relative_to(Private_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "Private-agent",
        }:
            return None
        ...
        # Path isn't under Private_HOME (e.g. /tmp/Private-*) — fall through.
        pass
