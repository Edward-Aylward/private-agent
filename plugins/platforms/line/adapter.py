"""
LINE Messaging API platform adapter for Private Agent.
...
to Private Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``Private_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            Private_home = Path.home().joinpath(".Private").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``Private status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``Private status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Private setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.Private/.env`` via ``Private_cli.config``.
    """
    print()
    ...
    try:
        from Private_cli.config import get_env_var, set_env_var
    except ImportError:
        print("Private_cli.config not available; set LINE_* vars manually in ~/.Private/.env")
        return
