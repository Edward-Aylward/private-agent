"""
IRC Platform Adapter for Private Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Private agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "Private-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Private Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Private Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: Private_, Private_1, Private_2...
...
def check_requirements() -> bool:
    ...
    # Private setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `Private gateway setup` flow for the IRC platform.

    Lazy-imports ``Private_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Private to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #Private — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.Private/.env")
    print_info("Restart the gateway for changes to take effect: Private gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "Private-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "Private-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Private Agent (cron)")
