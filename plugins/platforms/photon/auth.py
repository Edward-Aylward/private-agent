...
Credential storage mirrors every other Private channel:

    * runtime SDK creds  -> ``~/.Private/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.Private/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a Private dependency
...
# registers Private as its own client_id.
...
# Default name of the project Private provisions for the operator.
DEFAULT_PROJECT_NAME = "Private Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of Private-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.Private/auth.json`` honouring the active Private profile."""
    try:
        from Private_constants import get_Private_home
        return Path(get_Private_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.Private")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.Private/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.Private/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.Private/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: Private_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `Private photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `Private photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `Private photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `Private photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `Private photon setup`)"
