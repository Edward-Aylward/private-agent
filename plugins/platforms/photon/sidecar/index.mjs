// Private Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Private): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Private -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Private-Sidecar-Token: ${TOKEN}`):
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-Private-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Private supplied them so a known-good
