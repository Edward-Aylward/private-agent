...
Small Node helper that bridges Private Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Private is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Private-Sidecar-Token`)
...
The Private plugin's `Private photon setup` command runs `npm install`
here automatically.
