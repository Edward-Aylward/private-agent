"""
``Private photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Private
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `Private photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Private Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``Private photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Private Agent" project.
    ...
    #    and persist both (runtime creds -> ~/.Private/.env, ids -> auth.json).
    try:
    ...
    print("  Start the gateway:  Private gateway start --platform photon")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `Private photon install-sidecar`'}")
    return 0
...
#
# `Private gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``Private photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `Private gateway setup` wizard."""
