...
This plugin connects Private Agent to iMessage (and other Spectrum
...
TypeScript-only, Private runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` are loopback POSTs to the sidecar,
  authenticated with a shared `X-Private-Sidecar-Token`.
...
There is no separate `login` command; like every other Private channel,
...
Run `Private photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.Private/.env` (the same place every other
...
Management metadata lives in `~/.Private/auth.json` under `credential_pool`:
