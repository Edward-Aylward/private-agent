"""ntfy platform adapter (Private plugin).
...
``plugins/platforms/ntfy/``. The Private plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "Private-in"              # subscribe topic (incoming)
          publish_topic: "Private-out"     # optional — defaults to topic
...
_ECHO_TAG = "Private-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``Private cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Private dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `Private gateway status` without
