"""SimpleX Chat platform adapter (Private plugin).
...
This adapter ships as a Private platform plugin under
``plugins/platforms/simplex/``. The Private plugin loader scans the
...
Optional environment variables:
    ...
    Private_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``Private setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Private — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("Private_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Private setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.Private/.env`` via
    ``Private_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "Private_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.Private/.env"
        )
        return
