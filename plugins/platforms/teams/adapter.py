"""
Microsoft Teams platform adapter for Private Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Private"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        Private_action = data.get("Private_action", "")
        session_key = data.get("session_key", "")

        if not Private_action or not session_key:
        ...
        choice = choice_map.get(Private_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="Private_approve",
                    data={**btn_data_base, "Private_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="Private_approve",
                    data={**btn_data_base, "Private_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="Private_approve",
                    data={**btn_data_base, "Private_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="Private_approve",
                    data={**btn_data_base, "Private_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Private\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.Private/.env")
