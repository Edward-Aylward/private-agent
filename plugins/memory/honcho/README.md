...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/Private>
...
```bash
Private memory setup honcho   # configure Honcho directly (works on a fresh install)
Private memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
Private config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.Private/.env
```

> `Private honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `Private memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$Private_HOME/honcho.json` | Profile-local (isolated Private instances) |
| 2 | `~/.Private/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Private profile: `Private` (default) or `Private_<profile>`.
...
**Deployment shapes** (`Private memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Private to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Private session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`Private-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`Private`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-Private-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening Private in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Private session ID (timestamp + hex). Every `Private` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Private profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`Private`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Private profile: `Private` (default) or `Private_<profile>` (e.g. `Private -p coder` -> host key `Private_coder`). Older `Private.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `Private_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `Private memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `Private honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `Private memory setup`) |
| `Private honcho status` | Show resolved config for active profile |
| `Private honcho enable` / `disable` | Toggle Honcho for active profile |
| `Private honcho mode <mode>` | Change recall or observation mode |
| `Private honcho peer --user <name>` | Update user peer name |
| `Private honcho peer --ai <name>` | Update AI peer name |
| `Private honcho tokens --context <N>` | Set context token budget |
| `Private honcho tokens --dialectic <N>` | Set dialectic max chars |
| `Private honcho map <name>` | Map current directory to a session name |
| `Private honcho sync` | Create host blocks for all Private profiles |
