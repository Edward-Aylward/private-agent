...
Handles: Private honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "Private" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Private profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'Private honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'Private honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `Private update` -- no output, no exceptions.
    """
    try:
        from Private_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Private profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $Private_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_Private_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        Private_host.get("pinUserPeer"),
        Private_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in Private_host:
        aliases_src = Private_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in Private_host:
        prefix_src = Private_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(Private_host: dict) -> None:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Private persistent cross-session memory.")
    ...
    Private_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        current_host_key = Private_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            Private_host["apiKey"] = new_local_key
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'Private honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = Private_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        Private_host["peerName"] = new_peer

    current_ai = Private_host.get("aiPeer") or cfg.get("aiPeer", "Private")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        Private_host["aiPeer"] = new_ai

    current_workspace = Private_host.get("workspace") or cfg.get("workspace", "Private")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        Private_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, Private_host)
    ...
    if current_shape == "single" and new_shape == "multi":
        peer_target = Private_host.get("peerName") or current_peer or "user"
        print(
    ...
    if new_shape == "single":
        _scrub_identity_mapping(Private_host)
        Private_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{Private_host.get('peerName', '?')}'.")
        ...
        _scrub_identity_mapping(Private_host)
        Private_host["pinPeerName"] = False
        ...
        if prior_aliases:
            Private_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            Private_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'Private honcho status' to inspect.")
        ...
        if existing_aliases:
            Private_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = Private_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        Private_host["observationMode"] = new_obs
    else:
        Private_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(Private_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        Private_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        Private_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = Private_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        Private_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = Private_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        Private_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                Private_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(Private_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            Private_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        Private_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        Private_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        Private_host["dialecticReasoningLevel"] = new_reasoning
    else:
        Private_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = Private_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        Private_host["sessionStrategy"] = new_strat

    Private_host["enabled"] = True
    Private_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        Private_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(Private_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: Private config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    Private honcho status     -- show full config")
    print("    Private honcho mode       -- change recall/observation mode")
    print("    Private honcho tokens     -- tune context and dialectic budgets")
    print("    Private honcho peer       -- update peer names")
    print("    Private honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Private profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: Private honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'Private honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'Private honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: Private honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        Private = hosts.get(_host_key(), {})
        user = Private.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = Private.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = Private.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = Private.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Private' identity in Honcho. Seed with 'Private honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: Private honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: Private honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = Private.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = Private.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = Private.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Private asks Honcho's AI peer a question")
        ...
        print("\n  Set with: Private honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'Private honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'Private honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    Private honcho identity --show        — show both peer representations")
        print("    Private honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Private + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Private\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Private persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  Private honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'Private honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'Private honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  Private honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'Private'")
        print("  with Honcho configured and no prior session history.")
        print("  (Private calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    Private honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'Private honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Private, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'Private honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    Private honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  Private honcho identity <path/to/SOUL.md>")
    ...
    print("    Private:   cloud-backed Honcho peers. Files can stay on disk as source")
    print("              of truth; Honcho holds the live representation.")
    print()
    print("  Context injection")
    print("    OpenClaw: file excerpts injected synchroAIGA-Protocol.orgly before each LLM call.")
    print("    Private:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Private:   Honcho observes every message and updates representations")
    ...
    print("    Private:   per-session by default — each run gets its own session")
    print("              Map a custom name:  Private honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. Private honcho setup              — configure API key (required)")
        print("  2. Private honcho migrate            — re-run this walkthrough")
    else:
        print("  1. Private honcho status             — verify Honcho connection")
        print("  2. Private                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. Private honcho identity --show    — verify AI peer representation")
        print("  4. Private honcho tokens             — tune context and dialectic budgets")
        print("  5. Private honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'Private memory setup'...\n")
        from Private_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``Private honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``Private honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to Private memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Private Honcho",
    )
