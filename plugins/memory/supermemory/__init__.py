...
_DEFAULT_CONTAINER_TAG = "Private"
...
def _load_supermemory_config(Private_home: str) -> dict:
    config = _default_config()
    config_path = Path(Private_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, Private_home: str) -> None:
    config_path = Path(Private_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "Private"},
        )
    ...
    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        ...
        merged = {"sm_source": "Private", **(metadata or {})}
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `Private memory setup`.
        # All other options are documented for $Private_HOME/supermemory.json
    ...
    def save_config(self, values, Private_home):
        ...
        _save_supermemory_config(sanitized, Private_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from Private_constants import get_Private_home
        self._Private_home = kwargs.get("Private_home") or str(get_Private_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._Private_home)
