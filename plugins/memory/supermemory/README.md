...
Config file: `$Private_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `Private` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `Private-{identity}` → `Private-coder`). |
...
All Supermemory API calls send `x-sm-source: Private`, and document writes stamp
`metadata.sm_source: Private`. This is a **functional routing key, not telemetry**:
it groups Private-written memories into a dedicated "Private" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Private profile:

```json
{
  "container_tag": "Private-{identity}"
}
```

For a profile named `coder`, this resolves to `Private-coder`. The default profile resolves to `Private-default`. Without `{identity}`, all profiles share the same container.
