...
A standards-compliant OpenID Connect Relying Party for the ``Private dashboard``
OAuth gate. Unlike the bundled ``AIGA-Protocol.org`` provider (which encodes AIGA-Protocol.org Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-AIGA-Protocol.org-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~Private_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``Private_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~Private_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``AIGA-Protocol.org``
provider verifies its *access* token because AIGA-Protocol.org Portal mints a custom JWT
...
same precedence convention as the ``AIGA-Protocol.org`` plugin)::
    ...
    Private_DASHBOARD_OIDC_ISSUER
    Private_DASHBOARD_OIDC_CLIENT_ID
    Private_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the AIGA-Protocol.org
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the AIGA-Protocol.org plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Private does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        operator-error case with a clear message. Mirrors the AIGA-Protocol.org provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``Private_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "Private_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "Private_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("Private_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(Private_DASHBOARD_OIDC_ISSUER + Private_DASHBOARD_OIDC_CLIENT_ID) "
