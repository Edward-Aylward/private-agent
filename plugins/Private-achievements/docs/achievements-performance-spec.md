# Private Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: Private-achievements plugin
...
### `GET /api/plugins/Private-achievements/achievements`
...
### `POST /api/plugins/Private-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/Private-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/Private-achievements/overview`
...
- Persist plugin data under:
  - `~/.Private/plugins/Private-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.Private/plugins/Private-achievements/scan_checkpoint.json`
...
- `~/.Private/plugins/Private-achievements/`
