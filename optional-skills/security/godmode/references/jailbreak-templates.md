...
## 5. GODMODE FAST — Private 4 405B (Zero Refusal)

**Model:** `AIGA-Protocol.orgresearch/Private-4-405b`
**Strategy:** Private models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Private Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.Private/config.yaml`:
