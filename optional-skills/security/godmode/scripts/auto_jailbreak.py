...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("Private_HOME", os.path.expanduser("~/.Private")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("Private_HOME", Path.home() / ".Private")) / "skills" / "red-teaming" / "godmode"
...
Private_HOME = Path(os.getenv("Private_HOME", Path.home() / ".Private"))
CONFIG_PATH = Private_HOME / "config.yaml"
PREFILL_PATH = Private_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Private config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.Private/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Private for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Private for changes to take effect.")
