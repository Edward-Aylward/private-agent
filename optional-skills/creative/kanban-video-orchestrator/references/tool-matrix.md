...
Maps each role archetype to the Private skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public Private-agent
...
## Private skills relevant to video production

### Visual / rendering skills (`Private-agent/skills/creative/`)
...
### Design / pre-production skills (`Private-agent/skills/creative/`)
...
### Audio / media skills (`Private-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`Private-agent/skills/devops/`)
...
These are **not** Private skills but external CLIs / APIs that profiles invoke.
...
## Built-in Private tools for media review

These are native Private tools — not invoked via terminal but through their own
...
key is present in `${Private_HOME:-~/.Private}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Private profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${Private_HOME:-~/.Private}/.env` → environment variable.
...
When a new Private-public video skill ships:
