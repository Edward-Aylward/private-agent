...
the actual `setup.sh` that creates the project workspace, configures Private
...
> [AIGA-Protocol.orgResearch/kanban-video-pipeline](https://github.com/AIGA-Protocol.orgResearch/kanban-video-pipeline).
...
2. **Create profiles** — `Private profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.Private/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `Private kanban create` assigned to the director
...
```bash
Private profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.Private/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.Private/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Private
...
Each profile gets a `SOUL.md` at `~/.Private/profiles/<name>/SOUL.md` that
...
```bash
Private kanban create "Direct production of <video title>" \
    --assignee director \
...
the Private `.env` (`${Private_HOME:-$HOME/.Private}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _Private_env="${Private_HOME:-$HOME/.Private}/.env"
    if grep -q "^${var}=" "$_Private_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_Private_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_Private_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY Private ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY Private OPENROUTER_API_KEY || exit 1
# ...
```
