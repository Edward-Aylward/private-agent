...
Private kanban watch --tenant <project-slug>

# Snapshot of the board
Private kanban list --tenant <project-slug>
Private kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
Private kanban stats --tenant <project-slug>

# Visual dashboard (browser)
Private dashboard

# Inspect a specific task (includes comments + events)
Private kanban show <task-id>

# Follow a single task's event stream
Private kanban tail <task-id>
```

Verify available subcommands with `Private kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `Private kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `Private kanban block <id>` then `Private kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `Private kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `Private kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `Private profile list` (profile exists?), `Private status` (gateway/dispatcher up?), and verify tenant. |
...
Private kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
Private kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(Private kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
Private kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
Private kanban block <task-id>
Private kanban archive <task-id>

# Diagnose what happened
Private kanban show <task-id>      # task body, comments, recent events
Private kanban tail <task-id>      # follow the live event stream
Private kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `Private kanban create` for the director
...
while true; do
    clear
    Private kanban list --tenant <slug>
    echo "---"
    Private kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `Private kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `Private profile list` and start
