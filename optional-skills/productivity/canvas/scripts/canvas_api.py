#!/usr/bin/env python3
"""Canvas LMS API CLI for Private Agent.
...
def _check_config():
    ...
    if missing:
        Private_env = os.path.join(
            os.environ.get("Private_HOME", os.path.expanduser("~/.Private")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {Private_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Private Agent"
    )
