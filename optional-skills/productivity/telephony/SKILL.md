---
name: telephony
description: Give Private phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
- save provider credentials into `${Private_HOME:-~/.Private}/.env`
...
3. Buy it and save it into `${Private_HOME:-~/.Private}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `${Private_HOME:-~/.Private}/.env`
