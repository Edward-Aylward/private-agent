...
Usage (via cron with --no-agent):
    ...
    Private cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$Private_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Private-Watcher/1.0"})
