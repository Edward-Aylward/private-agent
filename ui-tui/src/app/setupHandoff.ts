import type { RunExternalProcess } from '@Private/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`Private ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching Private: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`Private ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
