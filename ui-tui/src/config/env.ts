import type { MouseTrackingMode } from '@Private/ink'
...
export const STARTUP_RESUME_ID = (process.env.Private_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.Private_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.Private_TUI_IMAGE ?? '').trim()
...
//
// - Private_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - Private_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.Private_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.Private_TUI_DISABLE_MOUSE)
...
export const NO_CONFIRM_DESTRUCTIVE = truthy(process.env.Private_TUI_NO_CONFIRM)

// Private_DEV_CREDITS — dev-only live-spend readout (Δ status segment + "(dev credits)"
// banner). Throwaway dev scaffolding; the whole readout gates on this one flag.
export const DEV_CREDITS_MODE = truthy(process.env.Private_DEV_CREDITS)

const inlineOverride = parseToggle(process.env.Private_TUI_INLINE)
...
// less fragile. Override explicitly with Private_TUI_INLINE=0/1.
...
// synthetic timer).
export const SHOW_FPS = truthy(process.env.Private_TUI_FPS)
