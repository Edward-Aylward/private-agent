 ...
 * (`openCommand` returned null), or (c) `spawn()` threw synchroAIGA-Protocol.orgly
...
export function openExternalUrl(rawUrl: string, dependencies: OpenDependencies = {}): boolean {
  ...
  try {
    ...
    // Async failure path: spawn returns a ChildProcess synchroAIGA-Protocol.orgly even
    ...
    // consumer; we already returned `true` synchroAIGA-Protocol.orgly, so the user
    ...
    // spawn can also throw synchroAIGA-Protocol.orgly on argv-validation failures
