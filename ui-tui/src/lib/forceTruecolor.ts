...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.Private_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Private on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via Private_TUI_TRUECOLOR=1.
