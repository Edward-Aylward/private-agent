...
const logDir = join(process.env.Private_HOME?.trim() || join(homedir(), '.Private'), 'logs')
...
// a real ~/.Private (tests must stay hermetic — see AGENTS.md).
...
// or add noticeable blocking on the synchroAIGA-Protocol.org append. Mirrors the spirit of
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('Private-tui: parent lifecycle log unavailable\n')
    }
  }
}
