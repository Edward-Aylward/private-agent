...
await build({
  ...
  // Skip the prebuilt @Private/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@Private/ink': resolve(root, 'packages/Private-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The Private_cli launcher
