...
React + Ink terminal UI for Private. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `Private_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.Private/.Private_history` or under `Private_HOME`.
...
```text
ui-tui/
  packages/Private-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      Private-ink.d.ts    type declarations for @Private/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent PrivateCLI subprocess for slash commands
```
